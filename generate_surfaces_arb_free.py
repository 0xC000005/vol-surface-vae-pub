...
if check_ex_feats:
    for i in range(ctx_len, ctx_len+1):
        ...
        if not os.path.exists(f"test_spx/cal_arb_free/{file_name}_gen{i}.npz"):
            os.makedirs("test_spx/cal_arb_free", exist_ok=True)
            ...
            np.savez(f"test_spx/cal_arb_free/{file_name}_gen{i}.npz", surfaces=surfaces, ex_feats=ex_feats)
else:
    for i in range(ctx_len, ctx_len+1):
        set_seeds(0)
        print(f"currently generating {i}")
        if not os.path.exists(f"test_spx/cal_arb_free/{file_name}_gen{i}.npy"):
            os.makedirs("test_spx/cal_arb_free", exist_ok=True)
            surfaces, _ = generate_surfaces_multiday(model_data = model_data,
                    ex_data = ex_data, vol_surface_data = vol_surf_data,
                    start_day=start_day, days_to_generate=days_to_generate, num_vaes = num_vaes,
                    model_type = CVAEMemRand, check_ex_feats=check_ex_feats, ctx_len=i)
            np.save(f"test_spx/cal_arb_free/{file_name}_gen{i}.npy", surfaces)