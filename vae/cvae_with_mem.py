...
class CVAEMemEncoder(BaseEncoder):
    def __init__(self, config: dict):
        ...
        '''
        super(CVAEMemEncoder, self).__init__(config)
    ...
    def forward(self, x):
        ...
        surface_embedding = surface_embedding.reshape((-1, seq_len, surface_embedding.shape[1])) # (B, T, surface_embedding_final_dim)
        ...
        if "ex_feats" in x:
            ...
            ex_feats_embedding = ex_feats_embedding.reshape((-1, seq_len, ex_feats_embedding.shape[1])) # (B, T, ex_feats_embedding_final_dim)
...
class CVAECtxMemEncoder(BaseEncoder):
    def __init__(self, config: dict):
        '''
        ...
        '''
        super(CVAECtxMemEncoder, self).__init__(config)
    ...
    def forward(self, x):
        ...
        surface_embedding = surface_embedding.reshape((-1, ctx_len, surface_embedding.shape[1]))
        ...
        if "ex_feats" in x:
            ...
            ex_feats_embedding = ex_feats_embedding.reshape((-1, ctx_len, ex_feats_embedding.shape[1])) # (B, T, ex_feats_embedding_final_dim)
...
class CVAEMemDecoder(BaseDecoder):
    def __init__(self, config: dict):
        '''
        ...
        super(CVAEMemDecoder, self).__init__(config)
...
class CVAEMem(BaseVAE):
    def __init__(self, config: dict):
        '''
            Conditional VAE with support for both linear and convolutional encoding/decoding. 
            The input size should be (B,T,H,W), sequence length will be used as the C channel
            
            Input:
                config: must contain seq_len, feat_dim, latent_dim, device, kl_weight, re_feat_weight, surface_hidden, ex_feats_dim, ex_feats_hidden, mem_type, mem_hidden, mem_layers, mem_dropout, ctx_len, ctx_surface_hidden, ctx_ex_feats_hidden
        ...
        '''
        super(CVAEMem, self).__init__(config)
        ...
        self.encoder = CVAEMemEncoder(config)
        self.ctx_encoder = CVAECtxMemEncoder(config)
        self.decoder = CVAEMemDecoder(config)
        self.to(self.device)
    ...
    def get_surface_given_conditions(self, c: dict[str, torch.Tensor], z: torch.Tensor=None, mu=0, std=1):
        ...
        assert ctx_surface.shape[1] == ctx_len, "context length mismatch"
        ...
        if "ex_feats" in c:
            ...
            assert ctx_ex_feats.shape[1] == ctx_len, "context length mismatch"
            ctx["ex_feats"] = ctx_ex_feats
        if z is not None:
            if len(z.shape) == 2:
                z = z.unsqueeze(0)
            assert z.shape[1] == self.config["seq_len"], "latent time dim mismatch"
            assert z.shape[2] == self.config["latent_dim"], "latent dim mismatch"
        else:
            z = mu + torch.randn((ctx_surface.shape[0], self.config["seq_len"], self.config["latent_dim"])) * std
    ...
    def forward(self, x: dict[str, torch.Tensor]):
        ...
        if len(surface.shape) == 3 and surface.shape[0] == self.config["seq_len"]:
            # unbatched data
            surface = surface.unsqueeze(0)
        ctx_surface = surface[:, :ctx_len, :, :] # c
        ctx_encoder_input = {"surface": ctx_surface}

        encoder_input = {"surface": surface}
        if "ex_feats" in x:
            ex_feats = x["ex_feats"].to(self.device)
            if len(ex_feats.shape) == 2 and ex_feats.shape[0] == self.config["seq_len"]:
                ex_feats = ex_feats.unsqueeze(0)
            ctx_ex_feats = ex_feats[:, :ctx_len, :]
        ...
        if "ex_feats" in x:
            ...
            return decoded_surface[:, ctx_len:, :, :], decoded_ex_feat[:, ctx_len:, :], z_mean, z_log_var, z
        else:
            decoded_surface = self.decoder(decoder_input) # P(x|c,z,t)

            return decoded_surface[:, ctx_len:, :, :], z_mean, z_log_var, z
    ...
    def train_step(self, x, optimizer: torch.optim.Optimizer):
        ...
        if len(surface.shape) == 3 and surface.shape[0] == self.config["seq_len"]:
            # unbatched data
            surface = surface.unsqueeze(0)
        surface_real = surface[:,ctx_len:, :, :].to(self.device)

        if "ex_feats" in x:
            ex_feats = x["ex_feats"]
            if len(ex_feats.shape) == 2 and ex_feats.shape[0] == self.config["seq_len"]:
                ex_feats = ex_feats.unsqueeze(0)
            ex_feats_real = ex_feats[:, ctx_len:, :,].to(self.device)
