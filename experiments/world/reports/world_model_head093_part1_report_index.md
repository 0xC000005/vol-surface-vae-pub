...
- HEAD136: `world_model_head136_scale_representation_surface.md` runs that
  ...
  readouts.
- HEAD137: `world_model_head137_context_target_jepa_design.md` defines the
...
- HEAD139: `world_model_head139_context_target_model_scaffold.md` adds the
  minimal latent context-to-target model and target-time masked loss. It is
  model/loss scaffolding only, not a trained context-to-target result.
- HEAD140: `world_model_head140_context_target_smoke.md` runs the first
  context-to-target training smoke. It is runnable and loss decreases, but
  representation health is weak, so it remains `DO_NOT_PROMOTE`.
- HEAD141: `world_model_head141_context_target_state_probe.md` compares the
  HEAD140 context encoder against scaled Barlow on identical frozen
  present-state probes. It is worse on current-IV retention (`0.015289` versus
  scaled Barlow `0.013756`) and has weaker rank (`11.59` versus `18.76`), so
  the minimal context-to-target branch remains `DO_NOT_PROMOTE`.
- HEAD142: `world_model_head142_context_target_latent_health.md` diagnoses why
  HEAD140/141 failed. The target latent is strongly mask-family identifiable,
  while the predicted latent is low-rank; high cosine (`0.979`) coexists with
  poor row retrieval (`top10=0.043`), so low latent MSE is not enough.
- HEAD143: `world_model_head143_context_target_clean_target_gate.md` maps the
  HEAD142 failure to the canonical JEPA target rule. It authorizes one
  clean-target context-to-target smoke where the target encoder sees the clean
  full window and target rows are selected from its output.
- HEAD144: `world_model_head144_context_target_clean_smoke.md` runs that
  clean-target smoke. It trains, but clean context rank falls to `7.32`, worse
  than HEAD140 and far below scaled Barlow, so it remains `DO_NOT_PROMOTE`.
- HEAD145: `world_model_head145_context_target_clean_quality.md` compares the
  clean-target checkpoint on present-state probes. It is worse than target-only
  and scaled Barlow on current-IV retention and rank, so the minimal
  context-to-target route remains blocked.
- HEAD146: `world_model_head146_context_target_route_decision.md` demotes the
  minimal context-to-target route and keeps scaled Barlow as the best known
  Part 1 candidate, still `DO_NOT_PROMOTE`.
- HEAD147: `world_model_head147_part1_candidate_decision_matrix.md`
  consolidates candidate status: scaled Barlow is active but not promoted, raw
  surface is the exact-state floor, and minimal context-to-target is demoted.
- HEAD148: `world_model_head148_scaled_gate_reconciliation.md` confirms the
  context-to-target negatives do not change the formal scaled Part 1 gate:
  scaled Barlow remains active but `DO_NOT_PROMOTE`.
- HEAD149: `world_model_head149_scale_exact_state_topology.md` decomposes the
  exact-state blocker topologically. The gap is broad across IV cells, but
  strongest in wing moneyness and edge maturities.
- HEAD150: `world_model_head150_surface_local_jepa_design_gate.md` opens a
  token/geometry-level context-to-target design route without implementing it or
  changing Part B gating.
- HEAD151: `world_model_head151_surface_local_data_contract.md` adds the
  surface-local token/geometry target data contract and focused test. It is
  scaffolding only.
- HEAD152: `world_model_head152_surface_local_target_coverage.md` audits the
  data contract target coverage and confirms it covers wing/edge problem
  regions including `iv_m0_t0`.
- HEAD153: `world_model_head153_surface_local_model_scaffold.md` adds the
  surface-local token/geometry model and loss scaffold with explicit target
  token-row selection. It is not trained evidence.
- HEAD154: `world_model_head154_surface_local_jepa_smoke.md` runs the first
  surface-local token JEPA smoke. It trains but remains weak on target-token
  retrieval and rank, so it is not promoted.
- HEAD155: `world_model_head155_surface_local_smoke_failure.md` diagnoses that
  the weak smoke is low-rank in the target latent itself and has additional
  predictor variance shrinkage.
- HEAD156: `world_model_head156_surface_local_target_geometry.md` audits
  geometry/family separability and shows target latents cluster by token/factor
  while exact-row retrieval remains weak.
- HEAD157: `world_model_head157_surface_local_route_decision.md` demotes the
  current surface-local context-to-target route as implemented and blocks small
  knob tuning.
- HEAD158: `world_model_head158_post_surface_local_candidate_consolidation.md`
  consolidates the post-demotion candidate table and keeps scaled Barlow as the
  active but unpromoted learned candidate.
...
- HEAD160: `world_model_head160_part1_gate_reconciliation_after_surface_local.md`
  ...
  blocked.
- HEAD161: `world_model_head161_next_work_wording_refresh.md` refreshes active
  next-work wording so future resumes do not re-enter completed or demoted
  context-to-target tuning tasks.
- HEAD162: `world_model_head162_active_doc_stale_route_scan.md` scans active
...
26. `experiments/world/reports/world_model_head139_context_target_model_scaffold.md`
27. `experiments/world/reports/world_model_head140_context_target_smoke.md`
28. `experiments/world/reports/world_model_head141_context_target_state_probe.md`
29. `experiments/world/reports/world_model_head142_context_target_latent_health.md`
30. `experiments/world/reports/world_model_head143_context_target_clean_target_gate.md`
31. `experiments/world/reports/world_model_head144_context_target_clean_smoke.md`
32. `experiments/world/reports/world_model_head145_context_target_clean_quality.md`
33. `experiments/world/reports/world_model_head146_context_target_route_decision.md`
34. `experiments/world/reports/world_model_head147_part1_candidate_decision_matrix.md`
35. `experiments/world/reports/world_model_head148_scaled_gate_reconciliation.md`
36. `experiments/world/reports/world_model_head149_scale_exact_state_topology.md`
37. `experiments/world/reports/world_model_head150_surface_local_jepa_design_gate.md`
38. `experiments/world/reports/world_model_head151_surface_local_data_contract.md`
39. `experiments/world/reports/world_model_head152_surface_local_target_coverage.md`
40. `experiments/world/reports/world_model_head153_surface_local_model_scaffold.md`
41. `experiments/world/reports/world_model_head154_surface_local_jepa_smoke.md`
42. `experiments/world/reports/world_model_head155_surface_local_smoke_failure.md`
43. `experiments/world/reports/world_model_head156_surface_local_target_geometry.md`
...
50. `experiments/world/reports/world_model_head089_part1_open_risk_ledger.md`
51. `experiments/world/reports/world_model_head086_downstream_probe_interpretation.md`
52. `experiments/world/reports/world_model_head097_mask_policy_coverage_audit.md`
53. `experiments/world/reports/world_model_head096_stop_condition_verification.md`
54. `experiments/world/reports/world_model_head100_sample_scale_caveat.md`
55. `experiments/world/reports/world_model_head102_downstream_probe_reporting_audit.md`
56. `experiments/world/reports/world_model_head105_downstream_target_scope_audit.md`
57. `experiments/world/reports/world_model_head107_manual_stop_runtime_guardrail.md`
58. `experiments/world/reports/world_model_head110_package_checker_guardrail_docs.md`
59. `experiments/world/reports/world_model_head112_open_risk_ledger_refresh.md`
60. `experiments/world/reports/world_model_head116_goal_checker_manual_stop.md`
