#!/bin/bash
#
# Generate VAE Teacher Forcing Sequences - All Periods with Configurable Sampling
#
# Generates full in-sequence data for all periods and horizons:
# - crisis: ~3 min (4 horizons)
# - insample: ~15 min (4 horizons)
# - oos: ~3 min (4 horizons)
# - gap: ~3 min (4 horizons)
# Total: ~25-30 minutes
#
...
#
#   # Run in background
#   nohup bash experiments/backfill/context20/run_generate_all_tf_sequences.sh prior > vae_tf_prior.log 2>&1 &
#
...
# Log file
LOGFILE="vae_tf_generation_${SAMPLING_MODE}_$(date +%Y%m%d_%H%M%S).log"
...
# Start
log_section "VAE TEACHER FORCING SEQUENCE GENERATION - START"
...
if [ ! -f "experiments/backfill/context20/generate_vae_tf_sequences.py" ]; then
    log "${RED}ERROR: Must run from repository root!${NC}"
    log "Current directory: $(pwd)"
    log "Expected file: experiments/backfill/context20/generate_vae_tf_sequences.py"
    exit 1
fi

# Check if model exists
if [ ! -f "models/backfill/context20_production/backfill_16yr.pt" ]; then
    log "${RED}ERROR: Model file not found!${NC}"
    log "Expected: models/backfill/context20_production/backfill_16yr.pt"
    exit 1
fi
...
log "Sampling mode: ${SAMPLING_MODE}"
log "Expected runtime: ~3 minutes"
log "Output: 4 files (H=1,7,14,30)"
echo ""

CRISIS_START=$(date +%s)
PYTHONPATH=. python experiments/backfill/context20/generate_vae_tf_sequences.py --period crisis --sampling_mode ${SAMPLING_MODE} 2>&1 | tee -a "$LOGFILE"
CRISIS_STATUS=$?
CRISIS_END=$(date +%s)
...
log "Sampling mode: ${SAMPLING_MODE}"
log "Expected runtime: ~15 minutes"
log "Output: 4 files (H=1,7,14,30)"
echo ""

INSAMPLE_START=$(date +%s)
PYTHONPATH=. python experiments/backfill/context20/generate_vae_tf_sequences.py --period insample --sampling_mode ${SAMPLING_MODE} 2>&1 | tee -a "$LOGFILE"
INSAMPLE_STATUS=$?
INSAMPLE_END=$(date +%s)
...
OOS_START=$(date +%s)
PYTHONPATH=. python experiments/backfill/context20/generate_vae_tf_sequences.py --period oos --sampling_mode ${SAMPLING_MODE} 2>&1 | tee -a "$LOGFILE"
OOS_STATUS=$?
OOS_END=$(date +%s)
...
GAP_START=$(date +%s)
PYTHONPATH=. python experiments/backfill/context20/generate_vae_tf_sequences.py --period gap --sampling_mode ${SAMPLING_MODE} 2>&1 | tee -a "$LOGFILE"
GAP_STATUS=$?
GAP_END=$(date +%s)
...
log "Checking 16 files (4 periods × 4 horizons)"
echo ""

VALIDATE_START=$(date +%s)
PYTHONPATH=. python experiments/backfill/context20/validate_vae_tf_sequences.py --sampling_mode ${SAMPLING_MODE} 2>&1 | tee -a "$LOGFILE"
VALIDATE_STATUS=$?
VALIDATE_END=$(date +%s)
...
OUTPUT_DIR="results/vae_baseline/predictions/autoregressive/${SAMPLING_MODE}"
if [ -d "$OUTPUT_DIR" ]; then
    ...
    log "Total files: $FILE_COUNT / 16 expected"
...
log "  2. Compute CI statistics:"
log "     python experiments/backfill/context20/compute_sequence_ci_width_stats.py --sampling_mode ${SAMPLING_MODE}"
log "  3. Generate visualizations:"
log "     python experiments/backfill/context20/visualize_sequence_ci_width_combined.py --sampling_mode ${SAMPLING_MODE}"
echo ""
