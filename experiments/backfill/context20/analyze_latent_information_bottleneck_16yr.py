#!/usr/bin/env python3
"""
Latent Information Bottleneck Analysis for Context20 Production Model (16yr)

This script analyzes the context20 production model to understand its latent space
health and compare with context60 models.

Context20 Model Specs:
- Context length: 20 days
- Latent dimensions: 5
- KL weight: 1e-5
- Training data: 16 years (2004-2019)
- Quantile regression enabled

Analysis Goals:
- KL divergence (mean ± std)
- PC1, PC2, PC3 variance percentages
- Effective dimensionality (components for 90%)
...
MODEL_PATH = PROJECT_ROOT / "models" / "backfill" / "context20_production" / "backfill_16yr.pt"
DATA_PATH = PROJECT_ROOT / "data" / "vol_surface_with_ret.npz"
OUTPUT_DIR = PROJECT_ROOT / "results" / "backfill_16yr" / "analysis" / "latent_bottleneck"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

print("=" * 80)
print("LATENT INFORMATION BOTTLENECK ANALYSIS - CONTEXT20 PRODUCTION (16YR)")
print("=" * 80)
print()
...
print(f"Context length: {config.get('context_len', 20)}")
print()
...
for idx in start_indices:
    context = test_surfaces[idx:idx+context_len]  # (20, 5, 5)
    contexts.append(context)
    context_endpoints.append(context[-1, 2, 2])  # ATM endpoint

contexts = np.array(contexts)  # (N, 20, 5, 5)
...
if len(similar_pairs) > 10:
    ...
    ax.set_title(f'Context20: Latent vs Prediction Distance for Similar Contexts\n' +
...
ax.set_title('Context20: PCA Scree Plot', fontsize=13)
ax.legend()
...
ax.set_title('Context20: Cumulative Variance Explained', fontsize=13)
ax.legend()
ax.grid(alpha=0.3)
...
ax.set_title('Context20: Latent Space (2D PCA) Colored by Context Endpoint', fontsize=13)
...
# Save metrics for comparison script
metrics = {
    'kl_divergence': np.nan,  # Will be computed separately
    'latent_dim': latent_z_final.shape[1],
    'pc1_variance': explained_variance_ratio[0],
    'pc2_variance': explained_variance_ratio[1],
    'pc3_variance': explained_variance_ratio[2] if len(explained_variance_ratio) > 2 else 0.0,
    'effective_dim': n_components_90,
    'effective_ratio': effective_ratio,
    'correlation': correlation if not np.isnan(correlation) else 0.0,
    'p_value': p_value if not np.isnan(correlation) else 1.0,
    'day1_p50_mean': day1_p50.mean(),
    'day1_p50_std': day1_p50.std(),
    'latent_mean': latent_z_final.mean(),
    'latent_std': latent_z_final.std(),
}

np.savez(OUTPUT_DIR / 'metrics.npz', **metrics)
print(f"Saved: {OUTPUT_DIR / 'metrics.npz'}")
print()

# === SUMMARY ===
print("=" * 80)
print("SUMMARY - CONTEXT20 PRODUCTION MODEL")
print("=" * 80)
print()

print(f"Latent Dimension: {latent_z_final.shape[1]}")
print(f"PC1 Variance: {explained_variance_ratio[0] * 100:.2f}%")
print(f"PC2 Variance: {explained_variance_ratio[1] * 100:.2f}%")
print(f"PC3 Variance: {explained_variance_ratio[2] * 100:.2f}%" if len(explained_variance_ratio) > 2 else "")
print(f"Effective Dimensionality: {n_components_90}/{latent_z_final.shape[1]} ({effective_ratio * 100:.1f}%)")
if not np.isnan(correlation):
    print(f"Latent-Prediction Correlation: {correlation:.3f}")
print()
