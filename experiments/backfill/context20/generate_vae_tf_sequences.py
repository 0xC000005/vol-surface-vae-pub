"""
Generate VAE Teacher Forcing Sequences with Configurable Sampling Strategy

Generates full in-sequence data for all horizons (H=1, 7, 14, 30) using teacher forcing.
...
- **prior**: Prior sampling via model.get_surface_given_conditions()
            ...
            Always conditions on real 20-day context (no prediction feedback)
...
Output: NPZ files in results/vae_baseline/predictions/autoregressive/{sampling_mode}/
...
def generate_and_save_horizon(model, vol_surf, ex_data,
    ...
    Args:
        ...
        horizon: Forecast horizon (1, 7, 14, or 30)
    ...
    output_dir = Path(f"results/vae_baseline/predictions/autoregressive/{sampling_mode}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate VAE teacher forcing sequences with configurable sampling strategy'
    )
    ...
    print("=" * 80)
    print("VAE TEACHER FORCING SEQUENCE GENERATION")
    print("=" * 80)
    ...
    print("Loading model...")
    model_file = "models/backfill/context20_production/backfill_16yr.pt"
    ...
    print(f"\nOutput directory: results/vae_baseline/predictions/autoregressive/{args.sampling_mode}/")
    print(f"Files created:")
    for h in horizons:
        print(f"  - vae_tf_{args.period}_h{h}.npz")
    print()
    print("Next steps:")
    print(f"  1. Validate outputs: python experiments/backfill/context20/validate_vae_tf_sequences.py --sampling_mode {args.sampling_mode}")
    print(f"  2. Compute CI stats: python experiments/backfill/context20/compute_sequence_ci_width_stats.py --sampling_mode {args.sampling_mode}")
    print()
