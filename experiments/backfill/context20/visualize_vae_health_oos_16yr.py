"""
Comprehensive VAE Health Visualizations for backfill_16yr Model - OUT-OF-SAMPLE

Creates visualizations for test set (2019-2023):
1. Posterior collapse (KL divergence heatmaps, bar charts)
2. Latent space structure (PCA, variance analysis)
3. Temporal dynamics (time series of latent evolution)
4. Reconstruction quality correlation

Note: Test set is entirely post-crisis, so no crisis/normal regime comparison.
"""
import numpy as np
...
print("=" * 80)
print("VAE HEALTH VISUALIZATIONS - backfill_16yr (OUT-OF-SAMPLE)")
print("=" * 80)
print()

# Create output directory
output_dir = Path("models/backfill/vae_health_figs_oos")
...
data = np.load("models/backfill/vae_health_oos_16yr.npz", allow_pickle=True)

latent_dim = int(data['latent_dim'])
context_len = int(data['context_len'])
horizons = data['horizons']
test_start = int(data['test_start'])
test_end = int(data['test_end'])

print(f"  Latent dim: {latent_dim}")
print(f"  Context length: {context_len}")
print(f"  Horizons: {horizons}")
print(f"  Test period: [{test_start}, {test_end}] (2019-2023)")
print()
...
ax.set_title('Per-Dimension KL Divergence Across Horizons (OUT-OF-SAMPLE)\n(Green = Higher KL = Active, Red = Lower KL = Collapsed)')
...
ax.set_title(f'Active vs Collapsed Dimensions - OUT-OF-SAMPLE (Threshold: KL < {collapse_threshold})')
ax.legend()
...
for i, h in enumerate(horizons):
    ...
    ax.set_title(f'Horizon {h}: Context vs Future KL (OOS)')
    ax.set_xticks(x)
    ax.legend()
    ax.grid(alpha=0.3)
...
ax.set_title('Per-Dimension Latent Variance Across Horizons (OUT-OF-SAMPLE)')
...
ax.set_title('Effective Dimensionality vs Horizon - OUT-OF-SAMPLE\n(Participation Ratio = (Σ var)² / Σ(var²))')
ax.grid(alpha=0.3)
ax.legend()
ax.set_ylim([0, latent_dim + 0.5])
...
# Plot (single color - all test set data)
fig, ax = plt.subplots(figsize=(10, 8))
ax.scatter(z_pca[:, 0], z_pca[:, 1],
           c='steelblue', alpha=0.4, s=20, label='Test Set (2019-2023)')
ax.set_xlabel(f'PC1 ({pca.explained_variance_ratio_[0]*100:.1f}% variance)')
ax.set_ylabel(f'PC2 ({pca.explained_variance_ratio_[1]*100:.1f}% variance)')
ax.set_title('PCA of Latent Representations (H30) - OUT-OF-SAMPLE')
ax.legend()
ax.grid(alpha=0.3)

plt.tight_layout()
plt.savefig(output_dir / 'pca_latent_space.png', dpi=300, bbox_inches='tight')
plt.close()
print("  ✓ Saved pca_latent_space.png")
...
# 3.1 Time series of latent means (H30 only)
h30_z_means = data['h30_z_means']  # (N, T, latent_dim)
...
for d in range(latent_dim):
    ax = axes[d]
    ax.plot(h30_date_indices, latent_means_over_time[:, d], linewidth=0.5, alpha=0.7, color='steelblue')
    ax.set_ylabel(f'Dim {d}')
    ax.grid(alpha=0.3)

axes[-1].set_xlabel('Date Index')
axes[0].set_title('Temporal Evolution of Mean Latent Dimensions (H30) - OUT-OF-SAMPLE')
...
sns.heatmap(corr_data.T, annot=True, fmt='.3f', cmap='RdBu_r', center=0,
            ...
            ax=ax, cbar_kws={'label': 'Correlation'}, vmin=-0.5, vmax=0.5)
ax.set_xlabel('Horizon')
ax.set_ylabel('Latent Statistic')
ax.set_title('Correlation Between Latent Statistics and RMSE - OUT-OF-SAMPLE\n(Positive = Higher latent stat → Higher RMSE)')
...
fig, ax = plt.subplots(figsize=(10, 6))
ax.scatter(latent_l2_norm, h30_rmse,
           c='steelblue', alpha=0.4, s=10, label='Test Set')
ax.set_xlabel('Latent L2 Norm (Future Timesteps)')
ax.set_ylabel('RMSE')
ax.set_title(f'Latent L2 Norm vs RMSE (H30) - OUT-OF-SAMPLE\nCorrelation: {data["h30_corr_l2"]:.3f}')
ax.legend()
ax.grid(alpha=0.3)
...
print()
print("=" * 80)
print("VISUALIZATION COMPLETE (OUT-OF-SAMPLE)")
print("=" * 80)
print()
...
print()
print("Note: Test set (2019-2023) is entirely post-crisis,")
print("      so no crisis/normal regime comparison visualizations.")
print()
