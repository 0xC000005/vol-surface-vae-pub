...
Principle:
  - keep the 183c/184b mean/covariance/pathwise residual backbone
  - keep the latent activity process from 184b
  - replace diffuse local activity modulation with a sparse node allocator and
    a learned sparse neighborhood kernel over the current geometry
  - use that operator to sharpen local residual concentration without giving up
    the generalized geometry abstraction
"""
...
class SparsePrecisionTransportModel(StateMetricTransportModel):
    def __init__(
        self,
    ...
    def build_activity_metric_controls(
        ...
        activity_scale = (
            self.precision_config["node_budget_scale"] * activity_gate * activity_budget
        ).unsqueeze(-1)
        node_strength = activity_scale * node_alloc
        ...
        metric_local = (
            background_scale * quiet_metric_local
            + self.precision_config["metric_delta_scale"] * precision_delta
        )
    ...
    def transport_velocity_with_pack(
        ...
        mod_v_basis = self.modulate_velocity_basis(sharpened_v_basis, metric_local, metric_band)
...
def sparse_precision_flow_matching_loss(
    model: SparsePrecisionTransportModel,
    ...
    _, _, quiet_metric_local, _, _, _ = super(
        SparsePrecisionTransportModel, model
    ...
    node_entropy = (-(node_alloc * node_alloc.clamp_min(1e-8).log()).sum(dim=-1) * activity_gate).mean()
    kernel_entropy = (
        (-(kernel * kernel.clamp_min(1e-8).log()).sum(dim=-1) * node_alloc).sum(dim=-1) * activity_gate
    )
...
def evaluate_teacher_forced(
    ...
    for history_01, future_01 in val_loader:
        ...
        _loss, metrics = sparse_precision_flow_matching_loss(model, history_01, future_01)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="184c_v0: sparse precision transport")
    ...
    def set_stage(stage: int):
        ...
        if stage == 1:
            ...
            stage_name = "sparse-precision-ctrl"
        else:
            ...
            stage_name = "sparse-precision-joint"
    ...
    print(f"\n{'=' * 72}\n184c_v0: sparse precision transport\n{'=' * 72}")
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for history_01, future_01 in train_loader:
            ...
            loss, metrics = sparse_precision_flow_matching_loss(model, history_01, future_01)
            ...
            torch.save(
                {
                    ...
                    "config": {
                        "type": "sparse_precision_transport_pathwise_residual_law_mean_reverting_covariance_mixture_structured_joint_student_t_184c",
