#!/usr/bin/env python
"""
241c: 241b multi-CRPS with encoder/decoder/flow/prior UNFROZEN (advisor ablation).
...
def sample_future_u_with_grad(
    ...
    241c: grad_through_teacher=True lets CRPS gradient reach encoder+decoder+prior
...
def main() -> None:
    ...
    parser.add_argument("--lr_backbone", type=float, default=3.0e-5,
                        help="241c: LR for UNFROZEN encoder/decoder/flow/prior. Small "
    ...
    # Single-stage fine-tune — same freeze pattern as 241a/183c Stage 2.
    # 241c: UNFREEZE encoder/decoder/flow/prior. This is the ablation's point —
    # test advisor's hypothesis that 241b plateau was DOF-access limited, not
    # interior-capacity limited. All params become trainable with tiered LRs.
            ...
            torch.save(
                {
                    ...
                    "config": {
                        "type": "241c_unfreeze_multi_crps_state_metric_transport",
