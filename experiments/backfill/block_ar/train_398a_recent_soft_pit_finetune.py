#!/usr/bin/env python
"""398a: recent-window soft-PIT calibration fine-tune for the 392a frontier."""
...
def soft_pit_moment_loss(
    samples_iv: torch.Tensor,
    target_iv: torch.Tensor,
    temperature: float,
    var_weight: float,
) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    """Match PIT moments to Uniform(0, 1) using differentiable sampled CDF ranks.

    samples_iv: (B, K, T, D)
    target_iv:  (B, T, D)
    """
    tau = max(float(temperature), 1e-6)
    soft_less_equal = torch.sigmoid((target_iv[:, None] - samples_iv) / tau)
    pit = soft_less_equal.mean(dim=1)
    pit_flat = pit.reshape(-1, pit.shape[-1])
    pit_mean_cell = pit_flat.mean(dim=0)
    pit_var_cell = pit_flat.var(dim=0, unbiased=False)
    mean_loss = (pit_mean_cell - 0.5).square().mean()
    var_target = torch.tensor(1.0 / 12.0, device=pit.device, dtype=pit.dtype)
    var_loss = (pit_var_cell - var_target).square().mean()
    loss = mean_loss + float(var_weight) * var_loss
    metrics = {
        "pit_loss": loss.detach(),
        "pit_mean_loss": mean_loss.detach(),
        "pit_var_loss": var_loss.detach(),
        "pit_mean": pit.mean().detach(),
        "pit_var": pit.var(unbiased=False).detach(),
        "pit_cell_mean_min": pit_mean_cell.min().detach(),
        "pit_cell_mean_max": pit_mean_cell.max().detach(),
        "pit_cell_var_min": pit_var_cell.min().detach(),
        "pit_cell_var_max": pit_var_cell.max().detach(),
    }
    return loss, metrics
...
def combined_loss(
    ...
    pit_loss, pit_metrics = soft_pit_moment_loss(
        samples_iv=sampled_iv,
        target_iv=target_iv,
        temperature=pit_temperature,
        var_weight=pit_var_weight,
    )
    total = float(fm_anchor_weight) * fm_loss + float(pit_weight) * pit_loss
    metrics = {
        "total": total.detach(),
...
def main() -> None:
    ...
    parser.add_argument("--pit_temperature", type=float, default=0.01)
    parser.add_argument("--pit_var_weight", type=float, default=1.0)
    parser.add_argument("--pit_weight", type=float, default=10.0)
        ...
        for hist_batch, fut_batch in loader:
            ...
            with torch.set_grad_enabled(train_mode):
                loss, metrics = combined_loss(
                    ...
                    pit_temperature=args.pit_temperature,
                    pit_var_weight=args.pit_var_weight,
                    pit_weight=args.pit_weight,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "train_pit_loss": train_avg["pit_loss"],
            "train_pit_mean": train_avg["pit_mean"],
            "train_pit_var": train_avg["pit_var"],
            "val_total": val_avg["total"],
            "val_fm_loss": val_avg["fm_loss"],
            "val_pit_loss": val_avg["pit_loss"],
            "val_pit_mean_loss": val_avg["pit_mean_loss"],
            "val_pit_var_loss": val_avg["pit_var_loss"],
            "val_pit_mean": val_avg["pit_mean"],
            "val_pit_var": val_avg["pit_var"],
        ...
        print(
            ...
            f"pit={rec['val_pit_loss']:.5f} mean={rec['val_pit_mean']:.3f} "
            f"var={rec['val_pit_var']:.3f} std={rec['val_sample_iv_std']:.4f}/"
            f"{rec['val_target_iv_std']:.4f} lr={rec['lr']:.2e} time={rec['sec']:.1f}s"
        )
