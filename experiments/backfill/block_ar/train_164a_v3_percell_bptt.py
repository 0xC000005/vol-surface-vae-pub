#!/usr/bin/env python
"""
164a_v3_percell_bptt: Per-Cell CLN + Corrected IS + Partial BPTT (N=3)

Change from IS-fix: gradient flows through N consecutive AR steps before
detaching. This gives multi-step credit assignment — the loss at step t+N
provides gradient back to step t's decoder output. Weather models all use
partial BPTT (FGN 8 steps, AIFS 12, FCN3 8).

Within N-step window: prev_frame keeps gradient, GRU update keeps gradient.
At window boundary: detach prev and GRU state.

Change from 164a_v3: ConditionalNorm produces per-cell scale/bias instead of
broadcast. Each cell gets its own noise modulation from shared z.

Root cause (Codex-verified): shared CLN broadcasts same scale/bias to all 25
cells. Jacobian has rank 7/8 but PC1=99.6%. Per-cell injection breaks the
bottleneck so CRPS can calibrate each cell independently.

Architecture:
  - Same as 164a_v3 except ConditionalNorm
  - ConditionalNorm: Linear(noise_dim, n_cells * d_model) -> (B, C, d_model)
  - Each cell gets different scale/bias from the same z vector
  - Spatial attention still provides coherence across cells

Usage:
    PYTHONPATH=. python experiments/backfill/block_ar/train_164a_v3_percell_cln.py \
        --epochs 80 --batch_size 32 --n_members 8 --noise_dim 32 \
        --lambda_vs 0.5 --lambda_is 0.5 \
        --output_dir models/backfill/afcrps_164a_v3_percell --device cuda
"""
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"164a_v3_percell_bptt: Per-Cell CLN + IS fix + Partial BPTT (N={args.bptt_steps})")
    print(f"{'='*60}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # ── Save best model ──
        config_dict = {
            "type": "ar_spatial_transformer_164a_v3_percell_bptt",
            "encoder": {
