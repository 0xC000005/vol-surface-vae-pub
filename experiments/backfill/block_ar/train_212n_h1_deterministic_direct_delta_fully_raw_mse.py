#!/usr/bin/env python
"""
212n: fully raw deterministic H=1 direct-delta point forecaster.

Keeps:
  - deterministic GRU encoder
  - raw linear delta output
  - no tanh / no clipping
  - raw unscaled delta MSE

Changes relative to 212m:
  - remove delta_scale normalization from history delta features too
"""
...
def build_history_features_raw(history_01: torch.Tensor) -> torch.Tensor:
    """Concatenate normalized levels and raw deltas per history step."""
    ...
    return torch.cat([levels_norm, deltas], dim=-1)


class DeterministicDirectDeltaFullyRawMSEModel(nn.Module):
    def __init__(
        self,
        n_cells: int = 25,
...
def evaluate_point_raw_mse(
    model: DeterministicDirectDeltaFullyRawMSEModel,
    loader: DataLoader,
    q95_threshold: float,
    q99_threshold: float,
) -> dict[str, float]:
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[DeterministicDirectDeltaFullyRawMSEModel, dict[str, Any]]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = payload["config"]
    if cfg["type"] != "minimal_h1_deterministic_direct_delta_212n":
...
def main() -> None:
    parser = argparse.ArgumentParser(description="212n H=1 deterministic direct delta fully raw + raw-delta MSE")
    ...
    print("212n H=1 deterministic direct delta fully raw + raw-delta MSE")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        payload = {
            ...
            "epoch": epoch,
            "config": {
                "type": "minimal_h1_deterministic_direct_delta_212n",
                "n_cells": 25,
                "history_feat_dim": 50,
                "hidden_dim": args.hidden_dim,
                "gru_layers": args.gru_layers,
