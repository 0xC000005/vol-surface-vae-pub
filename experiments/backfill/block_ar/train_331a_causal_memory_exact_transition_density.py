...
from diffusion.block_ar.causal_memory_exact_transition_density import (
    CausalMemoryExactTransitionDensity,
    CausalMemoryExactTransitionDensityConfig,
    save_checkpoint,
)
...
def compute_logit_stats(
    ...
    logits = iv_to_logit(
        torch.cat([flatten_cells(train_hist_01), flatten_cells(train_future_01)], dim=0),
        logit_eps,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="331a causal memory exact transition density")
    ...
    model = CausalMemoryExactTransitionDensity(cfg).to(device)
