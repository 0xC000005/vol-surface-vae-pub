#!/usr/bin/env python
"""
155a: Single-Pass Residual MLP + afCRPS (RC17-H1r)

No ODE. Direct (noise, condition) → residual via MLP. K ensemble members
trained with afCRPS loss on combined output (base_pred + residual).

The hypothesis: changing the loss from CFM (velocity matching) to afCRPS
(proper scoring rule with spread calibration gradient) produces calibrated
ensemble spread from the same base predictions.

Literature: AIFS-CRPS (2412.15832), CRPS-LAM (2510.09484)

Usage:
    PYTHONPATH=. python experiments/backfill/block_ar/train_155a_residual_afcrps.py \
        --base_model models/backfill/flow_153a/final_model.pt \
        --epochs 200 --batch_size 64 --n_members 8 --noise_dim 32 \
        --output_dir models/backfill/flow_155a --device cuda
"""
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"155a: Single-Pass Residual MLP + afCRPS (RC17-H1r)")
    print(f"{'='*60}")
    print(f"  Parameters: {n_params:,}")
    print(f"  K members: {args.n_members}, noise_dim: {args.noise_dim}")
    print(f"  afCRPS alpha: {args.alpha}, lambda_IS: {args.lambda_is}")
    ...
    for epoch in range(1, args.epochs + 1):
        t0 = time.time()
        mlp.train()
        epoch_loss = 0; epoch_mae = 0; epoch_spread = 0; epoch_is = 0; nb = 0
        ...
        if epoch % 40 == 0 or epoch == 1 or epoch == args.epochs:
            ...
            print(f"Ep {epoch:3d}  loss={train_loss:.4f}  val={val_loss:.4f}  "
                  f"mae={train_mae:.4f}  spread={train_spread:.4f}  IS={train_is:.4f}  "
                  f"({elapsed:.1f}s)")
