...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="544a local-shift-normalized empirical-score AR transition flow"
    )
    ...
    parser.add_argument("--local_scale_floor", type=float, default=0.01)
    ...
    cfg = LocalShiftNormalizedEmpiricalScoreTransitionFMConfig(
        ...
        local_scale_floor=args.local_scale_floor,
    )
    model = LocalShiftNormalizedEmpiricalScoreTransitionFlowMatching(cfg).to(device)
    quantiles, quantile_levels = compute_local_shift_quantiles(
        model,
        train_hist,
        train_future,
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "val_local_center_mean": val_avg["local_center_mean"],
            "val_local_scale_mean": val_avg["local_scale_mean"],
            "val_local_scale_min": val_avg["local_scale_min"],
        ...
        print(
            ...
            f"loc_scale={rec['val_local_scale_mean']:.4f} "
