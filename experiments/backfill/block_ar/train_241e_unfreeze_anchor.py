...
def multi_crps_loss(
    ...
    apply_crps: bool,
    use_checkpoint: bool,
    ref_model: StateMetricTransportModel | None = None,
    ...
    if apply_crps:
        ...
        # 241e: KS-preservation anchor. Compute reference pred_mean with frozen 183c
        # (ref_model) and penalise our pred_mean deviation from it. This prevents
        # unfrozen backbone from drifting off the well-calibrated 183c marginals.
        if ref_model is not None and lambda_anchor > 0:
            with torch.no_grad():
                ref_samples = ref_model.sample_batched(history_01, n_samples=K)
                if ref_samples.dim() == 4:
                    ref_samples = ref_samples.view(
                        ref_samples.shape[0], ref_samples.shape[1], ref_samples.shape[2], 5, 5,
                    )
                ref_pred_mean = ref_samples.mean(dim=1)  # (B, T, 5, 5)
            pred_mean = iv_samples.mean(dim=1)  # (B, T, 5, 5) — grad flows
            anchor_loss = (pred_mean - ref_pred_mean).abs().mean()
...
def main() -> None:
    ...
    # 241e: Build a FROZEN reference model from the same warm-start checkpoint for
    # anchor computation. ref_model.sample_batched(history) gives the 183c pred_mean
    # we want to anchor against during CRPS-driven drift.
    ref_model = None
    if args.lambda_anchor > 0 and args.warm_start_path is not None:
        print(f"Building frozen reference 183c model for anchor (λ_anchor={args.lambda_anchor})...")
        ...
        for batch_idx, (history_01, future_01) in enumerate(train_loader):
            ...
            with autocast_ctx:
                loss, metrics = multi_crps_loss(
                    ...
                    ref_model=ref_model, lambda_anchor=args.lambda_anchor,
                )
