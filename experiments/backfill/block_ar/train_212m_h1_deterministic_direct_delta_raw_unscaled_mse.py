#!/usr/bin/env python
"""
212m: 212l ablation with raw unscaled MSE on raw delta.

Keeps:
  - deterministic GRU encoder
  - raw linear delta output
  - no tanh / no clipping
  - feature normalization via build_history_features

Changes only:
  - training/eval loss uses raw-delta MSE, not q99-scaled MSE
"""
...
class DeterministicDirectDeltaRawUnscaledMSEModel(nn.Module):
    def __init__(
        self,
        n_cells: int = 25,
...
def evaluate_point_raw_mse(
    model: DeterministicDirectDeltaRawUnscaledMSEModel,
    loader: DataLoader,
    q95_threshold: float,
    q99_threshold: float,
) -> dict[str, float]:
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[DeterministicDirectDeltaRawUnscaledMSEModel, dict[str, Any]]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = payload["config"]
    if cfg["type"] != "minimal_h1_deterministic_direct_delta_212m":
...
def main() -> None:
    parser = argparse.ArgumentParser(description="212m H=1 deterministic direct delta raw linear + unscaled raw-delta MSE")
    ...
    print("212m H=1 deterministic direct delta raw linear + unscaled raw-delta MSE")
    print(f"  train_windows={train_hist.shape[0]} val_windows={val_hist.shape[0]}")
    print(f"  feature_delta_scale median={float(np.median(delta_scale)):.6f}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        payload = {
            ...
            "epoch": epoch,
            "config": {
                "type": "minimal_h1_deterministic_direct_delta_212m",
                "n_cells": 25,
                "history_feat_dim": 50,
                "hidden_dim": args.hidden_dim,
                "gru_layers": args.gru_layers,
