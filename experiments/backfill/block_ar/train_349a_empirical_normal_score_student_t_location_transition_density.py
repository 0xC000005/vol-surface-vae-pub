...
from diffusion.block_ar.empirical_normal_score_transition_coupling_density import (
    EmpiricalNormalScoreTransitionCouplingConfig,
    EmpiricalNormalScoreTransitionCouplingDensity,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="349a Student-t base location transition coupling likelihood"
    )
    ...
    parser.add_argument("--coupling_hidden", type=int, default=256)
    parser.add_argument("--coupling_dropout", type=float, default=0.1)
    parser.add_argument("--scale_clip", type=float, default=2.0)
    parser.add_argument("--location_hidden", type=int, default=256)
    parser.add_argument("--student_df_init", type=float, default=8.0)
    parser.add_argument("--student_df_min", type=float, default=2.1)
    ...
    cfg = EmpiricalNormalScoreTransitionCouplingConfig(
        ...
        coupling_hidden=args.coupling_hidden,
        coupling_dropout=args.coupling_dropout,
        scale_clip=args.scale_clip,
        use_location=True,
        location_hidden=args.location_hidden,
        base_distribution="student_t",
        student_df_init=args.student_df_init,
        student_df_min=args.student_df_min,
        n_quantiles=args.n_quantiles,
        cdf_eps=args.cdf_eps,
        sample_temperature=args.sample_temperature,
        max_sample_chunk=args.max_sample_chunk,
    )
    model = EmpiricalNormalScoreTransitionCouplingDensity(cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "val_base_std": val_avg["base_std"],
            "val_base_df": val_avg["base_df"],
            "val_log_det_per_dim": val_avg["log_det_per_dim"],
            "val_location_abs": val_avg["location_abs"],
            "val_location_std": val_avg["location_std"],
        ...
        print(
            ...
            f"base_std={rec['val_base_std']:.3f} "
            f"df={rec['val_base_df']:.2f} "
            f"loc_abs={rec['val_location_abs']:.3f} "
            f"logdet/d={rec['val_log_det_per_dim']:.3f} "
