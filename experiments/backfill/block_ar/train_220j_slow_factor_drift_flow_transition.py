#!/usr/bin/env python
"""
220j: slow-factor EMA + drift conditional flow transition model for multi-day rollout.

Goal:
  - keep the successful 212ai local-scale asinh innovation geometry
  - keep the persistent recurrent transition from 220d
  - keep 220i's slow factor memory extracted directly from surfaces
  - let that persistent low-dimensional state move the daily conditional center
    directly through a shared drift head
"""
...
class SlowFactorDriftFlowTransitionModel(ConditionalFlowLocalScaleAsinhNLLModel):
    def __init__(
        self,
        *args,
        ...
        **kwargs,
        ...
        self.factor_alpha = float(factor_alpha)
        ...
        self.surface_to_factor = nn.Sequential(
            nn.Linear(self.n_cells, factor_hidden),
            nn.SiLU(),
            nn.Linear(factor_hidden, self.factor_dim),
        )
        self.factor_to_cond = nn.Linear(self.factor_dim, self.hidden_dim)
        self.factor_to_drift = nn.Linear(self.factor_dim, self.n_cells)
        self._init_factor_params()

    def _init_factor_params(self) -> None:
        nn.init.normal_(self.factor_to_cond.weight, mean=0.0, std=1e-3)
        nn.init.zeros_(self.factor_to_cond.bias)
        nn.init.normal_(self.factor_to_drift.weight, mean=0.0, std=1e-3)
        nn.init.zeros_(self.factor_to_drift.bias)
    ...
    def _encode_surface_factor(self, surface_flat_01: torch.Tensor) -> torch.Tensor:
        return torch.tanh(self.surface_to_factor(surface_flat_01))

    def _history_factor_state(self, history_01: torch.Tensor) -> torch.Tensor:
        frames = history_01.reshape(history_01.shape[0], history_01.shape[1], self.n_cells)
        factor_state = self._encode_surface_factor(frames[:, 0])
        for t in range(1, frames.shape[1]):
            obs = self._encode_surface_factor(frames[:, t])
            factor_state = (1.0 - self.factor_alpha) * factor_state + self.factor_alpha * obs
        return factor_state
    ...
    def encode_history_with_state(
        ...
        factor_state = self._history_factor_state(history_01)
        return h_n, local_scale, prev, factor_state
    ...
    def update_factor_state(
        ...
        obs = self._encode_surface_factor(next_01)
        return (1.0 - self.factor_alpha) * factor_state + self.factor_alpha * obs
    ...
    def _condition_state(
        ...
        return state_stack[-1] + self.factor_to_cond(factor_state)

    def _drift_delta(self, factor_state: torch.Tensor) -> torch.Tensor:
        return self.factor_to_drift(factor_state)
    ...
    def sample_next_from_state(
        ...
        state_top = self._condition_state(state_stack, factor_state)
        ...
        drift_delta = self._drift_delta(factor_state)
        delta = drift_delta.unsqueeze(1) + innovation * local_scale.unsqueeze(1)
    ...
    def log_prob_v_given_state(
        ...
        cond = self._expand_condition(self._condition_state(state_stack, factor_state), 1)
    ...
    def sample_next_iv(
        ...
        state_stack, local_scale, prev, factor_state = self.encode_history_with_state(history_01)
    ...
    def sample_batched(
        ...
        **kwargs,
        ...
        base_state, base_scale, base_prev, base_factor = self.encode_history_with_state(history_01)
        ...
        for start in range(0, n_samples, chunk_size):
            ...
            factor_state = (
                base_factor.unsqueeze(1)
                .expand(batch_size, k, self.factor_dim)
                .reshape(batch_size * k, self.factor_dim)
                .clone()
            )
            ...
            for _ in range(n_steps):
                ...
                factor_state = self.update_factor_state(factor_state, next_flat)
                prev = next_flat
...
def multistep_flow_loss(
    model: SlowFactorDriftFlowTransitionModel,
    history_01: torch.Tensor,
    future_01: torch.Tensor,
    train_samples: int,
    nll_weight: float,
    factor_slow_weight: float,
    ...
    state_stack, local_scale, prev, factor_state = model.encode_history_with_state(history_01)
    ...
    total_factor_drift = torch.tensor(0.0, device=history_01.device)
    pred_mean_path = []
    target_mean_path = []

    for step in range(future_len):
        target_t = future_flat[:, step, :]
        drift_delta = model._drift_delta(factor_state)
        target_delta = target_t - prev
        target_v = torch.asinh((target_delta - drift_delta) / local_scale.clamp_min(model.scale_floor))

        prev_factor_state = factor_state
        ...
        nll_t = -model.log_prob_v_given_state(state_stack, factor_state, target_v).mean() / model.n_cells
        ...
        factor_state = model.update_factor_state(factor_state, target_t)
        total_factor_drift = total_factor_drift + (factor_state - prev_factor_state).pow(2).mean()
    ...
    total_loss = (
        ...
        + 0.05 * path_mean_loss
        + factor_slow_weight * (total_factor_drift / future_len)
    )
    metrics = {
        ...
        "factor_drift": (total_factor_drift / future_len).detach(),
        "factor_norm": factor_state.norm(dim=-1).mean().detach(),
        "drift_abs": drift_delta.abs().mean().detach(),
    }
...
def evaluate_teacher_forced(
    ...
    for history_01, future_01 in val_loader:
        _loss, metrics = multistep_flow_loss(
            ...
            factor_slow_weight=factor_slow_weight,
        )
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[SlowFactorDriftFlowTransitionModel, dict[str, Any]]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = payload["config"]
    if cfg["type"] != "slow_factor_drift_flow_local_scale_asinh_220j":
    ...
    model = SlowFactorDriftFlowTransitionModel(
        ...
        factor_dim=cfg.get("factor_dim", 16),
        factor_alpha=cfg.get("factor_alpha", 0.08),
        factor_hidden=cfg.get("factor_hidden", 64),
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="220j slow-factor drift flow transition model")
    ...
    parser.add_argument("--factor_alpha", type=float, default=0.08)
    parser.add_argument("--factor_hidden", type=int, default=64)
    parser.add_argument("--factor_slow_weight", type=float, default=0.02)
    ...
    print("220j slow-factor drift flow transition model")
    ...
    print(
        ...
        f"factor_dim={args.factor_dim} factor_alpha={args.factor_alpha}"
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for history_01, future_01 in train_loader:
            loss, metrics = multistep_flow_loss(
                ...
                factor_slow_weight=args.factor_slow_weight,
            )
            optimizer.zero_grad()
            loss.backward()
        ...
        payload = {
            ...
            "epoch": epoch,
            "config": {
                "type": "slow_factor_drift_flow_local_scale_asinh_220j",
                "n_cells": cfg["n_cells"],
                "history_feat_dim": cfg["history_feat_dim"],
                "hidden_dim": cfg["hidden_dim"],
                "gru_layers": cfg["gru_layers"],
