#!/usr/bin/env python
"""
212z: H=1 minimal direct stochastic delta model with local-scale asinh
innovation plus a small per-cell CRPS auxiliary loss.

Same local-scale setup as 212y, but training now adds a marginal calibration
term on top of the joint energy score. The decoder models the transformed
innovation:
...
This tests whether the remaining 212y bottleneck is local per-cell marginal
...
class MinimalDirectStochasticDeltaLocalScaleAsinhCRPSModel(nn.Module):
    def __init__(
        self,
        n_cells: int = 25,
    ...
    def training_loss(
        ...
        crps = sample_crps(v_samples, target_v)
        loss = energy + self.lambda_crps * crps
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[MinimalDirectStochasticDeltaLocalScaleAsinhCRPSModel, dict[str, Any]]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = payload["config"]
    if cfg["type"] != "minimal_h1_direct_stochastic_delta_212z_local_scale_asinh_crps":
        raise ValueError(f"Unexpected model type: {cfg['type']}")
    model = MinimalDirectStochasticDeltaLocalScaleAsinhCRPSModel(
        n_cells=cfg["n_cells"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="212z H=1 minimal direct stochastic delta local-scale asinh + CRPS")
    ...
    print("212z H=1 minimal direct stochastic delta local-scale asinh + CRPS")
    ...
    for epoch in range(1, args.epochs + 1):
        t0 = time.time()
        model.train()
        running = {"loss": 0.0, "energy": 0.0, "crps": 0.0, "mae": 0.0, "delta_std": 0.0, "v_std": 0.0}
        count = 0
        ...
        payload = {
            ...
            "epoch": epoch,
            "config": {
                "type": "minimal_h1_direct_stochastic_delta_212z_local_scale_asinh_crps",
                "n_cells": 25,
                "history_feat_dim": history_feat_dim,
                "hidden_dim": args.hidden_dim,
                "gru_layers": args.gru_layers,
