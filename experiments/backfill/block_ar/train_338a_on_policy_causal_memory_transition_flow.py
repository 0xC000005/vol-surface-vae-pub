...
def one_step_fm_loss(
    ...
    target_next_logit: torch.Tensor,
) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    x1 = target_next_logit - current_logit
    x0 = torch.randn_like(x1)
    bsz, n_cells = x1.shape
    t = torch.rand(bsz, device=x1.device, dtype=x1.dtype)
    x_t = (1.0 - t[:, None]) * x0 + t[:, None] * x1
    target_velocity = x1 - x0
    pred_velocity = model.predict_velocity(x_t, current_logit, memory_state, t)
...
def generate_prefix_logits(
    ...
    prefix = history_logits.clone()
    if n_generated_steps <= 0:
        return prefix
    dt = 1.0 / float(flow_steps)
    for _step in range(n_generated_steps):
        memory_state = model._encode_prefix_logits(prefix)[:, -1]
        current_logit = prefix[:, -1]
        x = temperature * torch.randn_like(current_logit)
        for flow_step in range(flow_steps):
            t = torch.full(
                (history_logits.shape[0],),
                (flow_step + 0.5) * dt,
                device=history_logits.device,
                dtype=history_logits.dtype,
            )
            x = x + dt * model.predict_velocity(x, current_logit, memory_state, t)
        prefix = torch.cat([prefix, (current_logit + x)[:, None, :]], dim=1)
    return prefix


def main() -> None:
    parser = argparse.ArgumentParser(
        description="338a on-policy fine-tune for 330c AR-FM"
    )
    parser.add_argument(
        "--init_checkpoint",
        type=str,
        default="models/backfill/330c_v0_s42/best_model.pt",
    )
    ...
    def run_epoch(loader: DataLoader, train_mode: bool) -> dict[str, float]:
        ...
        for hist_01, fut_01 in loader:
            ...
            with torch.set_grad_enabled(train_mode):
                ...
                history_logits = model.history_logits(hist_norm)
                target_future_logits = model.target_future_logits(fut_norm)
                horizon_idx = int(
                    torch.randint(0, cfg.future_len, (1,), device=device).item()
                )
                generated_prefix = generate_prefix_logits(
                    model,
                    history_logits,
                ...
                memory_state = model._encode_prefix_logits(generated_prefix.detach())[
                    :, -1
                ]
                current_logit = generated_prefix[:, -1].detach()
                target_next = target_future_logits[:, horizon_idx]
