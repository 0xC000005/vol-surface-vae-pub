#!/usr/bin/env python
"""
212w: H=1 minimal direct stochastic delta with multivariate Student-t latent.

Change from 212b:
  - replace Gaussian latent noise with shared-scale multivariate Student-t latent

Keep:
  - same GRU encoder
  - same direct decoder MLP([h, z])
  - same bounded output tanh(raw) * delta_scale
  - same energy score
  - same K=128 default training population
"""
...
class MinimalDirectStochasticDeltaStudentTLatentModel(nn.Module):
    def __init__(
        self,
        n_cells: int = 25,
        ...
        super().__init__()
        if student_df < 1:
            raise ValueError("student_df must be >= 1")
        self.n_cells = n_cells
        self.latent_dim = latent_dim
        self.student_df = int(student_df)
        self.noise_dim = latent_dim + self.student_df
        ...
        self.decoder = nn.Sequential(
            nn.Linear(hidden_dim + latent_dim, decoder_hidden),
            nn.SiLU(),
    ...
    def _student_t_latent(self, noise: torch.Tensor) -> torch.Tensor:
        eps = noise[..., : self.latent_dim]
        chi = noise[..., self.latent_dim :]
        g = chi.pow(2).sum(dim=-1, keepdim=True).clamp_min(1e-8)
        scale = torch.sqrt(g / float(self.student_df))
        return eps / scale
    ...
    def sample_delta(
        ...
        z = self._student_t_latent(noise)
        decoder_in = torch.cat([state_rep, z], dim=-1).reshape(batch * n_samples, self.hidden_dim + self.latent_dim)
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[MinimalDirectStochasticDeltaStudentTLatentModel, dict[str, Any]]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = payload["config"]
    if cfg["type"] != "minimal_h1_direct_stochastic_delta_212w_student_t_latent":
        raise ValueError(f"Unexpected model type: {cfg['type']}")
    model = MinimalDirectStochasticDeltaStudentTLatentModel(
        n_cells=cfg["n_cells"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="212w H=1 minimal direct stochastic delta with Student-t latent")
    ...
    parser.add_argument("--student_df", type=int, default=5)
    ...
    print("212w H=1 minimal direct stochastic delta with Student-t latent")
    ...
    print(f"  latent_dim={args.latent_dim} student_df={args.student_df}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        payload = {
            ...
            "epoch": epoch,
            "config": {
                "type": "minimal_h1_direct_stochastic_delta_212w_student_t_latent",
                "n_cells": 25,
                "history_feat_dim": 50,
                "hidden_dim": args.hidden_dim,
                "gru_layers": args.gru_layers,
                ...
                "latent_dim": args.latent_dim,
                "student_df": args.student_df,
                "noise_dim": args.latent_dim + args.student_df,
                "decoder_hidden": args.decoder_hidden,
