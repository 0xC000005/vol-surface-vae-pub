#!/usr/bin/env python
"""
220i: slow-factor EMA conditional flow transition model for multi-day rollout.

Goal:
  - keep the successful 212ai local-scale asinh innovation geometry
  - keep the persistent recurrent transition from 220d
  - replace 220g's entangled regime hidden state with a slow factor memory
    extracted directly from surfaces and updated by a small EMA
"""
...
class SlowFactorEMAFlowTransitionModel(ConditionalFlowLocalScaleAsinhNLLModel):
    def __init__(
        self,
        *args,
...
def multistep_flow_loss(
    model: SlowFactorEMAFlowTransitionModel,
    history_01: torch.Tensor,
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[SlowFactorEMAFlowTransitionModel, dict[str, Any]]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = payload["config"]
    if cfg["type"] != "slow_factor_ema_flow_local_scale_asinh_220i":
...
def main() -> None:
    parser = argparse.ArgumentParser(description="220i slow-factor EMA flow transition model")
    ...
    print("220i slow-factor EMA flow transition model")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        payload = {
            ...
            "epoch": epoch,
            "config": {
                "type": "slow_factor_ema_flow_local_scale_asinh_220i",
                "n_cells": cfg["n_cells"],
                "history_feat_dim": cfg["history_feat_dim"],
                "hidden_dim": cfg["hidden_dim"],
                "gru_layers": cfg["gru_layers"],
