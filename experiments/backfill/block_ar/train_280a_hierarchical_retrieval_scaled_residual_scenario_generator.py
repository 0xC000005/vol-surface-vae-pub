...
from diffusion.block_ar.hierarchical_retrieval_scaled_residual_scenario_generator import (
    HierarchicalScaledResidualRetrievalScenarioGenerator,
)
...
def epoch_pass(
    ...
    for history_norm, future_01 in loader:
        history_norm = history_norm.to(next(model.parameters()).device)
        future_01 = future_01.to(next(model.parameters()).device)
        query_z, target_scale = model.training_targets(history_norm, future_01)
        pred_scale = model.predict_scale_from_query(query_z)
        loss = (torch.log(pred_scale) - torch.log(target_scale)).pow(2).mean()
        mae = (pred_scale - target_scale).abs().mean()
        if training:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="280a-v0 learned residual scaling over fixed Stage A center path")
