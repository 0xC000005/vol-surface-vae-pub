#!/usr/bin/env python
"""487a: recent-window marginal-CRPS fine-tune from the 392a frontier."""
...
def combined_loss(
    ...
    crps_loss, target_dist, pair_dist = marginal_crps_score(
        sampled_scores,
        target_scores,
    )
    total = float(fm_anchor_weight) * fm_loss + float(crps_weight) * crps_loss
    metrics = {
        "total": total.detach(),
        "fm_loss": fm_loss.detach(),
        "crps": crps_loss.detach(),
        "crps_target_dist": target_dist.detach(),
        "crps_pair_dist": pair_dist.detach(),
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "train_crps": train_avg["crps"],
            "val_total": val_avg["total"],
            "val_fm_loss": val_avg["fm_loss"],
            "val_crps": val_avg["crps"],
            "val_crps_target_dist": val_avg["crps_target_dist"],
            "val_crps_pair_dist": val_avg["crps_pair_dist"],
        ...
        print(
            ...
            f"crps={rec['val_crps']:.5f} target={rec['val_crps_target_dist']:.3f} "
            f"pair={rec['val_crps_pair_dist']:.3f} "
