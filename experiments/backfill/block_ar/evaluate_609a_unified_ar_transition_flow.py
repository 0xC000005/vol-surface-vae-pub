#!/usr/bin/env python
"""609a: score generic unified AR transition checkpoints on the IV 11-suite."""
...
def summarize_results(results: dict[str, Any], alignment: dict[str, float]) -> list[str]:
    ...
    return [
        "- source: `609a generic empirical-score AR transition flow`",
...
def main() -> None:
    ...
    if alignment["history_max_abs_error"] > 1e-6 or alignment["future_max_abs_error"] > 1e-6:
        raise RuntimeError(f"609a/full-suite alignment failed: {alignment}")
    ...
    results["config"] = {
        "mode": "609a_unified_ar_transition_flow",
    ...
    write_markdown_summary(
        out_md,
        "609a Unified AR Transition Flow Official IV 11-Suite",
