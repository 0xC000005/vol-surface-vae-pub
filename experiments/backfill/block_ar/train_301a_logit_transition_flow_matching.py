...
from diffusion.block_ar.logit_transition_flow_matching import (
    LogitTransitionFMConfig,
    LogitTransitionFlowMatching,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="301a-v0 support-valid rolling logit-transition flow")
    ...
    model = LogitTransitionFlowMatching(cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"transition_abs={rec['val_transition_abs']:.3f} "
            f"lr={rec['lr']:.2e} "
            f"time={rec['sec']:.1f}s"
        )
