...
def build_windows(
    ...
    train_history_fast, _ = build_local_history_fast_slow(train_hist)
    val_history_fast, _ = build_local_history_fast_slow(val_hist)
...
def run_epoch(
    model: DeterministicLearnedRetrievalLocalHistoryTwoTimescaleMixedQueryDeltaBackbone,
    loader: DataLoader,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="287e-v0 learned retrieval with mixed fast/slow query coordinates")
    ...
    model = DeterministicLearnedRetrievalLocalHistoryTwoTimescaleMixedQueryDeltaBackbone(cfg).to(device)
