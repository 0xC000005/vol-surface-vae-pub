#!/usr/bin/env python
"""
212f: H=1 asymmetric modulated direct stochastic delta model with interval supervision.
...
def pinball_loss(pred_q: torch.Tensor, target: torch.Tensor, tau: float) -> torch.Tensor:
    err = target - pred_q
...
def training_loss(
    ...
    q05_loss = pinball_loss(q05, target_01, 0.05)
    q95_loss = pinball_loss(q95, target_01, 0.95)
    pinball = 0.5 * (q05_loss + q95_loss)
    loss = es + pinball_weight * pinball

    mean_delta = sample_delta.mean(dim=1)
    metrics = {
        "energy": es.detach(),
        "pinball": pinball.detach(),
        "q05_loss": q05_loss.detach(),
        "q95_loss": q95_loss.detach(),
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[AsymmetricModulatedDirectDeltaModel, dict[str, Any]]:
    ...
    cfg = payload["config"]
    if cfg["type"] != "minimal_h1_asymmetric_modulated_direct_delta_212f":
...
def main() -> None:
    parser = argparse.ArgumentParser(description="212f H=1 asymmetric modulated direct delta with q05/q95 pinball supervision")
    ...
    parser.add_argument("--pinball_weight", type=float, default=0.2)
    ...
    print("212f H=1 asymmetric modulated direct delta + interval supervision")
    ...
    print(f"  pinball_weight={args.pinball_weight:.3f} train_samples={args.train_samples}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for history_01, target_01 in train_loader:
            ...
            running["pinball"] += float(metrics["pinball"].item()) * batch
            running["q05_loss"] += float(metrics["q05_loss"].item()) * batch
            running["q95_loss"] += float(metrics["q95_loss"].item()) * batch
        ...
        payload = {
            ...
            "epoch": epoch,
            "config": {
                "type": "minimal_h1_asymmetric_modulated_direct_delta_212f",
                "n_cells": 25,
                "history_feat_dim": 50,
                "hidden_dim": args.hidden_dim,
                "gru_layers": args.gru_layers,
        ...
        print(
            ...
            f"pin={train_metrics['train_pinball']:.4f}  "
