#!/usr/bin/env python
"""625a: train a generic AR RealNVP transition law by exact conditional likelihood."""
...
def main() -> None:
    ...
    parser.add_argument("--scale_clip", type=float, default=1.5)
    ...
    cfg = GenericRealNVPTransitionConfig(
        ...
        coupling_layers=int(args.coupling_layers),
        coupling_hidden=int(args.coupling_hidden),
        coupling_depth=int(args.coupling_depth),
        model_dropout=float(args.model_dropout),
        scale_clip=float(args.scale_clip),
        sample_temperature=float(args.sample_temperature),
    )
    model = GenericRealNVPTransitionLaw(cfg).to(device)
