#!/usr/bin/env python
"""
185b_v0: Explicit event-residual decomposition on top of the 183c pathwise residual-law backbone.

Principle:
  - keep the 183c mean/covariance/pathwise residual backbone as the quiet branch
  - keep the graph/group-aware latent activity process from 185a
  - make the event branch learn an explicit residual event path in basis space
  - supervise the event branch against a sparse teacher event decomposition, so it
    must carry real residual magnitude instead of only nudging the quiet path
  - keep the mechanism reusable beyond IV surfaces by making geometry/group structure explicit
"""
...
from experiments.backfill.block_ar.train_182a_pathwise_residual_law import (
    ConditionalPathFlowTransformer,
)
...
class GraphGroupEventResidualDecompositionModel(StateMetricTransportModel):
    def __init__(
        self,
        ...
        self.event_amplitude_head = ScaledSigmoidHead(
            input_dim=4,
            hidden_dim=self.event_config["event_hidden_dim"],
            low=0.0,
            high=self.event_config["event_amp_max"],
            init_value=self.event_config["init_event_amp"],
        )
        self.event_band_head = CenteredTanhHead(
            input_dim=block_state_dim,
            hidden_dim=self.event_config["event_hidden_dim"],
            output_dim=3,
        ...
        self.event_scale_logit = nn.Parameter(
            torch.tensor(_scaled_logit(self.event_config["init_event_scale"], 0.0, self.event_config["event_scale_max"]))
        )
    ...
    def maybe_load_warm_start(self, ckpt_path: str | None, device: str | torch.device) -> None:
        ...
        if isinstance(payload.get("config"), dict):
            ckpt_type = str(payload["config"].get("type", ""))
        load_full_event_stack = ckpt_type in (
        ...
        skip_prefixes = () if load_full_event_stack else (
            "activity_process.",
            "activity_allocator.",
    ...
    def build_block_event_targets(self, target_local_gap: torch.Tensor) -> torch.Tensor:
        teacher = self.build_block_teacher_inputs(target_local_gap)
        concentration = teacher[..., 4]
        abs_mean = teacher[..., 2]
        abs_max = teacher[..., 3]
        conc_target = torch.sigmoid(
            (concentration - self.event_config["event_concentration_center"])
            / self.event_config["event_concentration_scale"]
        )
        amp_target = torch.sigmoid(
            (abs_max - self.event_config["event_absmax_center"])
            / self.event_config["event_absmax_scale"]
        )
        mean_target = torch.sigmoid(
            (abs_mean - self.event_config["event_absmean_center"])
            / self.event_config["event_absmean_scale"]
        )
    ...
    def build_teacher_event_components(
        ...
        full_mask = torch.ones_like(target_event_local, dtype=torch.bool)
        target_pos = topk_soft_allocation_masked(
            F.relu(target_event_local),
            full_mask,
            self.activity_config["teacher_topk"],
            self.activity_config["teacher_temperature"],
        )
        ...
        target_support = (target_pos + target_neg > 1e-8).to(target_basis.dtype)
        event_target = self.build_block_event_targets(target_event_local)
        expanded_target = self._expand_block_values(event_target).unsqueeze(-1)
        teacher_mask = target_support * expanded_target
        ...
        return target_pos, target_neg, target_support, teacher_mask, target_event_basis
    ...
    def build_event_path_controls(
        ...
        pos_scores, neg_scores = self.activity_allocator(node_features)
        ...
        signed_support = router_map * (alloc_pos - alloc_neg)
        event_amp = self.event_amplitude_head(node_features).squeeze(-1)
        seed_local = signed_support * event_amp * (activity_gate * activity_budget).unsqueeze(-1)
        neighbor_local = torch.einsum(
            "ij,btj->bti",
            self.normalized_adjacency.to(device=seed_local.device, dtype=seed_local.dtype),
            seed_local,
        )
        event_local = (
            (1.0 - self.event_config["graph_mix"]) * seed_local
            + self.event_config["graph_mix"] * neighbor_local
        )
        event_local = event_local - event_local.mean(dim=(1, 2), keepdim=True)
        event_local = event_local.clamp(
            min=-self.event_config["event_local_clip"],
            max=self.event_config["event_local_clip"],
        )
        block_inputs = self.build_block_state_inputs(state_local, state_band, quiet_metric_local)
        block_event_band = self.event_band_head(block_inputs) * block_gate.unsqueeze(-1)
        block_weight = block_gate.unsqueeze(-1).clamp_min(1e-3)
        event_band = (block_event_band * block_weight).sum(dim=1) / block_weight.sum(dim=1).clamp_min(1e-3)
    ...
    def transport_velocity_with_pack(
        ...
        (
            ...
            signed_support,
            block_event_band,
        ) = self.build_event_path_controls(
            z_t_flat,
            t,
            path_context,
        ...
        flat_support = signed_support.reshape(signed_support.shape[0], -1)
        metrics = {
            ...
            "event_support_top1_mean": flat_support.abs().max(dim=1).values.mean(),
...
def event_residual_decomposition_flow_matching_loss(
    model: GraphGroupEventResidualDecompositionModel,
    ...
    _, _, quiet_metric_local, quiet_metric_band, _, _ = super(
        GraphGroupEventResidualDecompositionModel, model
    ...
    (
        ...
        target_teacher_mask,
        target_event_basis,
    ) = model.build_teacher_event_components(target_basis, target_event_local)
    ...
    (
        ...
        _signed_support,
        _block_event_band,
    ) = model.build_event_path_controls(z_t, t, path_context, activity_pack=activity_pack, use_posterior=True)
    ...
    quiet_local_loss = F.smooth_l1_loss(quiet_metric_local, target_local_log)
    quiet_band_loss = F.smooth_l1_loss(quiet_metric_band, target_band_log)
    event_local_loss = F.smooth_l1_loss(event_local, target_event_local)
    event_band_loss = F.smooth_l1_loss(event_band, target_event_band)

    surf = event_local.view(event_local.shape[0], event_local.shape[1], 5, 5)
    event_time_tv = (event_local[:, 1:] - event_local[:, :-1]).abs().mean()
    event_row_tv = (surf[:, :, 1:] - surf[:, :, :-1]).abs().mean()
    event_col_tv = (surf[:, :, :, 1:] - surf[:, :, :, :-1]).abs().mean()
    event_tv_reg = event_time_tv + event_row_tv + event_col_tv

    event_target = model.build_block_event_targets(target_event_local)
    support_weight = model._expand_block_values(event_target).unsqueeze(-1)
    ...
    posterior_event_bce = F.binary_cross_entropy(post_gate, event_target)
    prior_event_bce = F.binary_cross_entropy(prior_gate, event_target)
    event_amp_reg = event_amp.mean()
    ...
    total = (
        ...
        + model.event_config["event_tv_reg_weight"] * event_tv_reg
        ...
        + model.event_config["posterior_event_bce_weight"] * posterior_event_bce
        + model.event_config["prior_event_bce_weight"] * prior_event_bce
        + model.event_config["event_amp_reg_weight"] * event_amp_reg
    )
    ...
    metrics = {
        ...
        "event_tv_reg": event_tv_reg,
        "event_budget_reg": budget_reg,
        "event_support_loss": support_loss,
        "event_activity_gate_kl": activity_gate_kl,
        "event_activity_budget_consistency": activity_budget_consistency,
        "event_activity_group_kl": activity_group_kl,
        "posterior_event_bce": posterior_event_bce,
        "prior_event_bce": prior_event_bce,
        "target_event_rate": event_target.mean(),
        "event_amp_reg": event_amp_reg,
        ...
        "target_event_teacher_mask_mean": target_teacher_mask.mean(),
...
def evaluate_teacher_forced(
    ...
    keys = [
        ...
        "event_budget_reg",
        "event_support_loss",
        "event_activity_gate_kl",
        "event_activity_budget_consistency",
        "event_activity_group_kl",
        "posterior_event_bce",
        "prior_event_bce",
        "target_event_rate",
        "event_amp_reg",
        ...
        "target_event_teacher_mask_mean",
    ...
    for history_01, future_01 in val_loader:
        ...
        _loss, metrics = event_residual_decomposition_flow_matching_loss(model, history_01, future_01)
    ...
    out["val_total_loss"] = out["val_event_total_loss"]
    return out


def main() -> None:
    parser = argparse.ArgumentParser(description="185b_v0: explicit event-residual decomposition")
    ...
    parser.add_argument("--event_amp_max", type=float, default=0.90)
    parser.add_argument("--init_event_amp", type=float, default=0.20)
    parser.add_argument("--event_band_max", type=float, default=0.30)
    parser.add_argument("--event_local_clip", type=float, default=0.65)
    parser.add_argument("--event_band_clip", type=float, default=0.30)
    parser.add_argument("--event_scale_max", type=float, default=1.25)
    parser.add_argument("--init_event_scale", type=float, default=0.50)
    parser.add_argument("--graph_mix", type=float, default=0.35)
    parser.add_argument("--quiet_local_loss_weight", type=float, default=0.30)
    parser.add_argument("--quiet_band_loss_weight", type=float, default=0.20)
    parser.add_argument("--event_local_loss_weight", type=float, default=0.65)
    parser.add_argument("--event_band_loss_weight", type=float, default=0.35)
    parser.add_argument("--event_tv_reg_weight", type=float, default=0.015)
    ...
    parser.add_argument("--event_amp_reg_weight", type=float, default=0.0)
    ...
    event_config = dict(
        ...
        init_event_amp=args.init_event_amp,
    ...
    def set_stage(stage: int):
        ...
        if stage == 1:
            model.path_context_adapter.requires_grad_(False)
            optimizer = torch.optim.AdamW(event_params, lr=args.lr_ctrl_stage1, weight_decay=args.weight_decay)
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max(args.epochs_stage1, 1))
            stage_name = "event-residual-ctrl"
        else:
            model.path_context_adapter.requires_grad_(True)
            params = [
                {"params": event_params, "lr": args.lr_ctrl_stage2},
                {
            ...
            stage_name = "event-residual-joint"
    ...
    print(f"\n{'=' * 72}\n185b_v0: explicit event-residual decomposition\n{'=' * 72}")
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for history_01, future_01 in train_loader:
            ...
            loss, metrics = event_residual_decomposition_flow_matching_loss(model, history_01, future_01)
            ...
            torch.save(
                {
                    ...
                    "config": {
                        "type": "graph_group_event_residual_decomposition_pathwise_residual_law_mean_reverting_covariance_mixture_structured_joint_student_t_185b",
        ...
        print(
            ...
            f"eScale={val_metrics['val_event_scale']:.3f}  "
            ...
            f"gTop={val_metrics['val_activity_router_top1_mean']:.3f}  eTop={val_metrics['val_event_support_top1_mean']:.3f}  "
