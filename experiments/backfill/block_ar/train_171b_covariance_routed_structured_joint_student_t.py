#!/usr/bin/env python
"""
171b: Covariance-routed structured joint Student-t
...
Change:
  - keep a shared conditional mean path
  - route only the covariance through a K-component mixture
  - train with exact covariance-mixture NLL
"""
...
class CovarianceRoutedStructuredJointStudentTDecoder(nn.Module):
    """Shared mean head plus mixture-routed structured covariance heads."""
    ...
    def forward(
        ...
        time_factor = self.time_factor_head(time_summary).view(
            batch, n_frames, n_components, self.time_rank
        ).permute(0, 2, 1, 3)
        time_diag = F.softplus(self.time_diag_head(time_summary).permute(0, 2, 1)) + self.diag_floor
        cell_factor = self.cell_factor_head(cell_summary).view(
            batch, n_cells, n_components, self.cell_rank
        ).permute(0, 2, 1, 3)
...
class CovarianceRoutedStructuredJointStudentTModel(nn.Module):
    """GRU history encoder + shared-mean covariance-routed joint Student-t future blocks."""
    ...
    def __init__(
        ...
        self.decoder = CovarianceRoutedStructuredJointStudentTDecoder(**decoder_config)
    ...
    def component_log_probs(
        ...
        batch, n_components, n_frames, n_cells = time_factor.shape[0], time_factor.shape[1], mu.shape[1], mu.shape[2]
        ...
        pi = target_u.new_tensor(torch.pi)
        log_norm = (
    ...
    def sample_future_u(self, history_01: torch.Tensor, n_samples: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for k in range(n_components):
            ...
            all_samples.append(mu.unsqueeze(1) + noise * t_scale)
        ...
        stacked_by_sample = stacked.permute(0, 2, 1, 3, 4)
        gather_idx = comp_idx.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand(
            batch,
            n_samples,
            1,
            n_frames,
...
def joint_nll_loss(
    model: CovarianceRoutedStructuredJointStudentTModel,
...
def main():
    parser = argparse.ArgumentParser(description="171b: covariance-routed structured joint Student-t")
    ...
    print(f"\n{'=' * 64}")
    print("171b: Covariance-routed structured joint future Student-t")
    print(f"{'=' * 64}")
    ...
    print("  Objective: exact covariance-mixture joint Student-t NLL")
    print("  Mean: shared across components")
    print("  Covariance: routed scale^2 * (Sigma_time kron Sigma_cell)")
            ...
            torch.save(
                {
                    ...
                    "config": {
                        "type": "covariance_routed_structured_joint_student_t_171b",
