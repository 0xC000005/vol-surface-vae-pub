#!/usr/bin/env python
"""
164a_v2: AR + No-LN Spatial Transformer + VS + Direct Output — NO BOUNDARY (RC20)

Single change from 164a: remove reflecting boundary. No scaling, no clamping.
The model must learn output magnitude, mean-reversion, and bounds from CRPS alone.

164a root cause: reflecting boundary created a degenerate fixed point. The model
learned a -0.34 systematic bias exploiting boundary reflection as a no-op.
99.4% of outputs hit the boundary. Removing it forces honest gradient signal.

Architecture (identical to 164a except no boundary):
  - Encoder: GRU + attention pool, RANDOM INIT, E2E
  - Decoder: Spatial transformer (25 cells/frame), no-LN, tanh, LayerScale 0.1
  - Generation: AR frame-by-frame, frame_t = prev_frame + tanh(delta)
  - Loss: afCRPS + VS (lambda=0.5), per-frame at D=25
  - NO reflecting boundary, NO clamp, NO output scaling

Usage:
    PYTHONPATH=. python experiments/backfill/block_ar/train_164a_v2_no_boundary.py \
        --epochs 80 --batch_size 32 --n_members 8 --noise_dim 32 \
        --lambda_vs 0.5 --lambda_is 0.5 \
        --output_dir models/backfill/afcrps_164a_v2 --device cuda
"""
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"164a_v2: AR + No-LN Spatial Transformer + VS — NO BOUNDARY (RC20)")
    print(f"{'='*60}")
    ...
    print(f"  Boundary: NONE (model must learn bounds from CRPS)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for hist, gt_frames, last_frame in train_loader:
            ...
            # Encode history (E2E, with gradients)
            cond = model.encode(normalize_iv(hist))  # (B, 128)
            ...
            # AR loop: per-step backward to limit peak activation memory
            # Encoder gradients accumulate across all 30 steps via cond_K
            optimizer.zero_grad()
            ...
            for t in range(T):
                ...
                # Per-step backward: release decoder activations, retain encoder graph
        ...
        with torch.no_grad():
            for v_hist, v_gt, v_last in val_loader:
                ...
                cond = model.encode(normalize_iv(v_hist))
        ...
        # ── Save best model ──
        config_dict = {
            "type": "ar_spatial_transformer_164a_v2",
            "encoder": {
