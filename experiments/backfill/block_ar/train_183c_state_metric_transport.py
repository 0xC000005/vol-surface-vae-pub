#!/usr/bin/env python
"""
183c_v0: State-dependent transport metric on top of the 183a/183b pathwise residual-law backbone.

Principle:
  - keep the 183b mean/covariance/pathwise residual backbone
  - keep state-conditioned local/band signal extraction
  - replace tiny post-hoc strength scalars with normalized state-dependent
    transport metrics that reallocate local radial energy more strongly
"""
...
class StateMetricTransportModel(IntegratedWidthTailPathwiseResidualLawModel):
    ...
    def maybe_load_warm_start(self, ckpt_path: str | None, device: str | torch.device) -> None:
        ...
        for key, value in state.items():
            if key.startswith("local_state_gate.") or key.startswith("band_state_gate."):
                skipped.append(key)
                continue
...
def state_metric_flow_matching_loss(
...
def evaluate_teacher_forced(
    ...
    for history_01, future_01 in val_loader:
        ...
        _loss, metrics = state_metric_flow_matching_loss(model, history_01, future_01)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="183c_v0: state-dependent transport metric")
    ...
    print(f"\n{'=' * 72}\n183c_v0: state-dependent transport metric\n{'=' * 72}")
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for history_01, future_01 in train_loader:
            ...
            loss, metrics = state_metric_flow_matching_loss(model, history_01, future_01)
            ...
            torch.save(
                {
                    ...
                    "config": {
                        "type": "state_metric_transport_pathwise_residual_law_mean_reverting_covariance_mixture_structured_joint_student_t_183c",
        ...
        print(
            ...
            f"lGate={val_metrics['val_local_state_gate_mean']:.3f}  bGate={val_metrics['val_band_state_gate_mean']:.3f}  "
