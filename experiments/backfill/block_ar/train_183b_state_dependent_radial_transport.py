#!/usr/bin/env python
"""
183b_v0: State-dependent radial transport on top of the 183a integrated pathwise residual-law backbone.

Principle:
  - keep the 183a mean/covariance/pathwise residual backbone
  - keep integrated local/band controls inside the transport
  - make those controls depend on the current latent residual state z_t
    and transport time t, not only on a static history-conditioned context
"""
...
class StateDependentRadialTransportModel(IntegratedWidthTailPathwiseResidualLawModel):
    def __init__(
        self,
    ...
    def build_state_dependent_controls(
        self,
        z_t_flat: torch.Tensor,
        t: torch.Tensor,
        ...
        raw_band = raw_band - (raw_band * band_weights.unsqueeze(0)).sum(dim=1, keepdim=True)
        ...
        return raw_local, raw_band, eff_local, eff_band, local_gate, band_gate
    ...
    def transport_velocity(
        ...
        raw_local, raw_band, eff_local, eff_band, local_gate, band_gate = self.build_state_dependent_controls(
...
def state_dependent_flow_matching_loss(
    ...
    raw_local, raw_band, _eff_local, _eff_band, _local_gate, _band_gate = model.build_state_dependent_controls(
        z_t, t, path_context
    )
...
def evaluate_teacher_forced(
    ...
    for history_01, future_01 in val_loader:
        ...
        _loss, metrics = state_dependent_flow_matching_loss(model, history_01, future_01)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="183b_v0: state-dependent radial transport")
    ...
    def set_stage(stage: int):
        if stage == 1:
            ...
            params = (
                ...
                + [model.local_strength_logit, model.band_strength_logit]
            )
    ...
    print(f"\n{'=' * 72}\n183b_v0: state-dependent radial transport\n{'=' * 72}")
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for history_01, future_01 in train_loader:
            ...
            loss, metrics = state_dependent_flow_matching_loss(model, history_01, future_01)
            ...
            torch.save(
                {
                    ...
                    "config": {
                        "type": "state_dependent_radial_transport_pathwise_residual_law_mean_reverting_covariance_mixture_structured_joint_student_t_183b",
