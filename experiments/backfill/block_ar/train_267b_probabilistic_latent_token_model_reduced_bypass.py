...
from diffusion.block_ar.probabilistic_latent_token_model_reduced_bypass import (
    ProbabilisticLatentTokenBypassConfig,
    ProbabilisticLatentTokenReducedBypass,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="267b-v0 probabilistic latent-token model with reduced decoder bypass")
    ...
    cfg = ProbabilisticLatentTokenBypassConfig(
        history_len=args.history_len,
        future_len=args.future_len,
        n_cells=d,
    ...
    model = ProbabilisticLatentTokenReducedBypass(cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "train_kl": train_avg["kl_loss"],
            "train_prior_std": train_avg["prior_std"],
            "train_post_std": train_avg["post_std"],
        ...
        print(
            ...
            f"kl={rec['val_kl']:.5f} "
            f"pstd={rec['val_prior_std']:.3f} "
            f"qstd={rec['val_post_std']:.3f} "
            f"lr={rec['lr']:.2e} "
            f"time={rec['sec']:.1f}s"
        )
