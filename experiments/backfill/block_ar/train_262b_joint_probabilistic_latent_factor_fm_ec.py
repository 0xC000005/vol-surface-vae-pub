#!/usr/bin/env python
"""
262b-v0: joint probabilistic latent-factor FM with bounded history-mean EC baseline.
"""
...
from diffusion.block_ar.joint_probabilistic_latent_factor_fm_ec import (
    JointProbabilisticLatentFactorFMEC,
    JointProbabilisticLatentFactorFMECConfig,
    save_checkpoint,
)
...
def fm_step(
    ...
    *,
    ...
    change_loss = F.smooth_l1_loss(model.decode_residual_change_coord(torch.zeros_like(eps_target), cond), target_coord)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="262b-v0 joint probabilistic latent-factor FM + history-mean EC")
    ...
    model = JointProbabilisticLatentFactorFMEC(cfg).to(device)
