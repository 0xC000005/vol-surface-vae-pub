...
from diffusion.block_ar.daily_joint_cholesky_transition_model import (
    DailyJointCholeskyTransitionConfig,
    DailyJointCholeskyTransitionModel,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="318a-v0 daily chain-rule joint Cholesky transition model"
    )
    parser.add_argument("--context_dim", type=int, default=160)
    parser.add_argument("--history_hidden", type=int, default=96)
    parser.add_argument("--encoder_dropout", type=float, default=0.1)
    parser.add_argument("--decoder_hidden", type=int, default=160)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"trans_std={rec['val_transition_std']:.3f} trans_abs={rec['val_transition_abs']:.3f} "
            f"diag={rec['val_diag_mean']:.3f} offdiag={rec['val_offdiag_abs']:.3f} "
            f"mean_abs={rec['val_mean_abs_err']:.3f} lr={rec['lr']:.2e} time={rec['sec']:.1f}s"
        )
