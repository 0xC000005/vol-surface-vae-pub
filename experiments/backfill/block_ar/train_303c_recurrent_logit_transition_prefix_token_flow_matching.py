...
from diffusion.block_ar.recurrent_logit_transition_prefix_token_flow_matching import (
    RecurrentLogitTransitionPrefixTokenFMConfig,
    RecurrentLogitTransitionPrefixTokenFlowMatching,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="303c-v0 recurrent prefix-token logit-transition flow")
    ...
    model = RecurrentLogitTransitionPrefixTokenFlowMatching(cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "train_state_logit_abs": train_avg["state_logit_abs"],
            "train_target_velocity_std": train_avg["target_velocity_std"],
        ...
        print(
            ...
            f"state_abs={rec['val_state_logit_abs']:.3f} "
            f"lr={rec['lr']:.2e} "
            f"time={rec['sec']:.1f}s"
        )
