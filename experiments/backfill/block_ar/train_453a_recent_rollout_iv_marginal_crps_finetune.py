#!/usr/bin/env python
"""453a: free-running IV-space marginal-CRPS fine-tune for the 392a frontier."""
...
def marginal_crps_score(
    ...
    """Mean univariate CRPS over horizon/cell in IV level space."""
...
def combined_loss(
    ...
    crps, target_abs, pair_abs = marginal_crps_score(sampled_iv, target_iv)
    total = float(fm_anchor_weight) * fm_loss + float(crps_weight) * crps
    metrics = {
        "total": total.detach(),
        "fm_loss": fm_loss.detach(),
        "iv_marginal_crps": crps.detach(),
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "train_iv_marginal_crps": train_avg["iv_marginal_crps"],
            "val_total": val_avg["total"],
            "val_fm_loss": val_avg["fm_loss"],
            "val_iv_marginal_crps": val_avg["iv_marginal_crps"],
        ...
        print(
            ...
            f"iv_crps={rec['val_iv_marginal_crps']:.5f} "
            f"target={rec['val_crps_target_abs']:.4f} pair={rec['val_crps_pair_abs']:.4f} "
