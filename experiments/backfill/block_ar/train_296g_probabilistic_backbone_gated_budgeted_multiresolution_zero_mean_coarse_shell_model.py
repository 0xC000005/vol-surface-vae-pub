...
from diffusion.block_ar.probabilistic_backbone_gated_budgeted_multiresolution_zero_mean_coarse_shell_model import (
    GatedBudgetedMultiresolutionZeroMeanCoarseShellCore,
    ProbabilisticBackboneGatedBudgetedMultiresolutionZeroMeanCoarseShellConfig,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="296g-v0 gated budgeted multiresolution zero-mean shell around frozen 277d backbone"
    )
    ...
    parser.add_argument("--knot_positions", type=str, default="1,2,3,5,10,15,22,30")
    parser.add_argument("--scale_floor", type=float, default=0.02)
    parser.add_argument("--budget_floor", type=float, default=0.05)
    parser.add_argument("--fast_knot_max", type=int, default=5)
    parser.add_argument("--gate_floor", type=float, default=0.02)
    parser.add_argument("--gate_bias_init", type=float, default=0.0)
    ...
    cfg = ProbabilisticBackboneGatedBudgetedMultiresolutionZeroMeanCoarseShellConfig(
        ...
        fast_knot_max=args.fast_knot_max,
        gate_floor=args.gate_floor,
        gate_bias_init=args.gate_bias_init,
        sample_scale_mult=args.sample_scale_mult,
    )
    model = GatedBudgetedMultiresolutionZeroMeanCoarseShellCore(cfg).to(device)
    ...
    print(f"Knot positions: {knot_positions}; fast_knot_max={args.fast_knot_max}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "train_budget_mean": train_avg["budget_mean"],
            "train_fast_gate_mean": train_avg["fast_gate_mean"],
            ...
            "val_budget_mean": val_avg["budget_mean"],
            "val_fast_gate_mean": val_avg["fast_gate_mean"],
        ...
        print(
            ...
            f"budget={rec['val_budget_mean']:.4f} "
            f"gate={rec['val_fast_gate_mean']:.4f} "
