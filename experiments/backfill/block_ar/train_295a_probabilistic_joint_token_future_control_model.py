...
from diffusion.block_ar.probabilistic_joint_token_future_control_model import (
    ProbabilisticJointTokenFutureControlModel,
    ProbabilisticJointTokenFutureControlModelConfig,
    save_checkpoint,
)
...
def build_codebook(
    ...
    residual_coords = target_coords(hist_norm, fut_norm, change_scale_eps).reshape(-1, hist_norm.shape[-1]).cpu()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="295a-v0 probabilistic fixed-horizon joint-token model with future control-state sequence"
    )
    ...
    parser.add_argument("--control_loss_weight", type=float, default=1.0)
    parser.add_argument("--control_radius", type=int, default=1)
    ...
    print("Building direct daily-change codebook...")
    residual_codebook = build_codebook(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "train_control_loss": train_avg["control_loss"],
            "train_control_mae": train_avg["control_mae"],
            ...
            "val_control_loss": val_avg["control_loss"],
            "val_control_mae": val_avg["control_mae"],
        ...
        print(
            ...
            f"control_mae={rec['val_control_mae']:.4f} "
            f"lr={rec['lr']:.2e} "
            f"time={rec['sec']:.1f}s"
        )
