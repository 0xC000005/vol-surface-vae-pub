...
from diffusion.block_ar.probabilistic_joint_token_path_support_model import (
    ProbabilisticJointTokenPathSupportModel,
    ProbabilisticJointTokenPathSupportModelConfig,
    save_checkpoint,
)
...
def build_coarse_codebook(
    ...
    coords = future_norm[:, knot_steps].reshape(train_future.shape[0], -1).cpu()
    ...
    for _ in range(iters):
        ...
        if empty.any():
            ...
            centers[empty] = repl
    return centers.view(coarse_codebook_size, n_knots, -1)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="293d-v0 probabilistic fixed-horizon joint-token path model with explicit coarse path support"
    )
