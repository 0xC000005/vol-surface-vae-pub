#!/usr/bin/env python
"""
170e: Structured joint future Student-t with local variance modulation
...
Keep 170d:
  ...
  - structured separable time/cell base covariance

Add:
  - positive history-conditioned time x cell scale field D_tc
  - exact NLL for Sigma = D_tc (Sigma_time kron Sigma_cell) D_tc
"""
...
class LocalScaleStructuredJointStudentTDecoder(nn.Module):
    """Factored temporal-spatial decoder with separable covariance and local scale field."""
    ...
    def __init__(
        ...
        self.local_log_scale_max = local_log_scale_max
        ...
        self.local_time_factor_head = nn.Linear(d_model, local_rank)
        self.local_cell_factor_head = nn.Linear(d_model, local_rank)
        self.local_time_bias_head = nn.Linear(d_model, 1)
        self.local_cell_bias_head = nn.Linear(d_model, 1)
    ...
    def _init_parameters(self, init_diag: float, init_scale: float, init_nu: float) -> None:
            ...
            if isinstance(module, nn.Linear):
                if any(
                    ...
                    for head_name in (
                        ...
                        "scale_head",
                        "nu_head",
                        "local_time_factor_head",
                        "local_cell_factor_head",
                        "local_time_bias_head",
                        "local_cell_bias_head",
                    )
                ):
                    continue
        ...
        nn.init.normal_(self.local_time_factor_head.weight, mean=0.0, std=1e-3)
        nn.init.normal_(self.local_time_factor_head.bias, mean=0.0, std=1e-3)
        nn.init.normal_(self.local_cell_factor_head.weight, mean=0.0, std=1e-3)
        nn.init.normal_(self.local_cell_factor_head.bias, mean=0.0, std=1e-3)
        nn.init.zeros_(self.local_time_bias_head.weight)
        nn.init.zeros_(self.local_time_bias_head.bias)
        nn.init.zeros_(self.local_cell_bias_head.weight)
        nn.init.zeros_(self.local_cell_bias_head.bias)
    ...
    def forward(
        ...
        prev_rep = prev_u.unsqueeze(1).expand(bsz, n_frames, n_cells)
        ...
        for layer in self.layers:
            h_temp = hidden.permute(0, 2, 1, 3).reshape(bsz * n_cells, n_frames, -1)
            ...
            hidden = h_temp.reshape(bsz, n_cells, n_frames, -1).permute(0, 2, 1, 3)

            h_spat = hidden.reshape(bsz * n_frames, n_cells, -1)
            ...
            hidden = h_spat.reshape(bsz, n_frames, n_cells, -1)
        ...
        local_time_factor = self.local_time_factor_head(time_summary)
        local_cell_factor = self.local_cell_factor_head(cell_summary)
        local_time_bias = self.local_time_bias_head(time_summary).squeeze(-1)
        local_cell_bias = self.local_cell_bias_head(cell_summary).squeeze(-1)
        local_log_scale = (
            torch.einsum("btr,bcr->btc", local_time_factor, local_cell_factor)
            + local_time_bias.unsqueeze(-1)
            + local_cell_bias.unsqueeze(1)
        )
        local_log_scale = local_log_scale - local_log_scale.mean(dim=(1, 2), keepdim=True)
        local_log_scale = torch.tanh(local_log_scale) * self.local_log_scale_max
        local_scale = torch.exp(local_log_scale)

        if self.fixed_nu is None:
            nu = F.softplus(self.nu_head(pooled).squeeze(-1)) + self.nu_floor
            nu = torch.clamp(nu, max=self.nu_max)
        else:
            nu = self.fixed_nu_value.expand(bsz).to(hidden.dtype)
...
class LocalScaleStructuredJointStudentTModel(nn.Module):
    """GRU history encoder + structured Student-t future block with local scale field."""
    ...
    def __init__(
        ...
        self.decoder = LocalScaleStructuredJointStudentTDecoder(**decoder_config)
    ...
    def student_t_nll(
        ...
        bsz, n_frames, n_cells = target_u.shape
        ...
        diff = (target_u - mu) / local_scale.clamp_min(1e-6)
        ...
        logdet_local = 2.0 * torch.log(local_scale.clamp_min(1e-6)).sum(dim=(1, 2))
        logdet = n_cells * logdet_t + n_frames * logdet_c + logdet_local
    ...
    def sample_future_u(self, history_01: torch.Tensor, n_samples: int) -> torch.Tensor:
        ...
        bsz, n_frames, n_cells = mu.shape
        cov_t, cov_c = self.covariance_parts(time_factor, time_diag, cell_factor, cell_diag, scale)
        chol_t = torch.linalg.cholesky(cov_t)
        chol_c = torch.linalg.cholesky(cov_c)

        z = torch.randn(bsz, n_samples, n_frames, n_cells, device=mu.device, dtype=mu.dtype)
        temp = torch.einsum("bij,bsjk->bsik", chol_t, z)
        ...
        t_scale = torch.rsqrt(mix).view(bsz, n_samples, 1, 1)

        return mu.unsqueeze(1) + noise * local_scale.unsqueeze(1) * t_scale
...
def joint_nll_loss(
    model: LocalScaleStructuredJointStudentTModel,
    ...
    metrics = {
        ...
        "time_eff_rank": effective_rank(cov_t).mean(),
        "cell_eff_rank": effective_rank(cov_c).mean(),
        "scale_mean": scale.mean(),
        "local_scale_std": local_scale.std(dim=(1, 2)).mean(),
        "nu_mean": nu.mean(),
    }
...
def evaluate_teacher_forced(
    ...
    for history_01, future_01 in val_loader:
        ...
        totals["val_local_scale_std"] += metrics["local_scale_std"].item() * batch_size
...
def main():
    parser = argparse.ArgumentParser(description="170e: local-scale structured joint Student-t")
    ...
    parser.add_argument("--local_log_scale_max", type=float, default=0.8)
    ...
    decoder_config = dict(
        ...
        local_log_scale_max=args.local_log_scale_max,
    ...
    print(f"\n{'=' * 64}")
    print("170e: Local-scale structured joint future Student-t")
    print(f"{'=' * 64}")
    ...
    print(f"  Time rank={args.time_rank} | Cell rank={args.cell_rank} | Local rank={args.local_rank}")
    print(f"  Support transform: logit(({args.support_lo}, {args.support_hi}))")
    print("  Objective: joint future Student-t NLL")
    print("  Covariance: D_tc (Sigma_time kron Sigma_cell) D_tc")
    print(f"  Local log-scale cap: +/-{args.local_log_scale_max:.2f}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for history_01, future_01 in train_loader:
            ...
            ep_local_std += metrics["local_scale_std"].item()
        ...
        train_metrics = {
            ...
            "train_local_scale_std": ep_local_std / max(nbatches, 1),
            ...
            torch.save(
                {
                    ...
                    "config": {
                        "type": "local_scale_structured_joint_student_t_170e",
        ...
        print(
            ...
            f"t_rank={val_metrics['val_time_eff_rank']:.2f}  "
            f"c_rank={val_metrics['val_cell_eff_rank']:.2f}  "
            f"scale={val_metrics['val_scale_mean']:.4f}  "
            f"local_std={val_metrics['val_local_scale_std']:.4f}  "
