#!/usr/bin/env python
"""
165a: Mean+Residual Decomposition (RC21 H1)

Separates conditional mean prediction from stochastic residual generation.
Oracle debiasing proved spread is adequate, centering is wrong. Probe 0 confirmed
conditions are used but weakly discriminative for per-window centering.

Architecture:
  mean_head(cond, prev_mean) -> mean_delta     # deterministic expected change
  mean_pred = prev_mean + mean_delta            # conditional mean
  resid_k = tanh(decoder(cond, prev_k, z_k))   # noise-dependent residual
  resid_centered = resid_k - resid_k.mean(K)   # zero-mean construction
  frame_k = mean_pred + resid_centered          # ensemble member

Loss:
  Huber on mean_pred (dense deterministic gradient for centering)
  afCRPS on frame_k (stochastic quality)
  Softplus barrier on frame_k (floor constraint)

Two-phase training:
  Phase 1 (mean_warmup_epochs): decoder frozen, MSE only on mean_head + encoder
  Phase 2 (remaining): joint training with MSE + CRPS + VS + IS + floor
...
class ARMeanResidualModel(nn.Module):
    ...
    def sample_batched(self, history, n_samples=50, **kwargs):
        ...
        with torch.no_grad():
            ...
            # --- Mean path (deterministic, computed once at B level) ---
            ...
            mean_frames = torch.stack(mean_frames, dim=1)  # (B, T, C)

            # --- Residual path (stochastic, per chunk) ---
            all_samples = []
            for start in range(0, n_samples, CHUNK):
                ...
                for t in range(T):
                    ...
                    raw_delta = self.decoder(cond_k, prev_k, z_t)
                    raw_resid = torch.tanh(raw_delta)  # (Bk, C)

                    # Zero-mean residual construction
                    raw_resid_Bk = raw_resid.reshape(B, k, C)
                    resid_centered = raw_resid_Bk - raw_resid_Bk.mean(dim=1, keepdim=True)

                    # Combine with mean prediction
                    frame_Bk = mean_frames[:, t].unsqueeze(1) + resid_centered  # (B, k, C)
                    frame_flat = frame_Bk.reshape(Bk, C)

                    # Reflecting boundary
                    frame_flat = reflecting_boundary(frame_flat)

                    chunk_frames.append(frame_Bk)

                    # GRU feedback with per-member frames
                ...
                # Apply reflecting boundary to full tensor
                chunk_frames = reflecting_boundary(chunk_frames)
        ...
        for hist, gt_frames, last_frame in train_loader:
            ...
            for t in range(T):
                ...
                if is_phase1:
                    ...
                    delta = model.decoder(cond_K, prev, z_t)  # (B*K, C)
                    raw_resid = torch.tanh(delta)  # (B*K, C)

                    # Zero-mean residual construction
                    raw_resid_BK = raw_resid.reshape(B, K, C)
                    resid_centered = raw_resid_BK - raw_resid_BK.mean(dim=1, keepdim=True)

                    # Combine: frame = mean_pred + centered_residual
                    frame_BK = mean_pred.unsqueeze(1) + resid_centered  # (B, K, C)
                    frame_t = frame_BK.reshape(B * K, C)
            ...
            for v_hist, v_gt, v_last in val_loader:
                ...
                for t in range(T):
                    ...
                    if not is_phase1:
                        ...
                        delta = model.decoder(v_cond_K, v_prev, z_t)
                        raw_resid = torch.tanh(delta)
                        raw_resid_BK = raw_resid.reshape(B2, K2, C)
                        resid_centered = raw_resid_BK - raw_resid_BK.mean(dim=1, keepdim=True)
                        frame_BK = v_mean_pred.unsqueeze(1) + resid_centered
                        frame_flat = frame_BK.reshape(B2 * K2, C)
