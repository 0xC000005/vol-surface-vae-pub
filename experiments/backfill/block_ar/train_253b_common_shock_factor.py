#!/usr/bin/env python
"""
253b: 253a-ec + bounded common shock channel.

Single decisive follow-up:
    keep the structural gains from 253a-ec and test whether a fast common shock path
    improves temporal law / MR / kurtosis / jump realism without reintroducing idio leakage.
"""
...
from diffusion.block_ar.dynamic_change_factor_shock_ssm import (
    DynamicChangeFactorShockSSM,
    DynamicFactorShockSSMConfig,
    config_to_dict,
)
...
def build_losses(
    ...
    model: DynamicChangeFactorShockSSM,
    lambda_level: float,
    lambda_change: float,
    lambda_jump: float,
...
def warm_start_from_253a(model: DynamicChangeFactorShockSSM, checkpoint_path: str, device: torch.device) -> list[str]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="253b common-shock dynamic factor model")
    ...
    parser.add_argument("--max_shock_ratio", type=float, default=1.00)
    parser.add_argument("--shock_shrink", type=float, default=0.10)
    ...
    cfg = DynamicFactorShockSSMConfig(
        history_len=args.history_len,
        future_len=args.future_len,
        n_cells=D,
        latent_dim=args.L,
        ...
        max_shock_ratio=args.max_shock_ratio,
        shock_shrink=args.shock_shrink,
