#!/usr/bin/env python
"""
261d-v0: scale-calibrated latent-factor residual FM on top of frozen 260e.
"""
...
from diffusion.block_ar.scale_calibrated_latent_factor_residual_fm import (
    ScaleCalibratedLatentFactorResidualFM,
    ScaleCalibratedLatentFactorResidualFMConfig,
    save_checkpoint,
)
...
def fm_step(
    ...
    *,
    scale_supervision_weight: float,
    ...
    with torch.no_grad():
        ...
        target_scale = factor_target.pow(2).mean(dim=-1, keepdim=True).sqrt().clamp_min(model.cfg.scale_floor)
        target_standardized = factor_target / target_scale

    x0 = torch.randn_like(target_standardized)
    t = torch.rand(bsz, device=target_standardized.device, dtype=target_standardized.dtype)
    t_view = t[:, None, None]
    x_t = (1.0 - t_view) * x0 + t_view * target_standardized
    target_v = target_standardized - x0
    pred_v = model.velocity(x_t, t, cond)
    fm_loss = (pred_v - target_v).pow(2).mean()

    pred_scale = cond["scale_profile"]
    scale_loss = F.smooth_l1_loss(pred_scale, target_scale)

    sample_std = model.sample_standardized_factor_residuals(history_norm, n_samples=4, cond=cond)
    sample_std = sample_std - sample_std.mean(dim=1, keepdim=True)
    scaled_factor = sample_std * pred_scale.unsqueeze(1)
    panel_resid_coord = model.decode_panel_residual(scaled_factor, cond["loadings"])
    panel_resid_std = panel_resid_coord.std(dim=1).mean()

    loss = fm_loss + scale_supervision_weight * scale_loss
    metrics = {
        "total": loss.detach(),
        "fm_loss": fm_loss.detach(),
        "scale_loss": scale_loss.detach(),
        "panel_resid_std": panel_resid_std.detach(),
        "pred_scale_mean": pred_scale.mean().detach(),
        "target_scale_mean": target_scale.mean().detach(),
    }
    return loss, metrics


def main() -> None:
    parser = argparse.ArgumentParser(description="261d-v0 scale-calibrated latent-factor residual FM")
    ...
    parser.add_argument("--scale_supervision_weight", type=float, default=1.0)
    ...
    model = ScaleCalibratedLatentFactorResidualFM(cfg, base_model=base_model).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"scale={rec['val_scale_loss']:.5f} pstd={rec['val_panel_resid_std']:.5f} "
            f"preds={rec['val_pred_scale_mean']:.4f}/{rec['val_target_scale_mean']:.4f} "
