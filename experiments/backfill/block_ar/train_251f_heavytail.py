#!/usr/bin/env python
"""
251f — H2: Student-t z_0 via LatentFM base distribution.

Transfers the 108a tail unlock (kurtosis 0.955 with Student-t z_0 + AR rho=0.8)
into the 251 stationary-SDE framework. Gaussian eps_t in the SDE is preserved
(CLT-safe increments); heavy tails enter ONLY at t=0 through LatentFM's base prior.
If they survive the 30-step SDE, we expect kurtosis and pathwise_jump gates to move.

Primary warm-start: 251b_final (stationary SDE, no OU reg confound).
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="251f: H2 Student-t z_0 base prior")
    ...
    # H2
    parser.add_argument("--latent_fm_base_dist", type=str, default="student_t", choices=["gaussian", "student_t"])
    parser.add_argument("--latent_fm_nu_init", type=float, default=-2.0,
                        help="softplus(-2) ~ 0.127; nu = 2 + softplus = ~2.13")
    # Training phases
    ...
    # Attach LatentFM if missing
    if not model.cfg.use_latent_fm:
        raise ValueError("H2 requires LatentFM (warm-start must have use_latent_fm=True)")

    # H2: Switch base distribution to Student-t
    if args.latent_fm_base_dist == "student_t":
        if model.latent_fm.nu_raw is None:
            model.cfg.latent_fm_base_dist = "student_t"
            model.cfg.latent_fm_nu_init = args.latent_fm_nu_init
            model.latent_fm.nu_raw = nn.Parameter(
                torch.tensor(float(args.latent_fm_nu_init), device=device)
            )
            nu = 2.0 + F.softplus(model.latent_fm.nu_raw).item()
            print(f"  H2 attached nu_raw={args.latent_fm_nu_init} -> nu={nu:.3f}")
        else:
            nu = 2.0 + F.softplus(model.latent_fm.nu_raw).item()
            print(f"  inherited nu_raw (nu={nu:.3f})")
        names = {n for n, _ in model.named_parameters()}
        assert any(n == "latent_fm.nu_raw" for n in names)

    # Ensure LatentSDE (inherit from warm-start)
    if args.use_latent_sde and model.latent_sde is None:
        raise ValueError("H2 expects warm-start with LatentSDE (251b, 251c)")
    ...
    print(f"\nPhase 1: freeze backbone ({n_train:,} trainable). epochs 1..{args.freeze_backbone_epochs}")
    ...
    for epoch in range(1, args.total_epochs + 1):
        ...
        nu_val = 2.0 + F.softplus(model.latent_fm.nu_raw).item() if model.latent_fm.nu_raw is not None else float("nan")
        print(
            f"Epoch {epoch:3d}/{args.total_epochs} [{phase}] "
            f"train_cell={train_avg.get('cell_crps', 0):.4f}  "
            f"val_cell={val_avg.get('cell_crps', 0):.4f}  "
            f"val_total={val_avg.get('total', 0):.4f}  nu={nu_val:.3f}  ({dt:.1f}s)"
        )
