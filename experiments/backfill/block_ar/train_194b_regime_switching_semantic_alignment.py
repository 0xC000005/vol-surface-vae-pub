#!/usr/bin/env python
"""
194b: Semantically aligned regime-switching AR latent-factor Student-t model.

Targeted follow-up to 194a:
  - keep the same AR regime-switching latent-factor class
  - keep the same sticky discrete path and exact teacher-forced marginalization
  - align quiet/mid/high state usage to concentrated quiet-state underfit
  - discourage broad-state activation on already-safe windows

Usage:
    PYTHONPATH=. python experiments/backfill/block_ar/train_194b_regime_switching_semantic_alignment.py \
        --output_dir models/backfill/regime_switching_ar_latent_factor_194b --device cuda
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="194b: semantically aligned regime-switching AR latent-factor Student-t")
    ...
    parser.add_argument("--warm_start", type=str, default="models/backfill/regime_switching_ar_latent_factor_194a/best_model.pt")
    ...
    print(f"\n{'=' * 64}")
    print("194b: Semantically Aligned Regime-Switching AR Latent-Factor Student-t")
    print(f"{'=' * 64}")
    ...
    print("  Objective: exact teacher-forced discrete-state marginal likelihood + semantic state alignment")
            ...
            torch.save(
                {
                    ...
                    "config": {
                        "type": "regime_switching_ar_latent_factor_194b",
