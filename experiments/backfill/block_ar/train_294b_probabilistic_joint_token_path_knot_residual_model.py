...
from diffusion.block_ar.probabilistic_joint_token_path_knot_residual_model import (
    ProbabilisticJointTokenPathKnotResidualModel,
    ProbabilisticJointTokenPathKnotResidualModelConfig,
    save_checkpoint,
)
...
def parse_knot_positions(raw: str, future_len: int) -> list[int]:
    knots = [int(x.strip()) for x in raw.split(",") if x.strip()]
    if not knots:
        raise ValueError("knot_positions must not be empty")
    if knots[0] < 1 or knots[-1] > future_len:
        raise ValueError(f"knot_positions must stay within [1, {future_len}]")
    if knots != sorted(knots):
        raise ValueError("knot_positions must be strictly increasing")
    if len(set(knots)) != len(knots):
        raise ValueError("knot_positions must be unique")
    return knots


def build_knot_interpolation_matrix(future_len: int, knot_positions: list[int]) -> torch.Tensor:
    n_knots = len(knot_positions)
    matrix = torch.zeros(future_len, n_knots, dtype=torch.float32)
...
def target_knot_values(
    history_norm: torch.Tensor,
    future_norm: torch.Tensor,
    knot_positions: list[int],
) -> torch.Tensor:
    curr = history_norm[:, -1].unsqueeze(1)
    delta_path = future_norm - curr
    knot_idx = torch.tensor([k - 1 for k in knot_positions], dtype=torch.long)
    return delta_path.index_select(dim=1, index=knot_idx)
...
def build_residual_codebook(
    ...
    knot_values = target_knot_values(hist_norm, fut_norm, knot_positions)
    curr = hist_norm[:, -1].unsqueeze(1)
    scaffold_levels = curr + torch.einsum("tk,bkc->btc", knot_matrix, knot_values)
    residual_coords = (
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="294b-v0 probabilistic fixed-horizon joint-token model with piecewise-linear knot scaffold"
    )
    ...
    parser.add_argument("--knot_positions", type=str, default="1,4,8,14,21,30")
    parser.add_argument("--knot_loss_weight", type=float, default=1.0)
    ...
    knot_positions = parse_knot_positions(args.knot_positions, args.future_len)
    knot_matrix = build_knot_interpolation_matrix(args.future_len, knot_positions)
    ...
    torch.save(knot_matrix, out_dir / "knot_matrix.pt")
    ...
    cfg = ProbabilisticJointTokenPathKnotResidualModelConfig(
        ...
        codebook_size=args.codebook_size,
        n_knots=len(knot_positions),
        knot_loss_weight=args.knot_loss_weight,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "train_knot_loss": train_avg["knot_loss"],
            "train_knot_mae": train_avg["knot_mae"],
            ...
            "val_knot_loss": val_avg["knot_loss"],
            "val_knot_mae": val_avg["knot_mae"],
        ...
        print(
            ...
            f"knot_mae={rec['val_knot_mae']:.4f} "
            f"lr={rec['lr']:.2e} "
            f"time={rec['sec']:.1f}s"
        )
