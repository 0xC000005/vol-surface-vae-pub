...
from diffusion.block_ar.history_conditioned_level_aware_probabilistic_token_state_logit_transition_model import (
    HistoryConditionedLevelAwareProbabilisticTokenStateLogitTransitionConfig,
    HistoryConditionedLevelAwareProbabilisticTokenStateLogitTransitionModel,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="314c-v0 history-conditioned level-aware token-state logit transition model"
    )
    parser.add_argument("--context_dim", type=int, default=96)
    parser.add_argument("--history_hidden", type=int, default=96)
    parser.add_argument("--encoder_dropout", type=float, default=0.1)
    parser.add_argument("--n_tokens", type=int, default=4)
    parser.add_argument("--token_dim", type=int, default=24)
    parser.add_argument("--posterior_hidden", type=int, default=128)
    parser.add_argument("--posterior_layers", type=int, default=2)
    parser.add_argument("--prior_layers", type=int, default=2)
    parser.add_argument("--prior_heads", type=int, default=4)
    parser.add_argument("--prior_ff_mult", type=int, default=4)
    ...
    model = HistoryConditionedLevelAwareProbabilisticTokenStateLogitTransitionModel(cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "val_kl": val_avg["kl_loss"],
            "val_prior_std": val_avg["prior_std"],
            "val_post_std": val_avg["post_std"],
            "val_transition_std": val_avg["transition_std"],
            "val_recon_transition_abs": val_avg["recon_transition_abs"],
        ...
        print(
            ...
            f"val={rec['val_total']:.5f} recon={rec['val_recon']:.5f} "
            f"kl={rec['val_kl']:.5f} pstd={rec['val_prior_std']:.3f} "
            f"qstd={rec['val_post_std']:.3f} trans_std={rec['val_transition_std']:.3f} "
            f"recon_abs={rec['val_recon_transition_abs']:.3f} "
