#!/usr/bin/env python
"""
Diagnostic script for Exp 156a: noise_dim=4 CLN transformer (H1-S1).

A. Per-cell CI breakdown (which cells are worst?)
B. Correlation matrix vs GT (is bottleneck forcing structure?)
C. Spread trajectory per epoch (from training history)
D. CLN scale analysis (are scales saturated?)
E. 1.3x scaling test (does scaling residuals fix CI?)
F. Spread-skill per horizon (where does it fail?)
"""
...
def generate_samples(model, conditions, base_preds, n_samples=50, device='cuda'):
    ...
    return np.array(all_samples)  # (N, n_samples, T, C)
...
def main():
    ...
    # Load model
    model, cfg = load_model('models/backfill/flow_156a/final_model.pt', device)
    ...
    for r in range(5):
        for c in range(5):
            ...
            cell_ci[r, c] = cov
    print("Cell CI (5x5 grid, rows=moneyness, cols=tenor):")
    for r in range(5):
    ...
    corr_diff = np.abs(gen_corr - gt_corr).mean()
    ...
    print(f"Corr ratio: {corr_ratio:.3f} (target ~1.0)")
    print(f"Mean |corr_diff|: {corr_diff:.3f}")
    print(f"Eff rank: gen={gen_er:.2f} vs GT={gt_er:.2f} (ratio={gen_er/gt_er:.3f})")
    results['corr_ratio'] = float(corr_ratio)
    results['corr_diff'] = float(corr_diff)
    ...
    # ===== C. Spread trajectory from training history =====
    print("\n=== C. Spread trajectory ===")
    hist_path = Path('models/backfill/flow_156a/training_history.json')
    if hist_path.exists():
        history = json.load(open(hist_path))
        spreads = [(h['epoch'], h.get('spread', None)) for h in history if 'spread' in h]
        print("Epoch  Spread")
        for ep, s in spreads[::20]:  # every 20 epochs
            print(f"  {ep:4d}  {s:.4f}")
        results['spread_ep1'] = spreads[0][1] if spreads else None
        results['spread_ep200'] = spreads[-1][1] if spreads else None
        results['spread_peak'] = max(s for _, s in spreads)
        results['spread_peak_epoch'] = [ep for ep, s in spreads if s == results['spread_peak']][0]
        print(f"Peak spread: {results['spread_peak']:.4f} at epoch {results['spread_peak_epoch']}")
        print(f"Final spread: {results['spread_ep200']:.4f}")
        print(f"Contraction: {results['spread_peak'] / results['spread_ep200']:.2f}x")
    ...
    for i, layer in enumerate(model.layers):
        for name in ['temp_cln', 'temp_ff_cln', 'spat_cln', 'spat_ff_cln']:
            ...
            b = layer[name].bias_proj.weight.detach()
            key = f"L{i}_{name}"
            scale_norms[key] = {
                'scale_norm': float(w.norm()),
                'bias_norm': float(b.norm()),
                'scale_range': [float(w.min()), float(w.max())],
            }
    total_scale = np.mean([v['scale_norm'] for v in scale_norms.values()])
    total_bias = np.mean([v['bias_norm'] for v in scale_norms.values()])
    print(f"Mean CLN scale norm: {total_scale:.3f}")
    print(f"Mean CLN bias norm: {total_bias:.3f}")
    # Check if scales are saturated (same across layers)
    layer_scales = [np.mean([scale_norms[f"L{i}_{n}"]['scale_norm']
                             for n in ['temp_cln', 'temp_ff_cln', 'spat_cln', 'spat_ff_cln']])
                    for i in range(4)]
    print(f"Per-layer mean scale: {['%.3f' % s for s in layer_scales]}")
    results['cln_mean_scale'] = total_scale
    results['cln_layer_scales'] = layer_scales
    ...
    with torch.no_grad():
        for i in range(N):
            ...
            residual = model(cond, noise).cpu().numpy()
            scaled_residual = residual * 1.3  # scale up
            combined = np.clip(val_preds[i] + scaled_residual, 0, 1)
            all_scaled.append(combined.reshape(50, 30, 25))
    scaled_samples = np.array(all_scaled)

    # CI at 1.3x
    worst_ci_scaled = 1.0
    for cidx in range(C):
        lo = np.percentile(scaled_samples[:, :, :, cidx], 5, axis=1)
        hi = np.percentile(scaled_samples[:, :, :, cidx], 95, axis=1)
        cov = ((gt[:, :, cidx] >= lo) & (gt[:, :, cidx] <= hi)).mean()
        worst_ci_scaled = min(worst_ci_scaled, cov)

    # Corr at 1.3x
    sc_ch = np.diff(scaled_samples[:, 0], axis=1).reshape(-1, C)
    sc_corr = np.corrcoef(sc_ch.T)
    sc_corr_ratio = np.abs(sc_corr).mean() / (np.abs(gt_corr).mean() + 1e-6)

    # KS at 1.3x
    ks_count = sum(1 for c2 in range(C) if ks_2samp(sc_ch[:, c2], gt_ch[:, c2])[0] < 0.15)

    print(f"1.3x scaling: CI={worst_ci_scaled:.3f} (was {results['ci_worst']:.3f})")
    print(f"1.3x scaling: corr={sc_corr_ratio:.3f} (was {corr_ratio:.3f})")
    print(f"1.3x scaling: KS={ks_count}/25")
    results['scaled_1_3x'] = {
        'ci_worst': float(worst_ci_scaled),
        'corr': float(sc_corr_ratio),
        'ks': ks_count,
    }

    # Also try 1.5x and 2.0x
    for scale in [1.5, 2.0]:
        ...
        for cidx in range(C):
            ...
            cov = ((gt[:, :, cidx] >= lo) & (gt[:, :, cidx] <= hi)).mean()
            wci = min(wci, cov)
        sc_ch2 = np.diff(sc[:, 0], axis=1).reshape(-1, C)
        sc_corr2 = np.corrcoef(sc_ch2.T)
        sc_cr = np.abs(sc_corr2).mean() / (np.abs(gt_corr).mean() + 1e-6)
        ks2 = sum(1 for c2 in range(C) if ks_2samp(sc_ch2[:, c2], gt_ch[:, c2])[0] < 0.15)
        print(f"{scale}x scaling: CI={wci:.3f}  corr={sc_cr:.3f}  KS={ks2}/25")
        results[f'scaled_{scale}x'] = {'ci_worst': float(wci), 'corr': float(sc_cr), 'ks': ks2}

    # ===== F. Spread-skill per horizon =====
    ...
    for h in range(30):
        lo = np.percentile(samples[:, :, h], 5, axis=1)
        hi = np.percentile(samples[:, :, h], 95, axis=1)
        results['horizon_ci'][h+1] = float(((gt[:, h] >= lo) & (gt[:, h] <= hi)).mean())

    # Growing uncertainty check
    spreads_h = [samples[:, :, h].std(axis=1).mean() for h in range(30)]
    gu = sum(1 for i in range(1, 30) if spreads_h[i] >= spreads_h[i-1] * 0.95)
    print(f"\nGrowing uncertainty: {gu}/29 monotonic steps")
    print(f"Spread h1={spreads_h[0]:.4f}  h30={spreads_h[-1]:.4f}  ratio={spreads_h[-1]/spreads_h[0]:.2f}x")
    results['growing_unc'] = gu
    results['spread_h1'] = float(spreads_h[0])
    results['spread_h30'] = float(spreads_h[-1])

    # Save results
    out_dir = Path('results/block_ar/156a_diag')
