...
from diffusion.block_ar.hierarchical_retrieval_temperature_residual_scenario_generator import (
    HierarchicalTemperatureResidualRetrievalScenarioGenerator,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="281a-v0 residual temperature hierarchy")
    ...
    optimizer = torch.optim.AdamW(model.temperature_head.parameters(), lr=args.lr)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_metrics["loss"] < best_val:
            ...
            best_epoch = epoch
            best_state = {k: v.detach().cpu() for k, v in model.temperature_head.state_dict().items()}
    ...
    save_payload = {
        ...
        "tau_max": args.tau_max,
        "hidden_dim": args.hidden_dim,
        "temperature_head_state_dict": best_state,
