...
from diffusion.block_ar.recurrent_student_t_transition_likelihood import (
    RecurrentStudentTTransitionConfig,
    RecurrentStudentTTransitionLikelihood,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="306a-v0 recurrent full-cov Student-t transition likelihood")
    parser.add_argument("--hidden_dim", type=int, default=160)
    parser.add_argument("--gru_layers", type=int, default=2)
    parser.add_argument("--gru_dropout", type=float, default=0.1)
    parser.add_argument("--param_hidden", type=int, default=320)
    parser.add_argument("--param_layers", type=int, default=3)
    parser.add_argument("--param_dropout", type=float, default=0.1)
    parser.add_argument("--logit_eps", type=float, default=1e-4)
    parser.add_argument("--scale_floor", type=float, default=1e-3)
    parser.add_argument("--min_df", type=float, default=2.5)
    parser.add_argument("--max_df", type=float, default=40.0)
    ...
    cfg = RecurrentStudentTTransitionConfig(
        ...
        param_layers=args.param_layers,
    ...
    model = RecurrentStudentTTransitionLikelihood(cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "val_loc_abs": val_avg["loc_abs"],
            "val_scale_diag_mean": val_avg["scale_diag_mean"],
            "val_df_mean": val_avg["df_mean"],
        ...
        print(
            ...
            f"loc={rec['val_loc_abs']:.3f} scale={rec['val_scale_diag_mean']:.3f} "
            f"df={rec['val_df_mean']:.2f} trans_std={rec['val_transition_std']:.3f} "
