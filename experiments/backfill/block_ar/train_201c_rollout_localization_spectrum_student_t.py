#!/usr/bin/env python
"""
201c: same Transformer AR Student-t model as 201b, with rollout-shape fixes.

Changes vs 201b:
  - keep underfit-aware weighting and self-fed rollout likelihood
  - add rollout spectrum matching to reduce shoulder smear
  - add rollout tail-count/timing matching
  - add rollout tail-location matching and concentration loss
  - keep the same architecture and fixed ex-ante selection score
"""
...
def smooth_indicator(x: torch.Tensor, threshold: float, temperature: float) -> torch.Tensor:
    return torch.sigmoid((x - threshold) / max(temperature, 1e-6))


def smooth_quiet_indicator(x: torch.Tensor, threshold: float, temperature: float) -> torch.Tensor:
    return torch.sigmoid((threshold - x) / max(temperature, 1e-6))


def reparameterized_next_iv_multi(
    ...
    sample_u = mu.unsqueeze(1) + total_noise * t_scale
    return unconstrained_to_iv(sample_u, lo=model.support_lo, hi=model.support_hi)


def rollout_distribution_losses(
    prev_01: torch.Tensor,
    target_t: torch.Tensor,
    probe_samples_iv: torch.Tensor,
    objective_config: dict[str, float],
) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    gt_delta_abs = (target_t - prev_01).abs()
    sample_delta_abs = (probe_samples_iv - prev_01.unsqueeze(1)).abs()

    q50 = objective_config["q50_threshold"]
    q95 = objective_config["q95_threshold"]
    q99 = objective_config["q99_threshold"]
    temp = objective_config["spectrum_temperature"]

    gen_quiet = smooth_quiet_indicator(sample_delta_abs, q50, temp)
    gen_extreme = smooth_indicator(sample_delta_abs, q99, temp)
    gen_shoulder = (1.0 - gen_quiet - gen_extreme).clamp_min(0.0)

    gt_quiet = (gt_delta_abs <= q50).float()
    gt_extreme = (gt_delta_abs >= q99).float()
    gt_shoulder = ((gt_delta_abs > q50) & (gt_delta_abs < q99)).float()

    gen_quiet_mass = gen_quiet.mean()
    gen_shoulder_mass = gen_shoulder.mean()
    gen_extreme_mass = gen_extreme.mean()
    gt_quiet_mass = gt_quiet.mean()
    gt_shoulder_mass = gt_shoulder.mean()
    gt_extreme_mass = gt_extreme.mean()

    spectrum_loss = (
        F.smooth_l1_loss(gen_quiet_mass, gt_quiet_mass)
        + F.smooth_l1_loss(gen_shoulder_mass, gt_shoulder_mass)
        + F.smooth_l1_loss(gen_extreme_mass, gt_extreme_mass)
    )

    soft_q95 = smooth_indicator(sample_delta_abs, q95, temp)
    soft_q99 = smooth_indicator(sample_delta_abs, q99, temp)
    gen_q95_step_count = soft_q95.sum(dim=-1).mean(dim=1)
    gen_q99_step_count = soft_q99.sum(dim=-1).mean(dim=1)
    gt_q95_step_count = (gt_delta_abs >= q95).float().sum(dim=-1)
    gt_q99_step_count = (gt_delta_abs >= q99).float().sum(dim=-1)

    tail_count_loss = (
        F.smooth_l1_loss(gen_q95_step_count, gt_q95_step_count)
        + F.smooth_l1_loss(gen_q99_step_count, gt_q99_step_count)
    )

    gt_tail_sev = F.relu(gt_delta_abs - q95)
    gt_tail_mass = gt_tail_sev.sum(dim=-1, keepdim=True)
    has_tail = (gt_tail_mass.squeeze(-1) > 0).float()
    target_map = gt_tail_sev / gt_tail_mass.clamp_min(1e-6)

    gen_tail_map = soft_q95.mean(dim=1)
    gen_tail_map = gen_tail_map / gen_tail_map.sum(dim=-1, keepdim=True).clamp_min(1e-6)
    localization_ce = -(target_map * torch.log(gen_tail_map.clamp_min(1e-6))).sum(dim=-1)
    localization_loss = (has_tail * localization_ce).sum() / has_tail.sum().clamp_min(1.0)

    gen_entropy = -(gen_tail_map * torch.log(gen_tail_map.clamp_min(1e-6))).sum(dim=-1)
    gen_entropy = gen_entropy / np.log(gen_tail_map.shape[-1])
    concentration_loss = (has_tail * gen_entropy).sum() / has_tail.sum().clamp_min(1.0)

    aux = {
        "gt_quiet_mass": gt_quiet_mass.detach(),
        "gen_quiet_mass": gen_quiet_mass.detach(),
        "gt_shoulder_mass": gt_shoulder_mass.detach(),
        "gen_shoulder_mass": gen_shoulder_mass.detach(),
        "gt_extreme_mass": gt_extreme_mass.detach(),
        "gen_extreme_mass": gen_extreme_mass.detach(),
        "gen_q95_step_count_mean": gen_q95_step_count.mean().detach(),
        "gt_q95_step_count_mean": gt_q95_step_count.mean().detach(),
        "gen_q99_step_count_mean": gen_q99_step_count.mean().detach(),
        "gt_q99_step_count_mean": gt_q99_step_count.mean().detach(),
        "tail_entropy": concentration_loss.detach(),
        "tail_localization_ce": localization_loss.detach(),
        "tail_steps_frac": has_tail.mean().detach(),
    }
    total = (
        objective_config["rollout_spectrum_weight"] * spectrum_loss
        + objective_config["rollout_tail_count_weight"] * tail_count_loss
        + objective_config["rollout_localization_weight"] * localization_loss
        + objective_config["rollout_concentration_weight"] * concentration_loss
    )
    return total, aux
...
def selffed_rollout_likelihood_objective(
    ...
    total_step_weight = 0.0
    total_spectrum = 0.0
    total_tail_count = 0.0
    total_localization = 0.0
    total_concentration = 0.0
    total_gen_quiet = 0.0
    total_gt_quiet = 0.0
    total_gen_shoulder = 0.0
    total_gt_shoulder = 0.0
    total_gen_extreme = 0.0
    total_gt_extreme = 0.0
    total_gen_q99 = 0.0
    total_gt_q99 = 0.0
    total_tail_steps = 0.0
    ...
    for step in range(rollout_steps):
        ...
        weighted_nll = (step_weight * nll_t).mean()
        weighted_mae = (step_weight * (mu_iv - target_t).abs().mean(dim=-1)).mean()
        probe_samples_iv = reparameterized_next_iv_multi(
            model=model,
            mu=mu,
            factor=factor,
            diag=diag,
            scale=scale,
            nu=nu,
            n_samples=int(objective_config["rollout_probe_samples"]),
        )
        rollout_shape_loss, rollout_aux = rollout_distribution_losses(
            prev_01=true_prev_01,
            target_t=target_t,
            probe_samples_iv=probe_samples_iv,
            objective_config=objective_config,
        )
        step_loss = (
            weighted_nll
        ...
        total_underfit = total_underfit + aux["underfit_mean"]
        total_step_weight = total_step_weight + aux["step_weight_mean"]
        total_spectrum = total_spectrum + objective_config["rollout_spectrum_weight"] * (
            F.smooth_l1_loss(rollout_aux["gen_quiet_mass"], rollout_aux["gt_quiet_mass"])
            + F.smooth_l1_loss(rollout_aux["gen_shoulder_mass"], rollout_aux["gt_shoulder_mass"])
            + F.smooth_l1_loss(rollout_aux["gen_extreme_mass"], rollout_aux["gt_extreme_mass"])
        )
        total_tail_count = total_tail_count + objective_config["rollout_tail_count_weight"] * (
            (rollout_aux["gen_q95_step_count_mean"] - rollout_aux["gt_q95_step_count_mean"]).abs()
            + (rollout_aux["gen_q99_step_count_mean"] - rollout_aux["gt_q99_step_count_mean"]).abs()
        )
        total_localization = total_localization + objective_config["rollout_localization_weight"] * rollout_aux["tail_localization_ce"]
        total_concentration = total_concentration + objective_config["rollout_concentration_weight"] * rollout_aux["tail_entropy"]
        total_gen_quiet = total_gen_quiet + rollout_aux["gen_quiet_mass"]
        total_gt_quiet = total_gt_quiet + rollout_aux["gt_quiet_mass"]
        total_gen_shoulder = total_gen_shoulder + rollout_aux["gen_shoulder_mass"]
        total_gt_shoulder = total_gt_shoulder + rollout_aux["gt_shoulder_mass"]
        total_gen_extreme = total_gen_extreme + rollout_aux["gen_extreme_mass"]
        total_gt_extreme = total_gt_extreme + rollout_aux["gt_extreme_mass"]
        total_gen_q99 = total_gen_q99 + rollout_aux["gen_q99_step_count_mean"]
        total_gt_q99 = total_gt_q99 + rollout_aux["gt_q99_step_count_mean"]
        total_tail_steps = total_tail_steps + rollout_aux["tail_steps_frac"]
    ...
    metrics = {
        ...
        "rollout_spectrum_loss": total_spectrum * scale_fac,
        "rollout_tail_count_loss": total_tail_count * scale_fac,
        "rollout_localization_loss": total_localization * scale_fac,
        "rollout_concentration_loss": total_concentration * scale_fac,
        "rollout_gen_quiet_mass": total_gen_quiet * scale_fac,
        "rollout_gt_quiet_mass": total_gt_quiet * scale_fac,
        "rollout_gen_shoulder_mass": total_gen_shoulder * scale_fac,
        "rollout_gt_shoulder_mass": total_gt_shoulder * scale_fac,
        "rollout_gen_extreme_mass": total_gen_extreme * scale_fac,
        "rollout_gt_extreme_mass": total_gt_extreme * scale_fac,
        "rollout_gen_q99_step_count": total_gen_q99 * scale_fac,
        "rollout_gt_q99_step_count": total_gt_q99 * scale_fac,
        "rollout_tail_steps_frac": total_tail_steps * scale_fac,
    }
    return total_loss * scale_fac, metrics
...
def evaluate_selffed_rollout(
    ...
    total = {
        ...
        "val_rollout_spectrum_loss": 0.0,
        "val_rollout_tail_count_loss": 0.0,
        "val_rollout_localization_loss": 0.0,
        "val_rollout_concentration_loss": 0.0,
        "val_rollout_gen_quiet_mass": 0.0,
        "val_rollout_gt_quiet_mass": 0.0,
        "val_rollout_gen_shoulder_mass": 0.0,
        "val_rollout_gt_shoulder_mass": 0.0,
        "val_rollout_gen_extreme_mass": 0.0,
        "val_rollout_gt_extreme_mass": 0.0,
        "val_rollout_gen_q99_step_count": 0.0,
        "val_rollout_gt_q99_step_count": 0.0,
        "val_rollout_tail_steps_frac": 0.0,
    }
    total_count = 0
    for history_01, future_01 in val_loader:
        ...
        total["val_rollout_spectrum_loss"] += metrics["rollout_spectrum_loss"].item() * bs
        total["val_rollout_tail_count_loss"] += metrics["rollout_tail_count_loss"].item() * bs
        total["val_rollout_localization_loss"] += metrics["rollout_localization_loss"].item() * bs
        total["val_rollout_concentration_loss"] += metrics["rollout_concentration_loss"].item() * bs
        total["val_rollout_gen_quiet_mass"] += metrics["rollout_gen_quiet_mass"].item() * bs
        total["val_rollout_gt_quiet_mass"] += metrics["rollout_gt_quiet_mass"].item() * bs
        total["val_rollout_gen_shoulder_mass"] += metrics["rollout_gen_shoulder_mass"].item() * bs
        total["val_rollout_gt_shoulder_mass"] += metrics["rollout_gt_shoulder_mass"].item() * bs
        total["val_rollout_gen_extreme_mass"] += metrics["rollout_gen_extreme_mass"].item() * bs
        total["val_rollout_gt_extreme_mass"] += metrics["rollout_gt_extreme_mass"].item() * bs
        total["val_rollout_gen_q99_step_count"] += metrics["rollout_gen_q99_step_count"].item() * bs
        total["val_rollout_gt_q99_step_count"] += metrics["rollout_gt_q99_step_count"].item() * bs
        total["val_rollout_tail_steps_frac"] += metrics["rollout_tail_steps_frac"].item() * bs
        total_count += bs
    return {k: v / max(total_count, 1) for k, v in total.items()}
...
def main() -> None:
    ...
    parser.add_argument("--rollout_probe_samples", type=int, default=4)
    parser.add_argument("--spectrum_temperature", type=float, default=0.02)
    parser.add_argument("--rollout_spectrum_weight", type=float, default=0.08)
    parser.add_argument("--rollout_tail_count_weight", type=float, default=0.03)
    parser.add_argument("--rollout_localization_weight", type=float, default=0.04)
    parser.add_argument("--rollout_concentration_weight", type=float, default=0.02)
    ...
    objective_config = dict(
        ...
        rollout_probe_samples=args.rollout_probe_samples,
        spectrum_temperature=args.spectrum_temperature,
        rollout_spectrum_weight=args.rollout_spectrum_weight,
        rollout_tail_count_weight=args.rollout_tail_count_weight,
        rollout_localization_weight=args.rollout_localization_weight,
        rollout_concentration_weight=args.rollout_concentration_weight,
    )
    ...
    print(f"\n{'=' * 76}")
    print("201c: Transformer AR Student-t with rollout localization/spectrum training")
    print(f"{'=' * 76}")
    ...
    print(f"  Tail thresholds: q50={q50_threshold:.5f}, q95={q95_threshold:.5f}, q99={q99_threshold:.5f}")
    print(f"  Underfit z-gate: {args.underfit_z_gate:.3f}")
    print(f"  Overwidth weight: {args.overwidth_weight}")
    print(
        f"  Self-fed rollout steps: {args.rollout_steps}, weight={args.rollout_weight}, "
        f"probe_samples={args.rollout_probe_samples}"
    )
    print(
        f"  Rollout losses: spectrum={args.rollout_spectrum_weight}, "
        f"count={args.rollout_tail_count_weight}, "
        f"localization={args.rollout_localization_weight}, "
        f"concentration={args.rollout_concentration_weight}"
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        ep = {
            ...
            "rollout_spectrum_loss": 0.0,
            "rollout_tail_count_loss": 0.0,
            "rollout_localization_loss": 0.0,
            "rollout_concentration_loss": 0.0,
            "rollout_gen_quiet_mass": 0.0,
            "rollout_gt_quiet_mass": 0.0,
            "rollout_gen_shoulder_mass": 0.0,
            "rollout_gt_shoulder_mass": 0.0,
            "rollout_gen_extreme_mass": 0.0,
            "rollout_gt_extreme_mass": 0.0,
            "rollout_gen_q99_step_count": 0.0,
            "rollout_gt_q99_step_count": 0.0,
            "rollout_tail_steps_frac": 0.0,
        }
        nb = 0
            ...
            torch.save(
                {
                    ...
                    "config": {
                        "type": "transformer_rollout_localization_spectrum_student_t_201c",
        ...
        print(
            ...
            f"ro_spec={val_rollout['val_rollout_spectrum_loss']:.4f}  "
            f"ro_loc={val_rollout['val_rollout_localization_loss']:.4f}  "
