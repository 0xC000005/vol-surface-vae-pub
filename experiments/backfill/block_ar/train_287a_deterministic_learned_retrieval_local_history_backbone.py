...
from diffusion.block_ar.deterministic_learned_retrieval_local_history_backbone import (
    DeterministicLearnedRetrievalLocalHistoryBackbone,
...
def build_local_history_future(
    ...
    prev = torch.cat([history_flat[:, -1:].contiguous(), future_flat[:, :-1]], dim=1)
    delta_z = (future_flat - prev) / hist_std
    future_repr = torch.cat([delta_z, future_z], dim=-1)
    return future_repr, future_z
...
def build_windows(
    ...
    train_future_repr, train_future_z = build_local_history_future(train_hist, train_future)
    val_future_repr, val_future_z = build_local_history_future(val_hist, val_future)
    train_future_z_01 = train_future_z.view(train_future_z.shape[0], train_future_z.shape[1], 5, 5)
    val_future_z_01 = val_future_z.view(val_future_z.shape[0], val_future_z.shape[1], 5, 5)
    return {
        "train_hist_norm": train_hist_norm,
        "train_future_repr": train_future_repr,
        "train_future_z_01": train_future_z_01,
        "val_hist_norm": val_hist_norm,
        "val_future_repr": val_future_repr,
        "val_future_z_01": val_future_z_01,
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="287a-v0 learned retrieval in a local-history future coordinate")
    ...
    model = DeterministicLearnedRetrievalLocalHistoryBackbone(cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_metrics["loss"] < best_val:
            ...
            save_checkpoint(
                ...
                library_future_z_01=windows["train_future_z_01"],
                extra=extra,
            )
