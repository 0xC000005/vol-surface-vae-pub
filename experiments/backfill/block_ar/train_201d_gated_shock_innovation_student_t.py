#!/usr/bin/env python
"""
201d: replace the single broad Student-t innovation with a base + localized shock law.

Keep fixed from 201b:
...
Change only:
  - innovation parameterization
  - add a sparse localized shock component on top of the base innovation
"""
...
def build_local_kernel_bank(grid_h: int = 5, grid_w: int = 5, sigma: float = 0.85) -> torch.Tensor:
    ...
    d2 = diffs.pow(2).sum(dim=-1)
    kernels = torch.exp(-0.5 * d2 / max(sigma * sigma, 1e-6))
    kernels = kernels / kernels.norm(dim=-1, keepdim=True).clamp_min(1e-8)
    return kernels


class GatedShockSpatialShapeScaleStudentTDecoder(SpatialShapeScaleStudentTDecoder):
    def __init__(
        self,
        *args,
        shock_scale_floor: float = 5e-4,
        init_gate_prob: float = 0.08,
        init_shock_scale: float = 0.035,
        **kwargs,
    ):
        super().__init__(*args, **kwargs)
        self.shock_scale_floor = shock_scale_floor

        d_model = self.mean_head.in_features
        self.gate_head = nn.Linear(d_model, 1)
        self.anchor_head = nn.Linear(d_model, 1)
        self.shock_scale_head = nn.Linear(d_model, 1)

        nn.init.zeros_(self.gate_head.weight)
        nn.init.constant_(self.gate_head.bias, float(np.log(init_gate_prob / (1.0 - init_gate_prob))))

        nn.init.zeros_(self.anchor_head.weight)
        nn.init.zeros_(self.anchor_head.bias)

        nn.init.zeros_(self.shock_scale_head.weight)
        nn.init.constant_(
            self.shock_scale_head.bias,
            inverse_softplus(max(init_shock_scale - self.shock_scale_floor, 1e-6)),
        )
    ...
    def forward(
        ...
        anchor_logits = self.anchor_head(h).squeeze(-1)
        shock_scale = F.softplus(self.shock_scale_head(pooled).squeeze(-1)) + self.shock_scale_floor
        ...
        return mu, factor, diag, scale, nu, gate_logit, anchor_logits, shock_scale


class TransformerGatedShockInnovationARModel(TransformerRolloutTailStudentTARModel):
    def __init__(
        self,
        ...
        base_decoder_config = {k: v for k, v in decoder_config.items() if k not in shock_keys}
        super().__init__(
        ...
        self.decoder = GatedShockSpatialShapeScaleStudentTDecoder(
            **base_decoder_config,
            shock_scale_floor=decoder_config.get("shock_scale_floor", 5e-4),
            init_gate_prob=decoder_config.get("init_gate_prob", 0.08),
            init_shock_scale=decoder_config.get("init_shock_scale", 0.035),
        )
        self.decoder_config = decoder_config

        sigma = float(decoder_config.get("shock_kernel_sigma", 0.85))
        kernel_bank = build_local_kernel_bank(5, 5, sigma=sigma)
        kernel_outer = kernel_bank.unsqueeze(-1) * kernel_bank.unsqueeze(-2)
        self.register_buffer("shock_kernel_bank", kernel_bank, persistent=False)
        self.register_buffer("shock_kernel_outer", kernel_outer, persistent=False)

        self.shock_fixed_nu = float(decoder_config.get("shock_fixed_nu", 4.0))
        self.gate_temperature = float(decoder_config.get("gate_temperature", 0.5))
        self.anchor_temperature = float(decoder_config.get("anchor_temperature", 0.6))
        self.gate_prob_eps = float(decoder_config.get("gate_prob_eps", 1e-4))
        self._cached_shock: dict[str, torch.Tensor] | None = None
    ...
    def forward_from_history(
        ...
        mu, factor, diag, scale, nu, gate_logit, anchor_logits, shock_scale = self.decoder(cond, prev_u)
        self._cached_shock = {
    ...
    def last_shock_params(self) -> dict[str, torch.Tensor]:
        if self._cached_shock is None:
            raise RuntimeError("Shock params requested before forward_from_history")
        return self._cached_shock

    def shock_statistics_from_history(self, history_01: torch.Tensor) -> dict[str, torch.Tensor]:
        _ = self.forward_from_history(history_01)
        shock = self.last_shock_params()
        gate_prob = torch.sigmoid(shock["gate_logit"]).clamp(self.gate_prob_eps, 1.0 - self.gate_prob_eps)
        anchor_probs = torch.softmax(shock["anchor_logits"], dim=-1)
        anchor_entropy = -(anchor_probs * torch.log(anchor_probs.clamp_min(1e-8))).sum(dim=-1)
        return {
            "gate_prob_mean": gate_prob.mean(),
            "gate_prob_min": gate_prob.min(),
            "gate_prob_max": gate_prob.max(),
            "anchor_top1_mean": anchor_probs.max(dim=-1).values.mean(),
            "anchor_entropy_mean": anchor_entropy.mean(),
            "shock_scale_mean": shock["shock_scale"].mean(),
        }
    ...
    def _student_t_log_prob_components(
        ...
        cov_components: torch.Tensor,
        nu_components: torch.Tensor,
    ) -> torch.Tensor:
        batch_size, n_components, d = mu_components.shape
        chol = torch.linalg.cholesky(cov_components.reshape(batch_size * n_components, d, d))
    ...
    def student_t_nll(
        ...
        shock = self.last_shock_params()
        base_cov = self.covariance(factor, diag, scale)
        base_logp = self._student_t_log_prob(target_u, mu, base_cov, nu)

        gate_prob = torch.sigmoid(shock["gate_logit"]).clamp(self.gate_prob_eps, 1.0 - self.gate_prob_eps)
        anchor_probs = torch.softmax(shock["anchor_logits"], dim=-1).clamp_min(1e-8)
        shock_var = shock["shock_scale"].pow(2)

        shock_cov = base_cov[:, None, :, :] + shock_var[:, None, None, None] * self.shock_kernel_outer.unsqueeze(0)
        shock_mu = mu[:, None, :].expand(-1, self.shock_kernel_bank.shape[0], -1)
        shock_nu = target_u.new_full((target_u.shape[0], self.shock_kernel_bank.shape[0]), self.shock_fixed_nu)
        shock_logp = self._student_t_log_prob_components(target_u, shock_mu, shock_cov, shock_nu)

        base_logw = torch.log1p(-gate_prob).unsqueeze(-1)
        shock_logw = torch.log(gate_prob).unsqueeze(-1) + torch.log(anchor_probs)
        total_logp = torch.logsumexp(torch.cat([base_logw + base_logp.unsqueeze(-1), shock_logw + shock_logp], dim=-1), dim=-1)
        return -total_logp
    ...
    def _sample_scalar_student_t(
        self,
        scale: torch.Tensor,
        n_samples: int | None = None,
    ) -> torch.Tensor:
        nu = self.shock_fixed_nu
        alpha = scale.new_full(scale.shape, nu / 2.0)
        beta = scale.new_full(scale.shape, nu / 2.0)

        if n_samples is None:
            eps = torch.randn_like(scale)
            mix = torch.distributions.Gamma(alpha, beta).rsample().clamp_min(1e-6)
            return scale * eps * torch.rsqrt(mix)

        eps = torch.randn(scale.shape[0], n_samples, device=scale.device, dtype=scale.dtype)
        mix = torch.distributions.Gamma(alpha, beta).rsample((n_samples,)).transpose(0, 1).clamp_min(1e-6)
        return scale.unsqueeze(1) * eps * torch.rsqrt(mix)
    ...
    def reparameterized_next_u(
        ...
        gate = RelaxedBernoulli(
            temperature=gate_prob.new_tensor(self.gate_temperature),
            probs=gate_prob,
        ).rsample()
        anchor_weights = F.gumbel_softmax(shock["anchor_logits"], tau=self.anchor_temperature, hard=False)
        kernel = anchor_weights @ self.shock_kernel_bank.to(dtype=mu.dtype)
        shock_amp = self._sample_scalar_student_t(shock["shock_scale"], n_samples=None)
        return base_sample + gate.unsqueeze(-1) * shock_amp.unsqueeze(-1) * kernel
    ...
    def sample_next_u(self, history_01: torch.Tensor, n_samples: int) -> torch.Tensor:
        ...
        gate_prob = torch.sigmoid(shock["gate_logit"]).clamp(self.gate_prob_eps, 1.0 - self.gate_prob_eps)
        gate = (torch.rand(mu.shape[0], n_samples, device=mu.device, dtype=mu.dtype) < gate_prob.unsqueeze(1)).to(mu.dtype)
        anchor_probs = torch.softmax(shock["anchor_logits"], dim=-1)
        anchor_idx = torch.multinomial(anchor_probs, num_samples=n_samples, replacement=True)
        kernel = self.shock_kernel_bank[anchor_idx]
        shock_amp = self._sample_scalar_student_t(shock["shock_scale"], n_samples=n_samples)
        return base_samples + gate.unsqueeze(-1) * shock_amp.unsqueeze(-1) * kernel


@torch.no_grad()
def evaluate_shock_statistics(
    model: TransformerGatedShockInnovationARModel,
    val_loader: DataLoader,
) -> dict[str, float]:
    model.eval()
    totals = {
        "val_gate_prob_mean": 0.0,
        "val_gate_prob_min": 0.0,
        "val_gate_prob_max": 0.0,
        "val_anchor_top1_mean": 0.0,
        "val_anchor_entropy_mean": 0.0,
        "val_shock_scale_mean": 0.0,
    }
    total_count = 0
    for history_01, _future_01 in val_loader:
        stats = model.shock_statistics_from_history(history_01)
        bs = history_01.shape[0]
        totals["val_gate_prob_mean"] += stats["gate_prob_mean"].item() * bs
        totals["val_gate_prob_min"] += stats["gate_prob_min"].item() * bs
        totals["val_gate_prob_max"] += stats["gate_prob_max"].item() * bs
        totals["val_anchor_top1_mean"] += stats["anchor_top1_mean"].item() * bs
        totals["val_anchor_entropy_mean"] += stats["anchor_entropy_mean"].item() * bs
        totals["val_shock_scale_mean"] += stats["shock_scale_mean"].item() * bs
        total_count += bs
    return {k: v / max(total_count, 1) for k, v in totals.items()}


def main() -> None:
    parser = argparse.ArgumentParser(description="201d gated-shock innovation Student-t")
    ...
    parser.add_argument("--shock_fixed_nu", type=float, default=4.0)
    parser.add_argument("--shock_scale_floor", type=float, default=5e-4)
    parser.add_argument("--init_gate_prob", type=float, default=0.08)
    parser.add_argument("--init_shock_scale", type=float, default=0.035)
    parser.add_argument("--shock_kernel_sigma", type=float, default=0.85)
    parser.add_argument("--gate_temperature", type=float, default=0.5)
    parser.add_argument("--anchor_temperature", type=float, default=0.6)
    ...
    decoder_config = dict(
        ...
        shock_fixed_nu=args.shock_fixed_nu,
        shock_scale_floor=args.shock_scale_floor,
        init_gate_prob=args.init_gate_prob,
        init_shock_scale=args.init_shock_scale,
        shock_kernel_sigma=args.shock_kernel_sigma,
    ...
    print(f"\n{'=' * 76}")
    print("201d: Transformer AR Student-t with gated localized shock innovation")
    print(f"{'=' * 76}")
    ...
    print(f"  Shock nu: {args.shock_fixed_nu:.2f} | init gate prob: {args.init_gate_prob:.3f} | init shock scale: {args.init_shock_scale:.4f}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        shock_diag = evaluate_shock_statistics(model, val_loader)
            ...
            torch.save(
                {
                    ...
                    "config": {
                        "type": "transformer_gated_shock_innovation_student_t_201d",
        ...
        print(
            ...
            f"gate={shock_diag['val_gate_prob_mean']:.3f}  "
            f"anchor_top1={shock_diag['val_anchor_top1_mean']:.3f}  "
            f"shock_scale={shock_diag['val_shock_scale_mean']:.4f}  "
