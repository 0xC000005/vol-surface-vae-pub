#!/usr/bin/env python
"""
212i: H=1 minimal deterministic direct delta model with per-window MSE + CVaR.
"""
...
class DeterministicDirectDeltaMSECVaRModel(nn.Module):
    def __init__(
        self,
        n_cells: int = 25,
    ...
    def training_loss(
        ...
        cvar_mse = empirical_cvar(per_window_mse, alpha=cvar_alpha)
        loss = mean_mse + cvar_lambda * cvar_mse
        metrics = {
            "mean_mse": mean_mse.detach(),
            "cvar_mse": cvar_mse.detach(),
            "loss": loss.detach(),
...
@torch.no_grad()
def evaluate_point(
    model: DeterministicDirectDeltaMSECVaRModel,
    loader: DataLoader,
    q95_threshold: float,
    q99_threshold: float,
    cvar_alpha: float,
    ...
    for history_01, target_01 in loader:
        ...
        sign_acc = ((pred_delta >= 0) == (target_delta >= 0)).float().mean()
        ...
        total["val_cvar_mse"] += float(cvar_mse.item()) * batch_size
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[DeterministicDirectDeltaMSECVaRModel, dict[str, Any]]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = payload["config"]
    if cfg["type"] != "minimal_h1_deterministic_direct_delta_212i":
...
def main() -> None:
    parser = argparse.ArgumentParser(description="212i H=1 deterministic direct delta + MSE/CVaR")
    ...
    print("212i H=1 deterministic direct delta + MSE/CVaR")
    ...
    print(
        f"  max_delta_factor={args.max_delta_factor:.2f} "
        f"cvar_alpha={args.cvar_alpha:.2f} cvar_lambda={args.cvar_lambda:.2f}"
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for history_01, target_01 in train_loader:
            ...
            running["cvar_mse"] += float(metrics["cvar_mse"].item()) * batch
        ...
        payload = {
            ...
            "epoch": epoch,
            "config": {
                "type": "minimal_h1_deterministic_direct_delta_212i",
                "n_cells": 25,
                "history_feat_dim": 50,
                "hidden_dim": args.hidden_dim,
                "gru_layers": args.gru_layers,
        ...
        print(
            ...
            f"CVaR={train_metrics['train_cvar_mse']:.4f}  "
