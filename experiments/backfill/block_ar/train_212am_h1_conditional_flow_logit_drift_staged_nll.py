#!/usr/bin/env python
"""
212am: H=1 support-by-construction conditional flow with drift in logit-IV space.

This branch moves the conditional law into a bounded-support-aware coordinate:

  y_t = logit(iv_t)

and models the one-day move as

  y_{t+1} = y_t + m(h_t) + s_t * sinh(v_t)

where:
  - m(h_t) is a learned conditional drift / center path
  - s_t is a causal EWMA local scale computed in logit-delta space
  - v_t is a conditional flow residual with exact likelihood

Samples are mapped back by:

  iv_{t+1} = sigmoid(y_{t+1})

so support is respected by construction rather than via post-hoc clipping.
Training keeps the population-level energy score on raw deltas while adding a
staged exact likelihood term on the residual law in logit space.
"""
...
def safe_logit(x: torch.Tensor, eps: float) -> torch.Tensor:
    x = x.clamp(min=eps, max=1.0 - eps)
    return torch.log(x) - torch.log1p(-x)


def build_logit_local_scale_history_features(
    ...
    levels_01 = history_01.reshape(batch, hist_len, -1)
    levels_y = safe_logit(levels_01, eps=support_eps)

    delta_obs = levels_y[:, 1:] - levels_y[:, :-1]
    ...
    deltas = torch.zeros_like(levels_y)
    deltas[:, 1:] = delta_obs
    delta_std = deltas / scale_seq

    feat_parts = [levels_y, delta_std]
...
class ConditionalFlowLogitDriftModel(nn.Module):
    def __init__(
        self,
        n_cells: int = 25,
        ...
        self.mean_head = nn.Linear(hidden_dim, n_cells)
        masks = []
    ...
    def _prev_01(self, history_01: torch.Tensor) -> torch.Tensor:
        return history_01[:, -1].reshape(history_01.shape[0], self.n_cells)

    def _prev_y(self, history_01: torch.Tensor) -> torch.Tensor:
        return safe_logit(self._prev_01(history_01), eps=self.support_eps)
    ...
    def encode_with_scale(self, history_01: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        feat, local_scale = build_logit_local_scale_history_features(
    ...
    def sample_transformed_residual(
        ...
        return v, local_scale, prev_y, mean_dy
    ...
    def sample_next_logit(
        ...
        v, local_scale, prev_y, mean_dy = self.sample_transformed_residual(
            history_01,
            n_samples=n_samples,
            noise=noise,
        )
        residual_dy = torch.sinh(v) * local_scale.unsqueeze(1)
    ...
    def sample_next_iv(
        ...
        return torch.sigmoid(self.sample_next_logit(history_01, n_samples=n_samples, noise=noise))
    ...
    def sample_delta(
        ...
        prev = self._prev_01(history_01).unsqueeze(1)
        return self.sample_next_iv(history_01, n_samples=n_samples, noise=noise) - prev
    ...
    def training_loss(
        ...
        prev_y = self._prev_y(history_01)
        target_y = safe_logit(target_01.reshape(history_01.shape[0], self.n_cells), eps=self.support_eps)
        target_delta = target_01.reshape(history_01.shape[0], self.n_cells) - prev_01

        v_samples, local_scale, _prev_y, mean_dy = self.sample_transformed_residual(
            history_01,
            n_samples=n_samples,
        )
        residual_dy = torch.sinh(v_samples) * local_scale.unsqueeze(1)
        ...
        target_v = torch.asinh((target_y - prev_y - mean_dy) / local_scale.clamp_min(self.scale_floor))
...
) -> tuple[ConditionalFlowLogitDriftModel, dict[str, Any]]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = payload["config"]
    if cfg["type"] != "minimal_h1_conditional_flow_212am_logit_drift_staged_nll":
        raise ValueError(f"Unexpected model type: {cfg['type']}")
    model = ConditionalFlowLogitDriftModel(
        n_cells=cfg["n_cells"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="212am support-by-construction logit-drift conditional flow")
    ...
    print("212am support-by-construction logit-drift conditional flow")
    print(f"  train_windows={train_hist.shape[0]} val_windows={val_hist.shape[0]}")
    print(f"  train_samples={args.train_samples} lambda_nll_max={args.lambda_nll_max}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        payload = {
            ...
            "epoch": epoch,
            "config": {
                "type": "minimal_h1_conditional_flow_212am_logit_drift_staged_nll",
                "n_cells": 25,
                "history_feat_dim": history_feat_dim,
                "hidden_dim": args.hidden_dim,
                "gru_layers": args.gru_layers,
