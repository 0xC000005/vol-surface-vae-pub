#!/usr/bin/env python
"""
Focused mechanistic review of 188b_v0.

Questions:
  1. Did the 188b amplitude gate actually remove prior event effect when decoded
     event amplitude/support is effectively zero?
  2. Do learned prior/posterior marked-event objects overlap the hard S3/S7
     late-turbulent slices?
  3. Is the branch now being bypassed cleanly, or still perturbing broad behavior?
"""
...
def build_model(ckpt_path: Path, device: torch.device) -> tuple[AmplitudeGatedMarkedEventResidualModel, dict[str, Any]]:
...
def decode_anchor_field(model: AmplitudeGatedMarkedEventResidualModel, slot_dec: dict[str, torch.Tensor]) -> torch.Tensor:
...
def teacher_slot_dec(
    model: AmplitudeGatedMarkedEventResidualModel,
    teacher_slots: dict[str, torch.Tensor],
    dtype: torch.dtype,
...
def load_history_row(ckpt_path: Path, epoch: int) -> dict[str, Any]:
    ...
    for row in rows:
        if int(row.get("epoch", -1)) == int(epoch):
            keep = [
                ...
                "stage",
                "val_total_loss",
                "val_prior_slot_active_mean",
                "val_prior_slot_amp_abs_mean",
                "val_prior_slot_node_top1_mean",
                "val_post_slot_amp_abs_mean",
                "val_raw_event_velocity_norm_mean",
                "val_event_velocity_norm_mean",
...
def marked_event_state_analysis(
    ...
    post_event_band_abs = []
    prior_raw_event_norm = []
    post_raw_event_norm = []
    prior_gated_event_norm = []
    post_gated_event_norm = []
    ...
    for start in range(0, history_norm.shape[0], batch_size):
        ...
        (
            ...
            _local_gate,
            _band_gate,
        ) = super(AmplitudeGatedMarkedEventResidualModel, model).build_state_metric_controls(z_t, t, path_context)
        ...
        prior_event_local, prior_event_band = model.decode_event_field(prior_dec)
        ...
        event_raw = model.event_transport(z_t, t, path_context)
        event_raw_basis = event_raw.view(z_t.shape[0], model.decoder.n_frames, model.decoder.n_cells)
        prior_gated_basis = model.event_white_gate(prior_event_local, prior_event_band, event_raw_basis)
        post_gated_basis = model.event_white_gate(post_event_local, post_event_band, event_raw_basis)
        ...
        prior_raw_event_norm.append(event_raw_basis.reshape(z_t.shape[0], -1).norm(dim=-1).detach().cpu().numpy())
        post_raw_event_norm.append(event_raw_basis.reshape(z_t.shape[0], -1).norm(dim=-1).detach().cpu().numpy())
        prior_gated_event_norm.append(prior_gated_basis.reshape(z_t.shape[0], -1).norm(dim=-1).detach().cpu().numpy())
        post_gated_event_norm.append(post_gated_basis.reshape(z_t.shape[0], -1).norm(dim=-1).detach().cpu().numpy())
    ...
    return {
        ...
        "prior_raw_event_norm": np.concatenate(prior_raw_event_norm, axis=0),
        "post_raw_event_norm": np.concatenate(post_raw_event_norm, axis=0),
        "prior_gated_event_norm": np.concatenate(prior_gated_event_norm, axis=0),
        "post_gated_event_norm": np.concatenate(post_gated_event_norm, axis=0),
    }
    ...
    out.update(
        {
            ...
            "prior_gated_event_norm_hard_windows": safe_mean(stats["prior_gated_event_norm"][hard_windows]),
            "prior_gated_event_norm_clean_turb_windows": safe_mean(stats["prior_gated_event_norm"][clean_turb_windows]),
            "post_gated_event_norm_hard_windows": safe_mean(stats["post_gated_event_norm"][hard_windows]),
            "post_gated_event_norm_clean_turb_windows": safe_mean(stats["post_gated_event_norm"][clean_turb_windows]),
            "prior_raw_event_norm_all_windows": safe_mean(stats["prior_raw_event_norm"]),
            "prior_gated_event_norm_all_windows": safe_mean(stats["prior_gated_event_norm"]),
            "post_raw_event_norm_all_windows": safe_mean(stats["post_raw_event_norm"]),
            "post_gated_event_norm_all_windows": safe_mean(stats["post_gated_event_norm"]),
            "prior_gate_efficiency_ratio": safe_share(
                safe_mean(stats["prior_gated_event_norm"]),
                safe_mean(stats["prior_raw_event_norm"]),
            ),
            "post_gate_efficiency_ratio": safe_share(
                safe_mean(stats["post_gated_event_norm"]),
                safe_mean(stats["post_raw_event_norm"]),
            ),
        }
    )
    return out
...
def build_event_off_model(model: AmplitudeGatedMarkedEventResidualModel) -> AmplitudeGatedMarkedEventResidualModel:
...
def review_checkpoint(
    ...
    return {
        ...
        "history_row": load_history_row(checkpoint_path, int(payload.get("epoch", -1))),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Focused mechanistic review of 188b amplitude-gated marked-event residual model")
    parser.add_argument(
        "--best_ckpt",
        default=(
            "models/backfill/"
            "amplitude_gated_marked_event_residual_pathwise_residual_law_mean_reverting_covariance_mixture_"
            "structured_joint_student_t_188b/best_model.pt"
        ),
    )
    parser.add_argument(
        "--final_ckpt",
        default=(
            "models/backfill/"
            "amplitude_gated_marked_event_residual_pathwise_residual_law_mean_reverting_covariance_mixture_"
            "structured_joint_student_t_188b/final_model.pt"
        ),
    )
    ...
    parser.add_argument(
        "--output_dir",
        default="results/validations/2026-04-06/analysis/188b_amplitude_gated_marked_event_mechanistic",
    )
