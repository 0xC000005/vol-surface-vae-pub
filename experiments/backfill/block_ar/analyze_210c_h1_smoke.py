#!/usr/bin/env python
"""
Fixed H=1 smoke evaluation for 210c.
"""
...
from experiments.backfill.block_ar.train_210c_h1_mixture_density_transformer import load_model


def main() -> None:
    parser = argparse.ArgumentParser(description="210c H=1 smoke eval")
    ...
    for start in range(0, history_01.shape[0], args.batch_size):
        ...
        stats = model.mixture_statistics(batch_hist)
        samples.append(samp.detach().cpu().numpy())
        mix_top1.append(float(stats["mix_top1_mean"].item()))
        mix_entropy.append(float(stats["mix_entropy_mean"].item()))
        mix_active_terms.append(stats["mix_marginal_probs"].detach().cpu().numpy())
    ...
    marginal = np.mean(np.stack(mix_active_terms, axis=0), axis=0)
    ...
    out = {
        ...
        "mixture_stats": {
            "mix_top1_mean": float(np.mean(mix_top1)),
            "mix_entropy_mean": float(np.mean(mix_entropy)),
            "mix_active_components": active,
            "mix_marginal_probs": marginal.tolist(),
        },
