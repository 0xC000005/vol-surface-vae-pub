#!/usr/bin/env python
"""
212j: 212i with an LSTM encoder instead of GRU.
"""
...
class DeterministicDirectDeltaLSTMMSECVaRModel(nn.Module):
    def __init__(
        self,
        n_cells: int = 25,
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[DeterministicDirectDeltaLSTMMSECVaRModel, dict[str, Any]]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = payload["config"]
    if cfg["type"] != "minimal_h1_deterministic_direct_delta_212j":
        raise ValueError(f"Unexpected model type: {cfg['type']}")
    model = DeterministicDirectDeltaLSTMMSECVaRModel(
        n_cells=cfg["n_cells"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="212j H=1 deterministic direct delta + LSTM + MSE/CVaR")
    ...
    print("212j H=1 deterministic direct delta + LSTM + MSE/CVaR")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        payload = {
            ...
            "epoch": epoch,
            "config": {
                "type": "minimal_h1_deterministic_direct_delta_212j",
                "n_cells": 25,
                "history_feat_dim": 50,
                "hidden_dim": args.hidden_dim,
                "lstm_layers": args.lstm_layers,
