#!/usr/bin/env python
"""
163a: No-LN CLN + VS (RC19-H2a+H3 Additivity Test)

Combines no-LN architecture with Variogram Score. Tests whether the
mechanisms are additive or redundant.

VS alone (161a) solves conditionality (turb/calm=1.462).
No-LN alone (159a/159b) provides transient signal.
Are they additive? If yes → compounding. If no → VS is sufficient.
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"163a: No-LN CLN + VS (RC19-H2a+H3 Additivity)")
    print(f"{'='*60}")
