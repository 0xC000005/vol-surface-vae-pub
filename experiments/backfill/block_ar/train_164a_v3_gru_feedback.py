#!/usr/bin/env python
"""
164a_v3: AR + No-LN Spatial Transformer + VS + GRU Feedback During Training (RC20)

Changes from 164a_v2:
  - GRU feedback during TRAINING (eliminates train-test mismatch)
  - At each AR step, detached generated frame is fed through GRU
  - Condition is recomputed from updated GRU hidden states
  - Model learns to use evolving condition (temporal awareness)

164a_v2 finding: model learned correct conditionality (turb/calm=1.24) but
GRU feedback during inference compressed ratio to 1.16. Train-test mismatch
because training used fixed condition. This version trains with feedback.

Architecture:
  - Encoder: GRU + attention pool, RANDOM INIT, E2E
  - Decoder: Spatial transformer (25 cells/frame), no-LN, tanh, LayerScale 0.1
  - Generation: AR frame-by-frame with GRU feedback at EACH step
  - Condition: recomputed per step from GRU attention pool
  - Loss: afCRPS + VS (lambda=0.5), per-frame at D=25
  - NO reflecting boundary, NO clamp, NO output scaling

Usage:
    PYTHONPATH=. python experiments/backfill/block_ar/train_164a_v3_gru_feedback.py \
        --epochs 80 --batch_size 32 --n_members 8 --noise_dim 32 \
        --lambda_vs 0.5 --lambda_is 0.5 \
        --output_dir models/backfill/afcrps_164a_v3 --device cuda
"""
...
class ConditionalNorm(nn.Module):
    ...
    def forward(self, x, z):
        ...
        scale = self.scale_proj(z).unsqueeze(1)  # (B, 1, d_model)
        bias = self.bias_proj(z).unsqueeze(1)
    ...
    def __init__(self, n_cells=25, d_model=128, n_heads=4, n_layers=4,
        ...
        for _ in range(n_layers):
            self.layers.append(nn.ModuleDict({
                'cln': ConditionalNorm(d_model, noise_dim),
                'attn': nn.MultiheadAttention(d_model, n_heads, batch_first=True),
                'ff': nn.Sequential(
                    nn.Linear(d_model, d_model * 4), nn.GELU(),
                    nn.Linear(d_model * 4, d_model),
                ),
                'ff_cln': ConditionalNorm(d_model, noise_dim),
            }))
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"164a_v3: AR + No-LN Spatial Transformer + VS + GRU Feedback (RC20)")
    print(f"{'='*60}")
    ...
    print(f"  Boundary: NONE | GRU feedback: DURING TRAINING (per-step condition update)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for hist, gt_frames, last_frame in train_loader:
            ...
            # GRU state for feedback (detached — no BPTT through AR chain)
            # Expand h_last for K members
            ...
            # AR loop with GRU feedback: condition recomputed per step
            optimizer.zero_grad()
            ...
            for t in range(T):
                ...
                # Recompute condition from GRU state (detached GRU, gradients through bottleneck only)
                if t > 0:
                    ...
                    cond_K = model.encoder.bottleneck(h_pooled_t)  # (B*K, 128)
                ...
                prev = frame_t.detach()
                with torch.no_grad():
                    frame_norm = normalize_iv(prev).unsqueeze(1)  # (B*K, 1, 25) in [-1,1]
        ...
        # ── Save best model ──
        config_dict = {
            "type": "ar_spatial_transformer_164a_v3",
            "encoder": {
