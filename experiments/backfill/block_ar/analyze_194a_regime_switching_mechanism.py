#!/usr/bin/env python
"""
Focused mechanistic review of 194a regime-switching AR latent-factor regression.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Mechanistic review of 194a regime-switching AR model")
    parser.add_argument("--best_ckpt", type=str, default="models/backfill/regime_switching_ar_latent_factor_194a/best_model.pt")
    parser.add_argument("--final_ckpt", type=str, default="models/backfill/regime_switching_ar_latent_factor_194a/final_model.pt")
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="results/validations/2026-04-07/analysis/194a_regime_switching_mechanistic",
    )
    ...
    models = {
        "194a_best": (best_model, best_payload),
        "194a_final": (final_model, final_payload),
    }
    ...
    conclusion = {
        ...
        "regime_separation_read": (
            "194a uses the discrete states meaningfully: calm windows shift toward the quiet state, "
        ...
        "quiet_state_read": (
            "If the quiet state's calm-h30 variance stays above 169c on clean points, then 194a is still too wide "
