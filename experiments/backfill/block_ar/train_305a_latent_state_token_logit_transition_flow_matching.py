...
from diffusion.block_ar.latent_state_token_logit_transition_flow_matching import (
    LatentStateJointTokenLogitTransitionFMConfig,
    LatentStateJointTokenLogitTransitionFlowMatching,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="305a-v0 latent state-aware joint token logit-transition flow")
    ...
    model = LatentStateJointTokenLogitTransitionFlowMatching(cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "val_path_loss": val_avg["path_loss"],
            "val_latent_loss": val_avg["latent_loss"],
            "val_transition_std": val_avg["transition_std"],
            "val_transition_abs": val_avg["transition_abs"],
            "val_state_logit_abs": val_avg["state_logit_abs"],
        ...
        print(
            ...
            f"path={rec['val_path_loss']:.5f} latent={rec['val_latent_loss']:.5f} "
            f"state_abs={rec['val_state_logit_abs']:.3f} lr={rec['lr']:.2e} time={rec['sec']:.1f}s"
        )
