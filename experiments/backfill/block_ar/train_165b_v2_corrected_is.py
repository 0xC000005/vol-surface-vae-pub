#!/usr/bin/env python
"""
165b_v2: Corrected IS + Ensemble Mean MSE (RC21 H1v5)

Two loss-function fixes on the baseline softplus model, no architecture change:

1. IS lambda corrected: 0.05 → 0.005 (was 42% of gradient, now ~16%)
   Gradient decomposition revealed IS was 10x too strong, starving CRPS.

2. Ensemble mean MSE added: lambda_center=0.55 (9% gradient budget)
   Direct centering gradient on decoder output. CRPS allocates only 13% to
   centering internally; this supplements it.

Gradient budget: CRPS 75%, IS 16%, Center 9%.
Previous: CRPS 47%, IS 42%, VS 11% (IS dominated, centering starved).

Usage:
    PYTHONPATH=. python experiments/backfill/block_ar/train_165b_v2_corrected_is.py \
        --epochs 80 --batch_size 16 --n_members 16 --noise_dim 32 \
        --lambda_vs 0.5 --lambda_is 0.005 --is_warmup_epochs 10 --bptt_steps 5 \
        --lambda_floor 2.5 --floor_tau 0.005 --floor_warmup_epochs 10 \
        --lambda_center 0.55 --center_warmup_epochs 5 \
        --output_dir models/backfill/afcrps_165b_v2 --device cuda
"""
...
def main():
    ...
    parser.add_argument("--lambda_center", type=float, default=0.55,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # ── Save best model ──
        config_dict = {
            "type": "ar_spatial_transformer_165b_v2_corrected_is",
            "encoder": {
