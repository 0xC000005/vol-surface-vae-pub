#!/usr/bin/env python
"""
165a_v3: Spatial Transformer Mean Head (RC21 H1v3)
...
# ---- Mean Head (RC21 H1v3 — Spatial Transformer) ----

class MeanHead(nn.Module):
    """Spatial transformer mean head: per-cell prediction with cross-cell attention.

    Each cell becomes a token. Condition is broadcast-added. 2-layer transformer
    lets cells share information while specializing. Zero-init output.

    v2 MLP: 153→128→128→25 (all cells from shared bottleneck, no spatial interaction)
    v3 spatial: 25 tokens × d_model, 2 attention layers (cells learn from neighbors)
    """

    def __init__(self, cond_dim=128, n_cells=25, hidden_dim=64, n_heads=4, n_layers=2):
        super().__init__()
        self.n_cells = n_cells
        self.d_model = hidden_dim

        # Per-cell value → token
        self.input_proj = nn.Linear(1, hidden_dim)

        # Condition → broadcast to all cells
        self.cond_proj = nn.Sequential(
            nn.Linear(cond_dim, hidden_dim), nn.SiLU(), nn.Linear(hidden_dim, hidden_dim),
        )

        # Spatial positional encoding (25 learned positions)
        self.spatial_pos = nn.Parameter(torch.randn(1, n_cells, hidden_dim) * 0.02)

        # Transformer layers (spatial attention over cells)
        self.layers = nn.ModuleList()
        for _ in range(n_layers):
            self.layers.append(nn.ModuleDict({
        ...
        # Output: per-cell delta (zero-init)
    ...
    def forward(self, cond, prev_mean):
        ...
        """
        h = self.input_proj(prev_mean.unsqueeze(-1))  # (B, 25, d_model)
        h = h + self.cond_proj(cond).unsqueeze(1)  # broadcast condition
        h = h + self.spatial_pos

        for layer in self.layers:
            h2 = layer['ln1'](h)
            attn_out, _ = layer['attn'](h2, h2, h2)
            h = h + attn_out
            h = h + layer['ff'](layer['ln2'](h))

        return self.output_proj(h).squeeze(-1)  # (B, 25)
...
def main():
    ...
    config_dict = {
        "type": "ar_spatial_transformer_165a_v3_spatial_mean",
        "encoder": {
