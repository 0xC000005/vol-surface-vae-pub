#!/usr/bin/env python
"""
185a_v0: Graph/group latent event-path residual law on top of the 183c pathwise residual-law backbone.
...
Principle:
  ...
  - add a separate latent event-path contribution in whitened geometry-aware space
  - infer event activity from a prior over context and a posterior over target residual structure
  - decode sparse event support and amplitudes, then propagate them on the geometry graph
...
class GraphGroupLatentEventPathModel(StateMetricTransportModel):
    def __init__(
        self,
...
def latent_event_path_flow_matching_loss(
    ...
    _, _, quiet_metric_local, quiet_metric_band, _, _ = super(
        GraphGroupLatentEventPathModel, model
...
def evaluate_teacher_forced(
    ...
    for history_01, future_01 in val_loader:
        ...
        _loss, metrics = latent_event_path_flow_matching_loss(model, history_01, future_01)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="185a_v0: graph/group latent event-path residual law")
    ...
    parser.add_argument("--init_event_amp", type=float, default=0.10)
    ...
    parser.add_argument("--init_event_scale", type=float, default=0.35)
    ...
    parser.add_argument("--event_amp_reg_weight", type=float, default=0.01)
    ...
    def set_stage(stage: int):
        ...
        if stage == 1:
            ...
            stage_name = "latent-event-path-ctrl"
        else:
            ...
            stage_name = "latent-event-path-joint"
    ...
    print(f"\n{'=' * 72}\n185a_v0: graph/group latent event-path residual law\n{'=' * 72}")
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for history_01, future_01 in train_loader:
            ...
            loss, metrics = latent_event_path_flow_matching_loss(model, history_01, future_01)
            ...
            torch.save(
                {
                    ...
                    "config": {
                        "type": "graph_group_latent_event_path_residual_law_mean_reverting_covariance_mixture_structured_joint_student_t_185a",
