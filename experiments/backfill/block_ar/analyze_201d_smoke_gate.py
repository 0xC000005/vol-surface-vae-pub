#!/usr/bin/env python
"""
Fixed smoke-gate evaluation for 201d.
"""
...
from experiments.backfill.block_ar.train_201d_gated_shock_innovation_student_t import (
    TransformerGatedShockInnovationARModel,
)
...
def load_model(checkpoint_path: str, device: torch.device):
    ...
    if raw_config["type"] != "transformer_gated_shock_innovation_student_t_201d":
...
def evaluate_shock_stats(
    ...
    total = {
        ...
        "gate_prob_min": 0.0,
        "gate_prob_max": 0.0,
        "anchor_top1_mean": 0.0,
        "anchor_entropy_mean": 0.0,
        "shock_scale_mean": 0.0,
    }
    count = 0
    for start in range(0, history_01.shape[0], batch_size):
        end = min(start + batch_size, history_01.shape[0])
        stats = model.shock_statistics_from_history(history_01[start:end].to(device))
        bs = end - start
        for k in total:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="201d smoke gate")
    ...
    rollout_diag = evaluate_rollout_subset(
        ...
        torch.utils.data.DataLoader(
            torch.utils.data.TensorDataset(history_01, future_01),
    ...
    shock_stats = evaluate_shock_stats(model, history_01, args.batch_size, device)
    ...
    gate_checks = {
        ...
        "hard_late_top1_ok": targeting.get("rollout", {}).get("top1_matches_true_top1", float("nan")) >= 0.25,
