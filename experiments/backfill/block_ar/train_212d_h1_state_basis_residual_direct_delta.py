#!/usr/bin/env python
"""
212d: H=1 state-basis residual direct delta model.

Minimal follow-up to 212c:
  - keep the explicit mean-delta head
  - replace free residual MLP([state, z]) with state-defined residual basis
  - noise only chooses coefficients inside state-produced directions
"""
...
class StateBasisResidualDirectDeltaModel(nn.Module):
    def __init__(
        self,
        n_cells: int = 25,
        ...
        self.basis_head = nn.Sequential(
            nn.Linear(hidden_dim, basis_hidden),
            nn.SiLU(),
            nn.Linear(basis_hidden, n_cells * basis_rank),
        )
    ...
    def _init_parameters(self) -> None:
        for name, module in self.named_modules():
            if isinstance(module, nn.Linear):
                if name.endswith("mean_head.2") or name.endswith("basis_head.2"):
                    continue
    ...
    def state_basis(self, state: torch.Tensor) -> torch.Tensor:
        basis = self.basis_head(state).reshape(state.shape[0], self.n_cells, self.basis_rank)
        # Keep residual directions at stable scale; state changes orientation, not raw norm explosion.
        basis_norm = torch.linalg.vector_norm(basis, dim=1, keepdim=True).clamp_min(1e-6)
        basis = basis / basis_norm * math.sqrt(self.n_cells)
        return basis
    ...
    def sample_delta(
        ...
        basis = self.state_basis(state)

        if coeffs is None:
            coeffs = torch.randn(batch, n_samples, self.basis_rank, device=history_01.device, dtype=history_01.dtype)
        residual_raw = torch.einsum("bcr,bkr->bkc", basis, coeffs) / math.sqrt(self.basis_rank)
        residual = torch.tanh(residual_raw) * (self.residual_scale_factor * self.delta_scale.view(1, 1, self.n_cells))
    ...
    def sample_next_iv(
        ...
        delta, _mean_delta = self.sample_delta(history_01, n_samples=n_samples, coeffs=coeffs)
...
@torch.no_grad()
def evaluate_h1(
    model: StateBasisResidualDirectDeltaModel,
    loader: DataLoader,
    q95_threshold: float,
    q99_threshold: float,
    eval_samples: int,
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[StateBasisResidualDirectDeltaModel, dict[str, Any]]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = payload["config"]
    if cfg["type"] != "minimal_h1_state_basis_residual_direct_delta_212d":
...
def main() -> None:
    parser = argparse.ArgumentParser(description="212d H=1 state-basis residual direct delta model")
    ...
    parser.add_argument("--basis_hidden", type=int, default=128)
    ...
    print("212d H=1 state-basis residual direct delta")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        payload = {
            ...
            "epoch": epoch,
            "config": {
                "type": "minimal_h1_state_basis_residual_direct_delta_212d",
                "n_cells": 25,
                "history_feat_dim": 50,
                "hidden_dim": args.hidden_dim,
                "gru_layers": args.gru_layers,
