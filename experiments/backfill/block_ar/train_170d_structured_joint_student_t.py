#!/usr/bin/env python
"""
170d: Structured Joint Future Support-Aware Student-t
...
Change:
  - replace flat 750-dim covariance with a structured spatiotemporal covariance
  - Sigma = scale^2 * (Sigma_time kron Sigma_cell)
  - Sigma_time and Sigma_cell are each low-rank + diagonal
"""
...
class StructuredJointStudentTDecoder(nn.Module):
    """Factored temporal-spatial decoder with separable covariance heads."""
    ...
    def forward(
        ...
        time_summary = h.mean(dim=2)
        cell_summary = h.mean(dim=1)
        ...
        return mu, time_factor, time_diag, cell_factor, cell_diag, scale, nu


class StructuredJointStudentTModel(nn.Module):
    """GRU history encoder + separable time/cell Student-t future block decoder."""
    ...
    def __init__(
        ...
        self.decoder = StructuredJointStudentTDecoder(**decoder_config)
    ...
    def covariance_parts(
        ...
        time_factor_norm, time_diag_norm, _ = self.normalized_components(time_factor, time_diag)
        cell_factor_norm, cell_diag_norm, _ = self.normalized_components(cell_factor, cell_diag)
    ...
    def sample_future_u(self, history_01: torch.Tensor, n_samples: int) -> torch.Tensor:
        mu, time_factor, time_diag, cell_factor, cell_diag, scale, nu = self.forward_from_history(history_01)
        B, T, C = mu.shape
        cov_t, cov_c = self.covariance_parts(time_factor, time_diag, cell_factor, cell_diag, scale)
        chol_t = torch.linalg.cholesky(cov_t)
        chol_c = torch.linalg.cholesky(cov_c)

        z = torch.randn(B, n_samples, T, C, device=mu.device, dtype=mu.dtype)
        ...
        t_scale = torch.rsqrt(mix).view(B, n_samples, 1, 1)

        return mu.unsqueeze(1) + noise * t_scale
...
def joint_nll_loss(
    ...
    mu, time_factor, time_diag, cell_factor, cell_diag, scale, nu = model.forward_from_history(history_01)
    nll = model.student_t_nll(target_u, mu, time_factor, time_diag, cell_factor, cell_diag, scale, nu)
...
def main():
    parser = argparse.ArgumentParser(description="170d: structured joint future Student-t")
    ...
    print(f"\n{'=' * 64}")
    print("170d: Structured Joint Future Support-Aware Student-t")
    print(f"{'=' * 64}")
    ...
    print("  Covariance: scale^2 * (Sigma_time kron Sigma_cell)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if is_best:
            ...
            torch.save({
                ...
                "config": {
                    "type": "structured_joint_student_t_170d",
