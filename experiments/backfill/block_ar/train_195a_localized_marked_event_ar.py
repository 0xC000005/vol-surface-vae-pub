#!/usr/bin/env python
"""
195a: Localized marked-event AR latent-factor Student-t model.

Replacement-style follow-up to 194c:
  - keep the same AR HMM latent-factor backbone
  - keep the same quiet/event 2-state regime path
  - replace the event state's coarse broadening with a localized marked-event carrier
  - train the event anchor/kernel against quiet-state underfit so event mass can move
    to the hard late-horizon cells instead of becoming a fixed template

Usage:
    PYTHONPATH=. python experiments/backfill/block_ar/train_195a_localized_marked_event_ar.py \
        --output_dir models/backfill/regime_switching_ar_latent_factor_195a --device cuda
"""
...
class StickyRegimeShapeScaleDecoder(nn.Module):
    """State-conditioned decoder with a localized marked-event carrier in state 1."""
    ...
    def __init__(
        ...
        event_families: tuple[str, ...] = ("delta", "cross1", "square1", "gauss08", "gauss14"),
        event_temperature: float = 0.40,
        event_amp_floor: float = 0.05,
    ):
        super().__init__()
        ...
        self.event_amp_floor = event_amp_floor
        self.event_families = tuple(event_families)
        ...
        self.event_anchor_head = nn.Sequential(
            nn.Linear(cond_dim, cond_dim),
            nn.SiLU(),
            nn.Linear(cond_dim, n_cells),
        )
        self.event_family_head = nn.Sequential(
            nn.Linear(cond_dim, cond_dim),
            nn.SiLU(),
            nn.Linear(cond_dim, len(self.event_families)),
        )
        self.event_amp_head = nn.Sequential(
            nn.Linear(cond_dim, cond_dim),
            nn.SiLU(),
            nn.Linear(cond_dim, 1),
        )

        # Use quiet/event endpoints for the 2-state branch instead of quiet/mid slicing.
        if num_states == 2:
            factor_state_init = torch.tensor([-0.20, 0.20], dtype=torch.float32)
            diag_state_init = torch.tensor([-0.10, 0.15], dtype=torch.float32)
            scale_state_init = torch.tensor([-0.45, 0.40], dtype=torch.float32)
            nu_state_init = torch.tensor([0.15, -0.25], dtype=torch.float32)
        else:
            factor_state_init = torch.tensor([-0.20, 0.00, 0.20], dtype=torch.float32)[:num_states]
            diag_state_init = torch.tensor([-0.10, 0.00, 0.15], dtype=torch.float32)[:num_states]
            scale_state_init = torch.tensor([-0.45, 0.00, 0.40], dtype=torch.float32)[:num_states]
            nu_state_init = torch.tensor([0.15, 0.00, -0.25], dtype=torch.float32)[:num_states]

        self.state_cond_offsets = nn.Parameter(torch.zeros(num_states, cond_dim))
        self.state_factor_log_scale = nn.Parameter(factor_state_init)
        self.state_diag_log_scale = nn.Parameter(diag_state_init)
        self.state_scale_log_scale = nn.Parameter(scale_state_init)
        if self.fixed_nu is None:
            self.state_nu_log_scale = nn.Parameter(nu_state_init)

        self.register_buffer("event_kernel_bank", self._build_event_kernel_bank())

        self._init_parameters(init_diag=init_diag, init_scale=init_scale, init_nu=init_nu)

    def _make_kernel(self, anchor: int, family: str) -> np.ndarray:
        side = int(round(self.n_cells ** 0.5))
        ax, ay = divmod(anchor, side)
        ker = np.zeros((side, side), dtype=np.float32)
        for i in range(side):
            for j in range(side):
                dx = abs(i - ax)
                dy = abs(j - ay)
                ...
                ker[i, j] = val
        flat = ker.reshape(-1)
        flat /= max(float(flat.sum()), 1e-8)
        return flat

    def _build_event_kernel_bank(self) -> torch.Tensor:
        kernels = []
        for fam in self.event_families:
            fam_kernels = [self._make_kernel(anchor, fam) for anchor in range(self.n_cells)]
            kernels.append(np.stack(fam_kernels, axis=0))
        return torch.tensor(np.stack(kernels, axis=0), dtype=torch.float32)
    ...
    def _init_parameters(self, init_diag: float, init_scale: float, init_nu: float) -> None:
        ...
        for module in (self.event_anchor_head, self.event_family_head, self.event_amp_head):
            for layer in module:
                if isinstance(layer, nn.Linear):
                    nn.init.zeros_(layer.weight)
                    nn.init.zeros_(layer.bias)

    def localized_event_field(
        self,
        cond: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        temp = max(float(self.event_temperature), 1e-3)
        anchor_logits = self.event_anchor_head(cond)
        family_logits = self.event_family_head(cond)
        anchor_probs = F.softmax(anchor_logits / temp, dim=-1)
        family_probs = F.softmax(family_logits / temp, dim=-1)
        kernel_mix = torch.einsum(
            "bf,bk,fkc->bc",
            family_probs,
            anchor_probs,
            self.event_kernel_bank.to(cond.dtype),
        )
        kernel_mix = kernel_mix / kernel_mix.sum(dim=-1, keepdim=True).clamp_min(1e-8)
        amp = F.softplus(self.event_amp_head(cond).squeeze(-1)) + self.event_amp_floor
        local_var_mult = 1.0 + amp.unsqueeze(-1) * kernel_mix
        return anchor_logits, family_logits, amp, local_var_mult

    def select_localized_event_field(
        self,
        cond: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        anchor_logits, family_logits, amp, _local_var_mult = self.localized_event_field(cond)
        anchor_idx = torch.multinomial(F.softmax(anchor_logits, dim=-1), num_samples=1).squeeze(-1)
        family_idx = torch.multinomial(F.softmax(family_logits, dim=-1), num_samples=1).squeeze(-1)
        kernel = self.event_kernel_bank[family_idx, anchor_idx].to(cond.device, dtype=cond.dtype)
        local_var_mult = 1.0 + amp.unsqueeze(-1) * kernel
        return anchor_idx, family_idx, amp, local_var_mult
    ...
    def forward(
        ...
        anchor_logits, family_logits, event_amp, local_var_mult = self.localized_event_field(cond)
        if num_states >= 2:
            local_std_mult = local_var_mult.sqrt()
            factor_event = factor[:, 1, :, :] * local_std_mult.unsqueeze(-1)
            diag_event = diag[:, 1, :] * local_std_mult
            factor = torch.cat([factor[:, :1, :, :], factor_event.unsqueeze(1)], dim=1)
            diag = torch.cat([diag[:, :1, :], diag_event.unsqueeze(1)], dim=1)

        return mu, factor, diag, scale, nu, anchor_logits, family_logits, event_amp
...
class RegimeSwitchingLatentFactorARModel(nn.Module):
    ...
    def state_params_from_history(
        ...
        mu, factor, diag, scale, nu, _anchor_logits, _family_logits, _event_amp = self.decoder(cond, prev_u)
        ...
        for start in range(0, n_samples, chunk_size):
            ...
            for _ in range(n_steps):
                ...
                if self.num_states >= 2:
                    event_mask = state_idx == 1
                    if event_mask.any():
                        _anchor_idx, _family_idx, _amp, local_var_mult = self.decoder.select_localized_event_field(
                            cond[event_mask]
                        )
                        local_std_mult = local_var_mult.sqrt()
                        factor_sel[event_mask] = factor_sel[event_mask] * local_std_mult.unsqueeze(-1)
                        diag_sel[event_mask] = diag_sel[event_mask] * local_std_mult
...
def build_semantic_state_targets(
    quiet_mu_u: torch.Tensor,
    quiet_var_cells: torch.Tensor,
    target_u: torch.Tensor,
    num_states: int,
    objective_config: dict,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Soft quiet/mid/high targets from concentrated quiet-state underfit."""
    quiet_resid2 = (target_u - quiet_mu_u).pow(2)
    quiet_ratio = quiet_resid2 / quiet_var_cells.clamp_min(objective_config["semantic_eps"])

    k = max(1, min(int(objective_config["semantic_topk"]), quiet_ratio.shape[-1]))
    topk_ratio = torch.topk(quiet_ratio, k=k, dim=-1).values.mean(dim=-1)

    quiet_score = torch.sigmoid(
        (objective_config["semantic_quiet_trigger"] - topk_ratio)
        / objective_config["semantic_temp"]
    )
    high_score = torch.sigmoid(
        (topk_ratio - objective_config["semantic_high_trigger"])
        / objective_config["semantic_temp"]
    )

    if num_states == 1:
        target = torch.ones_like(topk_ratio.unsqueeze(-1))
    elif num_states == 2:
        raw = torch.stack(
            [
                quiet_score + objective_config["semantic_floor_prob"],
                high_score + objective_config["semantic_floor_prob"],
            ],
            dim=-1,
        )
        target = raw / raw.sum(dim=-1, keepdim=True)
    else:
        mid_mass = (
            objective_config["semantic_mid_floor"]
            + (1.0 - quiet_score) * (1.0 - high_score)
        ) / float(num_states - 2)
        pieces = [quiet_score + objective_config["semantic_floor_prob"]]
        for _ in range(num_states - 2):
            pieces.append(mid_mass + objective_config["semantic_floor_prob"])
        pieces.append(high_score + objective_config["semantic_floor_prob"])
        raw = torch.stack(pieces, dim=-1)
        target = raw / raw.sum(dim=-1, keepdim=True)

    return target.detach(), topk_ratio.detach()


def build_local_event_targets(
    quiet_mu_u: torch.Tensor,
    quiet_var_cells: torch.Tensor,
    target_u: torch.Tensor,
    ...
    """Target localized event anchor/kernel from quiet-state underfit."""
    quiet_resid2 = (target_u - quiet_mu_u).pow(2)
    quiet_ratio = quiet_resid2 / quiet_var_cells.clamp_min(objective_config["semantic_eps"])
    excess = F.relu(quiet_ratio - objective_config["event_underfit_margin"])
    excess = excess.pow(objective_config["event_target_power"])
    excess_sum = excess.sum(dim=-1, keepdim=True)
    target_map = excess / excess_sum.clamp_min(1e-8)

    scores = torch.einsum("btc,fkc->btfk", target_map, kernel_bank)
    best_family_per_anchor = scores.max(dim=2).values
    anchor_idx = best_family_per_anchor.argmax(dim=-1)
    best_family = scores.gather(
        3,
        anchor_idx.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, scores.shape[2], 1),
    ).squeeze(-1).argmax(dim=2)

    k = max(1, min(int(objective_config["event_amp_topk"]), excess.shape[-1]))
    amp_target = torch.topk(excess, k=k, dim=-1).values.mean(dim=-1)
    return target_map.detach(), anchor_idx.detach(), best_family.detach(), amp_target.detach()
...
def regime_sequence_objective(
    ...
    for step in range(future_len):
        ...
        mu, factor, diag, scale, nu, anchor_logits, family_logits, event_amp = model.decoder(cond, prev_u)
        ...
        nu_states.append(nu)
        anchor_logits_steps.append(anchor_logits)
        family_logits_steps.append(family_logits)
        event_amp_steps.append(event_amp)
    ...
    anchor_logits_t = torch.stack(anchor_logits_steps, dim=1)
    family_logits_t = torch.stack(family_logits_steps, dim=1)
    event_amp_t = torch.stack(event_amp_steps, dim=1)
    ...
    quiet_mu_u = mu_states_u_t[:, :, 0, :]
    quiet_var_cells = pred_var_cells[:, :, 0, :]
    state_targets, topk_ratio = build_semantic_state_targets(
        quiet_mu_u=quiet_mu_u,
        quiet_var_cells=quiet_var_cells,
        target_u=target_u,
        num_states=model.num_states,
        objective_config=objective_config,
    )
    _event_target_map, event_anchor_idx, event_family_idx, event_amp_target = build_local_event_targets(
        quiet_mu_u=quiet_mu_u,
        quiet_var_cells=quiet_var_cells,
        target_u=target_u,
        kernel_bank=model.decoder.event_kernel_bank.to(target_u.device, dtype=target_u.dtype),
        objective_config=objective_config,
    )
    state_semantic_loss = -(
        state_targets * gamma.clamp_min(1e-8).log()
    ).sum(dim=-1).mean()

    event_weight = state_targets[..., -1].detach()
    anchor_ce = F.cross_entropy(
        anchor_logits_t.reshape(-1, anchor_logits_t.shape[-1]),
        event_anchor_idx.reshape(-1),
        reduction="none",
    ).reshape_as(event_weight)
    family_ce = F.cross_entropy(
        family_logits_t.reshape(-1, family_logits_t.shape[-1]),
        event_family_idx.reshape(-1),
        reduction="none",
    ).reshape_as(event_weight)
    event_amp_loss = F.smooth_l1_loss(event_amp_t, event_amp_target, reduction="none")
    weight_norm = event_weight.mean().clamp_min(1e-6)
    localized_anchor_loss = (anchor_ce * event_weight).mean() / weight_norm
    localized_family_loss = (family_ce * event_weight).mean() / weight_norm
    event_amp_alignment_loss = (event_amp_loss * event_weight).mean() / weight_norm

    if xi.shape[1] > 0:
        target_xi = state_targets[:, :-1].unsqueeze(-1) * state_targets[:, 1:].unsqueeze(-2)
        transition_semantic_loss = -(
            target_xi * xi.clamp_min(1e-8).log()
        ).sum(dim=(-1, -2)).mean()
    else:
        transition_semantic_loss = history_01.new_tensor(0.0)
    ...
    total_loss = (
        ...
        + objective_config["state_semantic_weight"] * state_semantic_loss
        + objective_config["transition_semantic_weight"] * transition_semantic_loss
        + objective_config["localized_anchor_weight"] * localized_anchor_loss
        + objective_config["localized_family_weight"] * localized_family_loss
        + objective_config["event_amp_weight"] * event_amp_alignment_loss
    ...
    metrics = {
        ...
        "state_semantic_loss": state_semantic_loss,
        "transition_semantic_loss": transition_semantic_loss,
        "localized_anchor_loss": localized_anchor_loss,
        "localized_family_loss": localized_family_loss,
        "event_amp_alignment_loss": event_amp_alignment_loss,
        "semantic_topk_ratio": topk_ratio.mean(),
        "semantic_quiet_target": state_targets[..., 0].mean(),
        "semantic_high_target": state_targets[..., -1].mean(),
        "event_amp_mean": event_amp_t.mean(),
...
def build_objective_config(args: argparse.Namespace) -> dict[str, float]:
    return dict(
        ...
        state_semantic_weight=args.state_semantic_weight,
        transition_semantic_weight=args.transition_semantic_weight,
        semantic_topk=args.semantic_topk,
        semantic_quiet_trigger=args.semantic_quiet_trigger,
        semantic_high_trigger=args.semantic_high_trigger,
        semantic_temp=args.semantic_temp,
        semantic_mid_floor=args.semantic_mid_floor,
        semantic_floor_prob=args.semantic_floor_prob,
        semantic_eps=args.semantic_eps,
        event_underfit_margin=args.event_underfit_margin,
        event_target_power=args.event_target_power,
        event_amp_topk=args.event_amp_topk,
        localized_anchor_weight=args.localized_anchor_weight,
        localized_family_weight=args.localized_family_weight,
        event_amp_weight=args.event_amp_weight,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="195a: localized marked-event AR latent-factor Student-t")
    ...
    parser.add_argument("--state_semantic_weight", type=float, default=0.18)
    parser.add_argument("--transition_semantic_weight", type=float, default=0.08)
    parser.add_argument("--semantic_topk", type=int, default=4)
    parser.add_argument("--semantic_quiet_trigger", type=float, default=0.95)
    parser.add_argument("--semantic_high_trigger", type=float, default=1.30)
    parser.add_argument("--semantic_temp", type=float, default=0.20)
    parser.add_argument("--semantic_mid_floor", type=float, default=0.20)
    parser.add_argument("--semantic_floor_prob", type=float, default=0.02)
    parser.add_argument("--semantic_eps", type=float, default=1e-6)
    parser.add_argument("--event_underfit_margin", type=float, default=1.10)
    parser.add_argument("--event_target_power", type=float, default=1.5)
    parser.add_argument("--event_amp_topk", type=int, default=3)
    parser.add_argument("--localized_anchor_weight", type=float, default=0.12)
    parser.add_argument("--localized_family_weight", type=float, default=0.08)
    parser.add_argument("--event_amp_weight", type=float, default=0.06)
    ...
    parser.add_argument("--warm_start", type=str, default="models/backfill/regime_switching_ar_latent_factor_194c/best_model.pt")
    ...
    decoder_config = dict(
        ...
        fixed_nu=fixed_nu,
        event_temperature=0.40,
        event_amp_floor=0.05,
    )
    regime_config = dict(
        num_states=args.num_states,
    ...
    print(f"\n{'=' * 64}")
    print("195a: Localized Marked-Event AR Latent-Factor Student-t")
    print(f"{'=' * 64}")
    ...
    print("  Objective: exact teacher-forced discrete-state marginal likelihood + quiet/event semantic alignment")
    print("             + localized event anchor/kernel supervision from quiet-state underfit")
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        selection_score = (
            ...
            + 0.06 * max(0.0, val_metrics["val_state_semantic_loss"] - 0.85)
            + 0.04 * max(0.0, val_metrics["val_transition_semantic_loss"] - 1.20)
            + 0.03 * max(0.0, val_metrics["val_localized_anchor_loss"] - 1.75)
            + 0.02 * max(0.0, val_metrics["val_localized_family_loss"] - 1.25)
            ...
            torch.save(
                {
                    ...
                    "selection_score": best_score,
                        "config": {
                        "type": "regime_switching_ar_latent_factor_195a",
        ...
        print(
            ...
            f"| sem={val_metrics['val_state_semantic_loss']:.4f} "
            f"| xsem={val_metrics['val_transition_semantic_loss']:.4f} "
            f"| anch={val_metrics['val_localized_anchor_loss']:.4f} "
            f"| fam={val_metrics['val_localized_family_loss']:.4f} "
