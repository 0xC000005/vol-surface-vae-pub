#!/usr/bin/env python
"""
261a-v0: residual scenario FM on top of frozen 260e.
"""
...
def fm_step(
    ...
    *,
    ...
    with torch.no_grad():
        ...
        target_resid = target_coord - center_coord

    x0 = torch.randn_like(target_resid)
    t = torch.rand(bsz, device=target_resid.device, dtype=target_resid.dtype)
    t_view = t[:, None, None]
    x_t = (1.0 - t_view) * x0 + t_view * target_resid
    target_v = target_resid - x0
    pred_v = model.velocity(x_t, t, cond)
    fm_loss = (pred_v - target_v).pow(2).mean()

    sampled_resid = model.sample_residual_paths(
        history_norm,
        n_samples=zero_mean_samples,
        cond=cond,
    )
    total_coord = center_coord.unsqueeze(1) + sampled_resid
    raw_total = model.base_model.inverse_transform_change(total_coord, history_norm)
    raw_resid_uncentered = raw_total - center_raw_change.unsqueeze(1)
    zero_mean_loss = raw_resid_uncentered.mean(dim=1).pow(2).mean()

    resid_coord_std = sampled_resid.std(dim=1).mean()
    raw_resid_std = raw_resid_uncentered.std(dim=1).mean()
    loss = fm_loss + zero_mean_weight * zero_mean_loss
    metrics = {
        "total": loss.detach(),
        "fm_loss": fm_loss.detach(),
        "zero_mean_loss": zero_mean_loss.detach(),
        "resid_coord_std": resid_coord_std.detach(),
        "raw_resid_std": raw_resid_std.detach(),
    }
    return loss, metrics


def main() -> None:
    parser = argparse.ArgumentParser(description="261a-v0 residual scenario FM")
    ...
    model = ResidualScenarioFM(cfg, base_model=base_model).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "train_raw_resid_std": train_avg["raw_resid_std"],
            "val_total": val_avg["total"],
            "val_fm": val_avg["fm_loss"],
            "val_zero_mean": val_avg["zero_mean_loss"],
            "val_raw_resid_std": val_avg["raw_resid_std"],
        ...
        print(
            ...
            f"zero={rec['val_zero_mean']:.5f} rawstd={rec['val_raw_resid_std']:.5f} "
