...
from diffusion.block_ar.ar_surface_probabilistic_token_model import (
    ARSurfaceProbabilisticTokenConfig,
    ARSurfaceProbabilisticTokenModel,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="274a-v0 autoregressive probabilistic latent-token state-space model"
    )
    ...
    model = ARSurfaceProbabilisticTokenModel(cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"kl={rec['val_kl']:.5f} "
            f"prior_std={rec['val_prior_std']:.4f} "
            f"post_std={rec['val_post_std']:.4f} "
            f"lr={rec['lr']:.2e} "
            f"time={rec['sec']:.1f}s"
        )
