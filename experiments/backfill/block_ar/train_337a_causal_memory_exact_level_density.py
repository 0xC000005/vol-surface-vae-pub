...
from diffusion.block_ar.causal_memory_exact_level_density import (
    CausalMemoryExactLevelDensity,
    CausalMemoryExactLevelDensityConfig,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="337a causal memory exact level density"
    )
    ...
    model = CausalMemoryExactLevelDensity(cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "train_base_std": train_avg["base_std"],
            "train_logdet": train_avg["logdet"],
            "val_total": val_avg["total"],
            "val_base_std": val_avg["base_std"],
            "val_logdet": val_avg["logdet"],
            "val_level_std": val_avg["level_std"],
        ...
        print(
            f"[ep {epoch:03d}] "
            f"train={rec['train_total']:.4f} val={rec['val_total']:.4f} "
            f"base_std={rec['val_base_std']:.3f} logdet={rec['val_logdet']:.3f} "
            f"level_std={rec['val_level_std']:.3f} "
