#!/usr/bin/env python
"""
187b_v0: Underfit-support residual model on top of the 183c backbone.
...
Principle:
  ...
  - train support on baseline under-explained concentration, not raw residual magnitude
"""
...
class SparseSupportResidualModel(StateMetricTransportModel):
    ...
    def build_teacher_support(
        ...
        score = target_event_local.clamp_min(0.0)
        ...
        for block_idx in range(self.n_support_blocks):
            ...
            if threshold > 0.0:
                positive_mask = positive_mask & (flat >= threshold)
            flat_support = flat_support * positive_mask.to(flat.dtype)
    ...
    def build_teacher_event_components(
        ...
        # Event support should represent where the quiet baseline under-explains
        # concentration, not where residual magnitude is simply large.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="187b_v0: underfit-support residual model")
    ...
    print(f"\n{'=' * 72}\n187b_v0: underfit-support residual model\n{'=' * 72}")
