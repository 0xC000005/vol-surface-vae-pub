#!/usr/bin/env python
"""
Diagnostic for Exp 157a: per-frame ES lambda=0.1, noise_dim=32.
Focus on: spread stability, correlation oscillation, ES gradient analysis.
"""
...
def main():
    ...
    model, cfg = load_model('models/backfill/flow_157a/final_model.pt', device)
    print(f"Model config: noise_dim={cfg['noise_dim']}, lambda_es={cfg.get('lambda_es', 0)}")

    cached = np.load("models/backfill/flow_154b/base_predictions.npz")
    val_preds, val_gts = cached["val_preds"], cached["val_gts"]
    ...
    for r in range(5):
        for c in range(5):
            ...
            cell_ci[r, c] = ((gt[:, :, cidx] >= lo) & (gt[:, :, cidx] <= hi)).mean()
    print("Cell CI:")
    for r in range(5):
    ...
    print(f"Worst: ({worst_r},{worst_c})={cell_ci[worst_r,worst_c]:.3f}, Mean={cell_ci.mean():.3f}")
    ...
    def eff_rank(cm):
        ev = np.linalg.eigvalsh(cm)[::-1]; ev = np.maximum(ev, 0)
        p = ev / (ev.sum() + 1e-10); p = p[p > 1e-10]
        return float(np.exp(-np.sum(p * np.log(p))))

    gen_er = eff_rank(gen_corr); gt_er = eff_rank(gt_corr)
    print(f"Corr ratio: {corr_ratio:.3f}, Eff rank: {gen_er:.2f} vs GT {gt_er:.2f}")
    results['corr_ratio'] = float(corr_ratio)
    results['eff_rank_ratio'] = gen_er / gt_er

    # C. Spread stability (training history)
    print("\n=== C. Spread stability ===")
    hist_path = Path('models/backfill/flow_157a/training_history.json')
    if hist_path.exists():
        history = json.load(open(hist_path))
        spreads = [(h['epoch'], h['spread']) for h in history]
        peak = max(s for _, s in spreads)
        final = spreads[-1][1]
        print(f"Peak: {peak:.4f}, Final: {final:.4f}, Contraction: {peak/final:.2f}x")
        # Compare with H1 models
        print(f"Contraction comparison:")
        print(f"  157a (ES):     {peak/final:.2f}x")
        print(f"  156a (dim=4):  1.87x")
        print(f"  156b (dim=8):  1.80x")
        results['spread_contraction'] = peak / final
    ...
    for scale in [0.8, 1.0, 1.3]:
        ...
        results[f'scaled_{scale}x'] = {'ci': float(wci), 'corr': float(sc_cr), 'ks': ks2}
    ...
    # F. Correlation oscillation analysis from training history
    print("\n=== F. Correlation oscillation (from eval checkpoints) ===")
    if hist_path.exists():
        eval_entries = [(h['epoch'], h.get('corr', None)) for h in history if 'corr' in h]
        if eval_entries:
            corr_vals = [c for _, c in eval_entries]
            print(f"Correlation trajectory: {['%.3f' % c for _, c in eval_entries]}")
            print(f"Std of correlation: {np.std(corr_vals):.3f}")
            print(f"Range: {min(corr_vals):.3f} - {max(corr_vals):.3f}")
            results['corr_std'] = float(np.std(corr_vals))

    out_dir = Path('results/block_ar/157a_diag')
    out_dir.mkdir(parents=True, exist_ok=True)
    with open(out_dir / 'diagnostics.json', 'w') as f:
        json.dump(results, f, indent=2, default=str)
    print(f"\nSaved to {out_dir}/diagnostics.json")
