#!/usr/bin/env python
"""
263a-v0: joint state-space latent-factor FM.
"""
...
from diffusion.block_ar.joint_state_space_latent_factor_fm import (
    JointStateSpaceLatentFactorFM,
    JointStateSpaceLatentFactorFMConfig,
    save_checkpoint,
)
...
def fm_step(
    ...
    *,
    ...
    change_loss = F.smooth_l1_loss(model.decode_change_coord(zero_eps, cond), target_coord)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="263a-v0 joint state-space latent-factor FM")
    ...
    model = JointStateSpaceLatentFactorFM(cfg).to(device)
