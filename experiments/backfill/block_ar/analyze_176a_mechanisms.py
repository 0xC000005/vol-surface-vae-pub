#!/usr/bin/env python
"""
Focused mechanistic analysis for 176a.

Questions:
  1. Did the latent residual components actually specialize?
  2. Is the gate prior condition-responsive or nearly uniform?
  3. Are posterior assignments concentrated on hard failure slices?
  4. Would forced use of a single component help on specific slices?
  5. What is the next principled move after 176a?
"""
...
def load_model(checkpoint_path: str, device: str):
    ...
    if raw_config["type"] != "latent_regime_structured_residual_student_t_176a":
        raise ValueError(
            "Expected latent_regime_structured_residual_student_t_176a, "
...
def summarize_top_windows(values: np.ndarray, top_k: int = 10, reverse: bool = True) -> list[dict[str, Any]]:
    idx = np.argsort(values)
    if reverse:
        idx = idx[::-1]
    out = []
    for i in idx[:top_k]:
        out.append({"window": int(i), "value": float(values[i])})
    return out
...
def analyze_176a(
    ...
    forced_component_cov = None
    forced_component_width = None
    forced_component_tc = None
    forced_component_h30_worst = None

    # Collect batchwise tensors to later compute forced-component behavior.
    hist_batches = []
    fut_batches = []
    ...
    for start in range(0, n_windows, batch_size):
        ...
        component_scale.append(scale.detach().cpu())
        component_local_rms.append(aux["local_delta_rms"].detach().cpu())
        component_white_std.append(aux["white_std"].detach().cpu())
        component_z_std.append(aux["z_std"].detach().cpu())
    ...
    component_scale = torch.cat(component_scale).numpy()
    component_local_rms = torch.cat(component_local_rms).numpy()
    component_white_std = torch.cat(component_white_std).numpy()
    component_z_std = torch.cat(component_z_std).numpy()

    n_components = prior_probs.shape[1]
    arg_post = posterior_probs.argmax(axis=1)
    arg_prior = prior_probs.argmax(axis=1)
    ...
    for k in range(n_components):
        ...
        component_specialization[f"component_{k}"] = {
            ...
            "avg_scale": float(component_scale[:, k].mean()),
            "avg_local_delta_rms": float(component_local_rms[:, k].mean()),
            "avg_white_std": float(component_white_std[:, k].mean()),
            "avg_z_std": float(component_z_std[:, k].mean()),
    ...
    # Forced single-component counterfactuals on the full subset.
    ...
    for k in range(n_components):
        ...
        for start in range(0, n_windows, batch_size):
            ...
            n_frames = mu.shape[1]
            chol_t = torch.linalg.cholesky(model.covariance_parts(time_factor, time_diag, cell_factor, cell_diag, scale)[0][:, k])
            chol_c = torch.linalg.cholesky(model.covariance_parts(time_factor, time_diag, cell_factor, cell_diag, scale)[1][:, k])
            local_scale = torch.exp(0.5 * local_delta[:, k])
            ctx = flow_context[:, k]
    ...
    # Next-step inference
    prior_is_condition_responsive = any(abs(v) > 0.15 for v in prior_condition_correlation.values())
    components_specialize_on_hard_windows = any(v["hard_window_fraction"] > hard_window_mask.mean() + 0.10 for v in component_specialization.values())
    at_least_one_component_handles_hard_slices = any(v > 0.68 for v in forced_component_h30_worst.values())

    next_step_inference = {
        "prior_is_condition_responsive": bool(prior_is_condition_responsive),
        "components_specialize_on_hard_windows": bool(components_specialize_on_hard_windows),
        "at_least_one_component_handles_h30_hard_cells_better": bool(at_least_one_component_handles_hard_slices),
        "suggestion": (
            "sharpen_prior_routing"
            if (not prior_is_condition_responsive and components_specialize_on_hard_windows)
            else "redesign_component_factorization"
        ),
    }
    ...
    return {
        ...
        "top_hard_windows": summarize_top_windows(-per_window_cov, top_k=15, reverse=True),
        "next_step_inference": next_step_inference,
    }


def main():
    parser = argparse.ArgumentParser(description="Mechanistic analysis for 176a")
