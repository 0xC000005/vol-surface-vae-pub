#!/usr/bin/env python
"""
173a: Local log-variance correction on top of the 172a residual-flow model.
...
Add:
  - a small history-conditioned local log-variance residual field
...
class LocalVarianceResidualFlowStructuredJointDecoder(ResidualFlowStructuredJointDecoder):
    """172a decoder plus a small centered time x cell log-variance residual."""
    ...
    def forward(
        ...
        local_delta_raw = self.local_logvar_head(hidden).squeeze(-1)
...
class LocalVarianceResidualFlowStructuredJointStudentTModel(
    ResidualFlowStructuredJointStudentTModel
):
    """172a model with a regularized local variance correction field."""
    ...
    def __init__(
        ...
        self.decoder = LocalVarianceResidualFlowStructuredJointDecoder(**decoder_config)
...
def joint_nll_loss(
    ...
    loss = nll + local_var_penalty * local_penalty
    metrics = {
        "joint_nll": nll,
        "total_loss": loss,
...
def evaluate_teacher_forced(
    ...
    for history_01, future_01 in val_loader:
        loss, metrics = joint_nll_loss(model, history_01, future_01, local_var_penalty=local_var_penalty)
...
def main():
    parser = argparse.ArgumentParser(description="173a: local variance residual on top of 172a")
    ...
    print(f"\n{'=' * 64}")
    print("173a: Local variance residual on top of 172a")
    print(f"{'=' * 64}")
    ...
    print(f"  Local delta clip={args.local_delta_clip} | local penalty={args.local_var_penalty}")
    print(f"  Support transform: logit(({args.support_lo}, {args.support_hi}))")
    print("  Objective: exact joint likelihood + local variance shrinkage")
