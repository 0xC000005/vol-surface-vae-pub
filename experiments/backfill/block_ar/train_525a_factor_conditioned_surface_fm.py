#!/usr/bin/env python
"""525a: zero-gated factor-conditioned extension of the 340c surface-level FM."""
...
def load_factor_extended_checkpoint(
    checkpoint: str,
    factor_dim: int,
    device: torch.device,
...
def main() -> None:
    ...
    train_block = build_factor_history_block(
    ...
    model, payload = load_factor_extended_checkpoint(args.checkpoint, factor_dim, device)

    for name, param in model.named_parameters():
        param.requires_grad = name.startswith("factor_")
    trainable = [p for p in model.parameters() if p.requires_grad]
    if not trainable:
        raise RuntimeError("No factor parameters are trainable")
