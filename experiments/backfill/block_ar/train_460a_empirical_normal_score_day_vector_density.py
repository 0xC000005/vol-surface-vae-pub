...
from diffusion.block_ar.empirical_normal_score_day_vector_density import (
    EmpiricalNormalScoreDayVectorDensity,
    EmpiricalNormalScoreDayVectorDensityConfig,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="460a empirical-normal-score day-vector conditional density"
    )
    ...
    parser.add_argument("--chol_diag_floor", type=float, default=1e-3)
    ...
    cfg = EmpiricalNormalScoreDayVectorDensityConfig(
        ...
        chol_diag_floor=args.chol_diag_floor,
        sample_temperature=args.sample_temperature,
        max_sample_chunk=args.max_sample_chunk,
    )
    model = EmpiricalNormalScoreDayVectorDensity(cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"loc_abs={rec['val_loc_abs']:.3f} "
            f"diag={rec['val_diag_mean']:.3f}/{rec['val_diag_min']:.3f} "
