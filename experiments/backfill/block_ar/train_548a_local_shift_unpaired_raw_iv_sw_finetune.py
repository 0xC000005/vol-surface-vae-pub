#!/usr/bin/env python
"""548a: unpaired raw-IV joint-law SW fine-tune for the 544a local-shift AR flow."""
...
from experiments.backfill.block_ar.train_453a_recent_rollout_iv_marginal_crps_finetune import (
    build_recent_block,
)
...
def joint_sliced_wasserstein_loss(
    ...
    """Unpaired batch-law alignment of raw-IV `(history, future)` paths."""
    ...
    fut_gen = generated_iv.reshape(bsz * n_samples, horizon * n_cells) * float(
        future_weight
    )
...
def combined_loss(
    ...
    history_iv = denormalize_iv(model._flatten(history_norm))
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"sw={rec['val_joint_sw']:.5f} center={rec['val_center_gap']:.4f} "
            f"spread={rec['val_sample_spread']:.4f} "
