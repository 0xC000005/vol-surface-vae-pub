#!/usr/bin/env python
"""494a: recent-window interval-score fine-tune from the 392a frontier."""
...
def central_interval_score(
    ...
    """Proper central interval score averaged over horizon/cell marginals."""
    if samples.ndim != 4 or target.ndim != 3:
        raise ValueError("Expected samples [B,K,T,C] and target [B,T,C]")
    lo_q = float(alpha) / 2.0
    hi_q = 1.0 - lo_q
    lo = torch.quantile(samples, lo_q, dim=1)
    hi = torch.quantile(samples, hi_q, dim=1)
    width = hi - lo
    below = torch.relu(lo - target)
    above = torch.relu(target - hi)
    miss = (2.0 / float(alpha)) * (below + above)
    score = width + miss
    return score.mean(), width.mean(), below.mean(), above.mean()
...
def combined_loss(
    ...
    interval_loss, width, below, above = central_interval_score(
        sampled_scores,
        target_scores,
        alpha=interval_alpha,
    )
    total = float(fm_anchor_weight) * fm_loss + float(interval_weight) * interval_loss
    metrics = {
        "total": total.detach(),
        "fm_loss": fm_loss.detach(),
        "interval": interval_loss.detach(),
        "interval_width": width.detach(),
        "interval_below": below.detach(),
        "interval_above": above.detach(),
...
def main() -> None:
    ...
    parser.add_argument("--interval_alpha", type=float, default=0.10)
    parser.add_argument("--interval_weight", type=float, default=0.02)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "train_interval": train_avg["interval"],
            "val_total": val_avg["total"],
            "val_fm_loss": val_avg["fm_loss"],
            "val_interval": val_avg["interval"],
            "val_interval_width": val_avg["interval_width"],
            "val_interval_below": val_avg["interval_below"],
            "val_interval_above": val_avg["interval_above"],
        ...
        print(
            ...
            f"is={rec['val_interval']:.5f} width={rec['val_interval_width']:.3f} "
            f"miss={rec['val_interval_below']:.3f}/{rec['val_interval_above']:.3f} "
