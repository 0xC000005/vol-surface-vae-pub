...
from diffusion.block_ar.probabilistic_joint_token_path_latent_model import (
    ProbabilisticJointTokenPathLatentModel,
    ProbabilisticJointTokenPathLatentModelConfig,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="293b-v0 probabilistic fixed-horizon joint-token path model with global path latent"
    )
    ...
    model = ProbabilisticJointTokenPathLatentModel(cfg, codebook.to(device)).to(device)
