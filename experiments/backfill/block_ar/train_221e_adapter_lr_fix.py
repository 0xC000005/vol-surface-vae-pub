#!/usr/bin/env python
"""
221e: Adapter LR fix — slow adapter growth to preserve h=1 quality through training.

221d proved the adapter pattern works (h=30 MR 1.106, all horizon ratios pass at epoch 4)
but adapter grows too fast (norm 0→20 over 50 epochs), degrading h=1 from 83%→71%.
Best epoch was 4/50 — the adapter barely had time to help before it started hurting.

Fix: reduce adapter LR from 1e-3 to 1e-4 (10x slower growth). Everything else identical.
Prediction: h=1 stays above 78% through epoch 50, h=30 MR still improves.
"""
...
class AdapterRecurrentFlowTransitionModel(RecurrentFlowTransitionModel):
    ...
    def __init__(self, *args, adapter_hidden: int = 64, adapter_ramp_steps: int = 0, **kwargs):
        ...
        self.adapter_ramp_steps = adapter_ramp_steps
...
def main() -> None:
    parser = argparse.ArgumentParser(description="221e adapter LR fix multi-day AR flow")
    ...
    parser.add_argument("--adapter_ramp_steps", type=int, default=0,
                        help="Ramp adapter weight from 0 to 1 over this many steps (0=disabled)")
    ...
    model = AdapterRecurrentFlowTransitionModel(
        ...
        adapter_ramp_steps=args.adapter_ramp_steps,
    ).to(device)
    ...
    print(f"221e adapter LR fix multi-day AR flow")
