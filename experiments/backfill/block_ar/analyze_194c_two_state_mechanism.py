#!/usr/bin/env python
"""
Focused mechanistic review of 194c two-state quiet/event regression.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Mechanistic review of 194c two-state quiet/event AR model")
    parser.add_argument("--best_ckpt", type=str, default="models/backfill/regime_switching_ar_latent_factor_194c/best_model.pt")
    parser.add_argument("--final_ckpt", type=str, default="models/backfill/regime_switching_ar_latent_factor_194c/final_model.pt")
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="results/validations/2026-04-07/analysis/194c_two_state_mechanistic",
    )
    ...
    models = {
        "194c_best": (best_model, best_payload),
        "194c_final": (final_model, final_payload),
    }
    ...
    ablation = {
        ...
        "best_forced_event": compute_rollout_stats(
            best_model,
            ablation_hist,
            ablation_future,
            device=device,
    ...
    conclusion = {
        ...
        "regime_separation_read": (
            "194c still uses the discrete states meaningfully: calm windows shift toward the quiet state, "
        ...
        "quiet_state_read": (
            "If the quiet state's calm-h30 variance stays above 169c on clean points, then 194c is still too wide "
