#!/usr/bin/env python
"""
Focused mechanistic review of 187a_v0.

Questions:
  1. Does learned sparse support overlap the exact hard S3/S7 slices?
  2. Does turning the event branch off help or hurt broad behavior?
  3. Is the teacher support decomposition aligned with the hard slices, or is it
     mostly selecting something else?
"""
...
from experiments.backfill.block_ar.train_187a_sparse_support_residual import SparseSupportResidualModel
...
def sample_eval_arrays(
    ...
    shoulder_ratio = float(
        (((pooled_gen > q50) & (pooled_gen <= q95)).mean()) / max((((pooled_gt > q50) & (pooled_gt <= q95)).mean()), 1e-12)
    )
    ...
    kurt_ratio = float(pearson_kurtosis(torch.from_numpy(gen_diff[:, 0])) / max(pearson_kurtosis(torch.from_numpy(gt_diff)), 1e-12))
...
def target_state_support_analysis(
    ...
    for start in range(0, history_norm.shape[0], batch_size):
        ...
        support_pack = model.infer_support_process(
            ...
            state_local=model.build_state_features(z_t)[0],
            quiet_metric_local=quiet_metric_local,
            t=t,
            teacher_abs=teacher_abs,
        )
        prior_controls = model.build_sparse_support_controls(z_t, t, path_context, support_pack=support_pack, use_posterior=False)
        post_controls = model.build_sparse_support_controls(z_t, t, path_context, support_pack=support_pack, use_posterior=True)
...
def review_checkpoint(
    ...
    review = {
        ...
        "event_off_ablation": {
            ...
            "hard_late_coverage_delta_off_minus_on": float(safe_mean(off_coverage_flat[hard_mask]) - safe_mean(coverage_flat[hard_mask])),
        },
    }
    return review
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Focused mechanistic review of 187a sparse-support residual model")
    parser.add_argument(
        "--best_ckpt",
        default="models/backfill/latent_sparse_support_residual_pathwise_residual_law_mean_reverting_covariance_mixture_structured_joint_student_t_187a/best_model.pt",
    )
    parser.add_argument(
        "--final_ckpt",
        default="models/backfill/latent_sparse_support_residual_pathwise_residual_law_mean_reverting_covariance_mixture_structured_joint_student_t_187a/final_model.pt",
    )
    ...
    parser.add_argument(
        "--output_dir",
        default="results/validations/2026-04-06/analysis/187a_sparse_support_mechanistic",
    )
