#!/usr/bin/env python
"""
V1-style management report for the 183c/189a/190a pathwise residual-law family.

Purpose:
  - reuse the original V1 visual language that is easier to interpret
  - support newer flow-matching pathwise models that do not expose the old DDPM
    `sample()` API
  - add a dedicated mean-reversion / volatility-clustering figure for one strongly
    reverting cell
"""
...
from experiments.backfill.block_ar.train_169a_transformed_student_t import unconstrained_to_iv
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="V1-style management report for 183c-family models")
    parser.add_argument(
        "--model_path",
    ...
    parser.add_argument(
        "--output_dir",
        default="results/block_ar/management_report_183c_v1",
    )
    parser.add_argument(
        "--summary_path",
        default="results/block_ar/183c_best_v2_s3mrjspec_full_30d/summary.json",
        help="Optional strict benchmark summary for selecting a representative mean-reverting cell.",
    )
...
def load_model(model_path: str, device: str):
    ...
    if model_type == "state_metric_transport_pathwise_residual_law_mean_reverting_covariance_mixture_structured_joint_student_t_183c":
        ...
        model = StructuredConditionInterfaceModel(
            ...
            state_config=raw_config["state"],
            metric_config=raw_config["metric"],
            condition_interface_config=raw_config["condition_interface"],
            base_nu=raw_config.get("base_nu", 8.0),
            mix_chunk_size=raw_config.get("mix_chunk_size", 27),
            **common_kwargs,
        )
...
def generate_all_data(model, config: dict, device: str, n_samples: int, max_windows: int, batch_size: int):
    ...
    all_samples = []
    support_lo = config.get("support_lo", 0.01)
    support_hi = config.get("support_hi", 1.0)
    for start in tqdm(range(0, n_windows, batch_size), desc="Sampling"):
        ...
        with torch.no_grad():
            future_u = model.sample_future_u(hist, n_samples=n_samples)
            samples = unconstrained_to_iv(future_u, lo=support_lo, hi=support_hi)
        samples_np = samples.cpu().numpy().reshape(stop - start, n_samples, future_len, 5, 5)
...
def plot_mean_reversion_clustering(data: dict, output_dir: str, summary_path: str | None):
    ...
    full_days = np.arange(-len(history) + 1, len(future) + 1)
...
def main():
    ...
    print("Loading 183c-family model...")
