...
def build_losses(
    ...
    model: DualTimescaleLowRankTemporal,
    lambda_level: float,
    lambda_change: float,
    lambda_jump: float,
...
def warm_start_from_253a(model: DualTimescaleLowRankTemporal, checkpoint_path: str, device: torch.device) -> list[str]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="254a-v0 dual-timescale temporal model")
    ...
    cfg = DualTimescaleTemporalConfig(
        history_len=args.history_len,
        future_len=args.future_len,
        n_cells=D,
        latent_dim=args.L,
            ...
            for batch_idx, (hist_01, fut_flat) in enumerate(loader):
                ...
                if train_mode and global_step % args.log_every_n_steps == 0:
                    print(
                        ...
                        f"fast_gate={metrics['fast_gate_mean'].item():.4f} "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"val_fast_gate={val_avg.get('fast_gate_mean', 0):.4f}  "
