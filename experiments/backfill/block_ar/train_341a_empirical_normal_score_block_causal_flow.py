...
from diffusion.block_ar.empirical_normal_score_block_causal_flow_matching import (
    EmpiricalNormalScoreBlockCausalFMConfig,
    EmpiricalNormalScoreBlockCausalFlowMatching,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="341a empirical normal-score block-causal path flow"
    )
    ...
    parser.add_argument(
        ...
        default="prefix",
        help="Recorded for config compatibility; 341a block velocity always uses prefix tokens.",
    )
    ...
    model = EmpiricalNormalScoreBlockCausalFlowMatching(cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "val_daily_transition_std": val_avg["daily_transition_std"],
            "val_daily_transition_abs": val_avg["daily_transition_abs"],
            "val_block_transition_abs": val_avg["block_transition_abs"],
        ...
        print(
            ...
            f"daily_std={rec['val_daily_transition_std']:.3f} "
            f"daily_abs={rec['val_daily_transition_abs']:.3f} "
            f"block_abs={rec['val_block_transition_abs']:.3f} "
            f"vel_std={rec['val_target_velocity_std']:.3f} "
