#!/usr/bin/env python
"""
212y: H=1 minimal direct stochastic delta model with local-scale asinh innovation.

Same local-scale setup as 212x, but the decoder now models the transformed
innovation:
...
This tests whether the remaining 212x bottleneck is due to the innovation-law
geometry rather than the scale process itself.
"""
...
class MinimalDirectStochasticDeltaLocalScaleAsinhModel(nn.Module):
    def __init__(
        self,
        n_cells: int = 25,
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[MinimalDirectStochasticDeltaLocalScaleAsinhModel, dict[str, Any]]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = payload["config"]
    if cfg["type"] != "minimal_h1_direct_stochastic_delta_212y_local_scale_asinh":
        raise ValueError(f"Unexpected model type: {cfg['type']}")
    model = MinimalDirectStochasticDeltaLocalScaleAsinhModel(
        n_cells=cfg["n_cells"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="212y H=1 minimal direct stochastic delta local-scale asinh output")
    ...
    print("212y H=1 minimal direct stochastic delta local-scale asinh output")
    ...
    for epoch in range(1, args.epochs + 1):
        t0 = time.time()
        model.train()
        running = {"loss": 0.0, "energy": 0.0, "mae": 0.0, "delta_std": 0.0, "v_std": 0.0}
        count = 0
        ...
        payload = {
            ...
            "epoch": epoch,
            "config": {
                "type": "minimal_h1_direct_stochastic_delta_212y_local_scale_asinh",
                "n_cells": 25,
                "history_feat_dim": history_feat_dim,
                "hidden_dim": args.hidden_dim,
                "gru_layers": args.gru_layers,
