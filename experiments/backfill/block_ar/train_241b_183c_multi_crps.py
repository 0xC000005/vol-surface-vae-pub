#!/usr/bin/env python
"""
241b: 183c + multi-functional proper-scoring-rule fine-tune.
...
def sample_future_u_with_grad(
    ...
    """Mirrors sample_future_u() body WITHOUT @torch.no_grad on the ODE section.

    Returns IV samples in [support_lo, support_hi] range, shape (B, K, T, H, W).
    """
    # Teacher pathway — no grad (encoder/decoder/prior frozen anyway).
    with torch.no_grad():
        (
    ...
    # Everything below is linear algebra on tensors from the grad-enabled ODE output.
    # No trainable params past this point — but grad flows back to basis_paths.
            ...
            torch.save(
                {
                    ...
                    "config": {
                        "type": "241b_multi_crps_state_metric_transport",
