#!/usr/bin/env python
"""
161a: E2E CLN + Variogram Score (RC19-H3-S1)

Based on 158a (E2E CLN Transformer) with Variogram Score added.
VS directly penalizes pairwise dependency structure, providing gradient
signal that afCRPS (marginal-only) lacks.

Evidence: dualGNN (2509.02784) at D=18-30: VS outperforms ES for correlation.
At D=25, ~300 cell pairs provide strong signal.
...
def evaluate_model(encoder, mean_pred, cln_model, surfaces, start_indices,
    ...
    """Full evaluation on a set of windows, including turb/calm."""
...
def main():
    ...
    parser.add_argument("--lambda_vs", type=float, default=0.5,
                        help="Variogram score weight (dualGNN: 0.3-0.7 optimal at D=25)")
    ...
    print(f"\n{'='*60}")
    print(f"161a: E2E CLN + Variogram Score (RC19-H3-S1)")
    print(f"{'='*60}")
    ...
    print(f"  noise_dim={args.noise_dim}, d_model={args.d_model}, lambda_vs={args.lambda_vs}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for (idx_batch,) in train_loader:
            ...
            vs_loss = variogram_score(combined, gt) if args.lambda_vs > 0 else torch.tensor(0.0, device=device)
            loss = crps + args.lambda_is * is_loss + args.lambda_es * es_loss + args.lambda_vs * vs_loss
        ...
        if val_loss < best_val:
            ...
            torch.save({
                ...
                "config": {
                    ...
                    "type": "end_to_end_cln_vs_transformer",
