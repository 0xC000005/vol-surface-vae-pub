#!/usr/bin/env python
"""
221c: Warm-start multi-day AR conditional flow — fix GRU/cell LR mismatch.

Key change from 221b: GRU LR raised from 5e-5 to 1e-3 (same as recurrent cells).
221b diagnosis: evaluation uses GRU re-encoding (not recurrent cells), but GRU had
20x lower LR than cells, so learned dynamics were invisible at eval time. Fix: let
GRU and cells co-adapt at the same rate. Only protect flow (1e-5).

All other settings identical to 221b (dual-space ES, h=1 anchor, slow SS ramp, etc.)
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="221c warm-start multi-day AR flow (GRU/cell LR fix)")
    ...
    parser.add_argument("--lr_gru", type=float, default=1e-3)
    ...
    print(f"221c warm-start multi-day AR flow (GRU/cell LR fix)")
    ...
    config = {
        "type": "warmstart_multiday_ar_221c",
        "n_cells": cfg["n_cells"],
        "history_feat_dim": cfg["history_feat_dim"],
        "hidden_dim": cfg["hidden_dim"],
        "gru_layers": cfg["gru_layers"],
