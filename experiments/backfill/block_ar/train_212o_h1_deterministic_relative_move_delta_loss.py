#!/usr/bin/env python
"""
212o: deterministic H=1 point forecaster with relative-move output and delta loss.

Keeps the fully raw deterministic setup from 212n, but changes the decoder
parameterization:
  - head outputs per-cell relative move
  - delta is reconstructed as last_level * relative_move
  - training loss is still raw-delta MSE
"""
...
class DeterministicRelativeMoveDeltaLossModel(nn.Module):
    def __init__(
        self,
        n_cells: int = 25,
    ...
    def training_loss(self, history_01: torch.Tensor, target_01: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        per_window_mse = ((pred_delta - target_delta) ** 2).mean(dim=1)
        ...
        metrics = {
            ...
            "pred_abs_rel_move": rel_move.abs().mean().detach(),
        }
        return mean_mse, metrics


@torch.no_grad()
def evaluate_point_raw_mse(
    model: DeterministicRelativeMoveDeltaLossModel,
    loader: DataLoader,
    q95_threshold: float,
    q99_threshold: float,
) -> dict[str, float]:
    ...
    for history_01, target_01 in loader:
        ...
        target_delta = target_01 - prev
        rel_move = model.predict_relative_move(history_01)
        pred_delta = prev * rel_move
        per_window_mse = ((pred_delta - target_delta) ** 2).mean(dim=1)
        mean_mse = per_window_mse.mean()
        ...
        total["val_pred_abs_rel_move"] += float(rel_move.abs().mean().item()) * batch_size
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[DeterministicRelativeMoveDeltaLossModel, dict[str, Any]]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = payload["config"]
    if cfg["type"] != "minimal_h1_deterministic_relative_move_212o":
...
def main() -> None:
    parser = argparse.ArgumentParser(description="212o H=1 deterministic relative-move output + raw-delta MSE")
    ...
    print("212o H=1 deterministic relative-move output + raw-delta MSE")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_metrics = evaluate_point_raw_mse(
            model,
            val_loader,
        ...
        payload = {
            ...
            "epoch": epoch,
            "config": {
                "type": "minimal_h1_deterministic_relative_move_212o",
                "n_cells": 25,
                "history_feat_dim": 50,
                "hidden_dim": args.hidden_dim,
                "gru_layers": args.gru_layers,
        ...
        print(
            ...
            f"MSE={train_metrics['train_mean_mse']:.6f}  "
