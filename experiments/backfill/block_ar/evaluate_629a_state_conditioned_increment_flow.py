#!/usr/bin/env python
"""629a: score state-conditioned increment checkpoints on the IV 11-suite."""
...
def generate_iv_samples(
    ...
    *,
    ...
    for start in range(0, int(history_level.shape[0]), int(batch_size)):
        ...
        hist_level = torch.from_numpy(history_level[start:end]).to(device)
        hist_increment = torch.from_numpy(history_increment[start:end]).to(device)
...
def summarize_results(results: dict[str, Any], alignment: dict[str, float]) -> list[str]:
    ...
    return [
        "- source: `629a state-conditioned encoded-increment transition flow`",
...
def main() -> None:
    ...
    results["config"] = {
        "mode": "629a_state_conditioned_increment_flow",
