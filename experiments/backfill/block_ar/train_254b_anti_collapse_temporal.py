...
Second 254-family experiment after 254a-v0 over-collapsed into a near-rank-1 common path.
"""
...
from diffusion.block_ar.anti_collapse_dual_timescale_temporal import (
    AntiCollapseDualTimescaleTemporal,
    AntiCollapseDualTimescaleConfig,
    config_to_dict,
)
from diffusion.block_ar.dual_timescale_low_rank_temporal import load_model as load_254a_model
...
def build_losses(
    ...
    model: AntiCollapseDualTimescaleTemporal,
    lambda_level: float,
    lambda_change: float,
    lambda_jump: float,
    lambda_terminal: float,
    lambda_ortho: float,
    lambda_slow_smooth: float,
    ...
    slow_diff = aux["slow_latent"][:, 1:] - aux["slow_latent"][:, :-1]
    slow_smooth = slow_diff.pow(2).mean()
    spectral_penalty, top1_share, eff_rank = model.spectral_collapse_penalty(aux["dynamic_loadings"])
    ...
    total = (
        ...
        + lambda_ortho * ortho
        + lambda_slow_smooth * slow_smooth
        + lambda_spectral * spectral_penalty
    )
    metrics = {
        ...
        "ortho": ortho.detach(),
        "slow_smooth": slow_smooth.detach(),
        "spectral_penalty": spectral_penalty.detach(),
        "top1_share": top1_share.detach(),
...
def warm_start_from_254a(model: AntiCollapseDualTimescaleTemporal, checkpoint_path: str, device: torch.device) -> list[str]:
    old_model, _ = load_254a_model(checkpoint_path, device)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="254b anti-collapse temporal model")
    ...
    parser.add_argument("--slow_loading_scale", type=float, default=0.15)
    parser.add_argument("--fast_loading_scale", type=float, default=0.20)
    parser.add_argument("--top1_share_target", type=float, default=0.55)
    parser.add_argument("--rank_floor", type=float, default=3.0)
    ...
    parser.add_argument("--lambda_slow_smooth", type=float, default=1e-3)
    ...
    parser.add_argument("--init_from", type=str, default="models/backfill/254a_v0_L8_s42/best_model.pt")
    ...
    cfg = AntiCollapseDualTimescaleConfig(
        history_len=args.history_len,
        future_len=args.future_len,
        n_cells=D,
        latent_dim=args.L,
        ...
        slow_loading_scale=args.slow_loading_scale,
        fast_loading_scale=args.fast_loading_scale,
        top1_share_target=args.top1_share_target,
        rank_floor=args.rank_floor,
    )
    model = AntiCollapseDualTimescaleTemporal(cfg).to(device)
    copied = warm_start_from_254a(model, args.init_from, device)
            ...
            for batch_idx, (hist_01, fut_flat) in enumerate(loader):
                ...
                with autocast_ctx():
                    ...
                    loss, metrics = build_losses(
                        ...
                        lambda_slow_smooth=args.lambda_slow_smooth,
                ...
                if train_mode and global_step % args.log_every_n_steps == 0:
                    print(
                        ...
                        f"top1={metrics['top1_share'].item():.4f} "
                        f"rank={metrics['eff_rank'].item():.3f} "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"val_top1={val_avg.get('top1_share', 0):.4f}  "
            f"val_rank={val_avg.get('eff_rank', 0):.3f}  "
