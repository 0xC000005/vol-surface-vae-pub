#!/usr/bin/env python
"""628a: score encoded-increment AR transition checkpoints on the IV 11-suite."""
...
from diffusion.block_ar.generic_empirical_score_transition_flow_matching import load_model  # noqa: E402
...
def build_val_block(args: argparse.Namespace, payload: dict[str, Any]) -> tuple[np.ndarray, np.ndarray, list[Any], Any]:
    ...
    history_increment, _future_increment, history_state, _future_state, specs = select_increment_scope(
        block,
        scope,
        int(args.iv_count),
    )
    ...
    return history_increment.astype(np.float32), history_state.astype(np.float32), specs, block
...
def generate_iv_samples(
    ...
    *,
    ...
    for start in range(0, int(history_increment.shape[0]), int(batch_size)):
        end = min(start + int(batch_size), int(history_increment.shape[0]))
        hist = torch.from_numpy(history_increment[start:end]).to(device)
        ...
        panel_arr = reconstruct_state_from_increments(history_state[start:end, -1, :], increment_arr, specs)
        arr = panel_arr[..., :iv_count]
        chunks.append(arr.reshape(end - start, int(samples), int(n_steps), 5, 5).astype(np.float32))
        print(f"  generated windows {end}/{history_increment.shape[0]}", flush=True)
...
def summarize_results(results: dict[str, Any], alignment: dict[str, float]) -> list[str]:
    ...
    return [
        "- source: `628a encoded-increment generic AR transition flow`",
...
def main() -> None:
    ...
    history_increment, history_state, specs, block = build_val_block(args, payload)
    cfg = payload["config"]
    n_windows = min(int(args.max_windows), int(history_increment.shape[0]))
    history_increment = history_increment[:n_windows]
    history_state = history_state[:n_windows]
    ...
    if alignment["history_max_abs_error"] > 1e-6 or alignment["future_max_abs_error"] > 1e-6:
        raise RuntimeError(f"628a/full-suite alignment failed: {alignment}")
    ...
    results["config"] = {
        "mode": "628a_unified_ar_increment_transition_flow",
        ...
        "model_coordinate": payload.get("model_coordinate", "encoded_increment"),
    ...
    write_markdown_summary(
        out_md,
        "628a Encoded-Increment AR Transition Flow Official IV 11-Suite",
