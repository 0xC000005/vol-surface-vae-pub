...
from diffusion.block_ar.recurrent_latent_state_future_logit_path_flow import (
    RecurrentLatentStateFutureLogitPathFlow,
    RecurrentLatentStateFutureLogitPathFlowConfig,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="328a-v0 recurrent latent state future-logit path flow"
    )
    parser.add_argument("--context_dim", type=int, default=160)
    parser.add_argument("--history_hidden", type=int, default=96)
    parser.add_argument("--encoder_dropout", type=float, default=0.1)
    parser.add_argument("--latent_tokens", type=int, default=12)
    ...
    model = RecurrentLatentStateFutureLogitPathFlow(cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "val_recon_mse": val_avg["recon_mse"],
            "val_latent_fm": val_avg["latent_fm"],
            "val_target_std": val_avg["target_std"],
            "val_recon_std": val_avg["recon_std"],
            "val_target_delta_std": val_avg["target_delta_std"],
            "val_recon_delta_std": val_avg["recon_delta_std"],
            "val_latent_std": val_avg["latent_std"],
            "val_latent_abs": val_avg["latent_abs"],
        ...
        print(
            ...
            f"recon={rec['val_recon_mse']:.5f} latent_fm={rec['val_latent_fm']:.5f} "
            f"target_std={rec['val_target_std']:.3f} recon_std={rec['val_recon_std']:.3f} "
            f"target_dstd={rec['val_target_delta_std']:.3f} recon_dstd={rec['val_recon_delta_std']:.3f} "
            f"latent_std={rec['val_latent_std']:.3f} lr={rec['lr']:.2e} time={rec['sec']:.1f}s"
        )
