#!/usr/bin/env python
"""
164a_v3_percell_bptt_softplus: Per-Cell CLN + BPTT + Softplus Floor Barrier (RC20.6)

Adds softplus floor barrier to the per-step loss: tau * softplus(-frame_t / tau).
Encodes the mathematical support constraint IV >= 0. lambda_floor=2.5 is gradient-
matched at 10% budget (Codex #4: base grad 0.2384, floor grad 0.00982, ratio 24.3x).

4 Codex verifications on design:
  1. Softplus over margin/bare-ReLU (generic, no artificial margin)
  2. Reflecting boundary analogy mechanically false (loss vs forward-pass)
  3. Lambda sweep unprincipled → gradient matching
  4. lambda=2.5 from 10% gradient budget on saved BPTT model
...
Usage:
    PYTHONPATH=. python experiments/backfill/block_ar/train_164a_v3_percell_bptt_softplus.py \
        ...
        --output_dir models/backfill/afcrps_164a_v3_percell_bptt_softplus --device cuda
"""
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # ── Save best model ──
        config_dict = {
            "type": "ar_spatial_transformer_164a_v3_percell_bptt_softplus",
            "encoder": {
        ...
        print(f"Ep {epoch:3d}  loss={tl:.4f}  val={val_loss:.4f}  "
              ...
              f"floor={tfloor:.6f}  lf={lambda_floor_eff:.2f}  ({elapsed:.1f}s)"
