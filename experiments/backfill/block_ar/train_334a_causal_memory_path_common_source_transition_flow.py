...
from diffusion.block_ar.causal_memory_path_common_source_transition_flow_matching import (
    CausalMemoryPathCommonSourceTransitionFMConfig,
    CausalMemoryPathCommonSourceTransitionFlowMatching,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="334a causal memory path-common-source transition FM"
    )
    ...
    parser.add_argument("--no_standardize_logits", action="store_true")
    ...
    cfg = CausalMemoryPathCommonSourceTransitionFMConfig(
        ...
        standardize_logits=not args.no_standardize_logits,
        logit_eps=args.logit_eps,
        logit_std_floor=args.logit_std_floor,
        flow_steps=args.flow_steps,
        sample_temperature=args.sample_temperature,
    )
    model = CausalMemoryPathCommonSourceTransitionFlowMatching(cfg).to(device)
    if cfg.standardize_logits:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "train_source_std": train_avg["source_std"],
            "train_source_common_std": train_avg["source_common_std"],
            ...
            "val_source_std": val_avg["source_std"],
            "val_source_common_std": val_avg["source_common_std"],
        ...
        print(
            ...
            f"source_std={rec['val_source_std']:.3f} "
            f"common_std={rec['val_source_common_std']:.3f} "
