...
from diffusion.block_ar.ar_seq2seq_transformer_joint_token_backbone import (
    ARSeq2SeqTransformerJointTokenBackbone,
    ARSeq2SeqTransformerJointTokenConfig,
    save_checkpoint,
)
...
def build_joint_codebook(
    ...
    coords = target_coords(hist_norm, fut_norm, change_scale_eps).reshape(-1, hist_norm.shape[-1]).cpu()
    ...
    if k < codebook_size:
        extra = coords[torch.randint(0, n, (codebook_size - k,))]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="276b-v0 deterministic joint-token next-change seq2seq backbone"
    )
    ...
    model = ARSeq2SeqTransformerJointTokenBackbone(cfg, codebook.to(device)).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "train_token_ce": train_avg["token_ce"],
            "train_token_acc": train_avg["token_acc"],
            "val_total": val_avg["total"],
            "val_token_ce": val_avg["token_ce"],
