#!/usr/bin/env python
"""
194c: True 2-state quiet/event AR latent-factor Student-t model.

Targeted simplification after 194b:
  - keep the same AR regime-switching latent-factor class
  - collapse the ambiguous 3-state path into a true quiet/event split
  - keep exact teacher-forced marginalization and semantic quiet/event alignment
  - remove the mid-state failure mode where turbulence leaks into the wrong broad state

Usage:
    PYTHONPATH=. python experiments/backfill/block_ar/train_194c_two_state_quiet_event.py \
        --output_dir models/backfill/regime_switching_ar_latent_factor_194c --device cuda
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="194c: true 2-state quiet/event AR latent-factor Student-t")
    ...
    parser.add_argument("--warm_start", type=str, default="models/backfill/regime_switching_ar_latent_factor_194b/best_model.pt")
    ...
    print(f"\n{'=' * 64}")
    print("194c: True 2-State Quiet/Event Regime-Switching AR Latent-Factor Student-t")
    print(f"{'=' * 64}")
            ...
            torch.save(
                {
                    ...
                    "config": {
                        "type": "regime_switching_ar_latent_factor_194c",
