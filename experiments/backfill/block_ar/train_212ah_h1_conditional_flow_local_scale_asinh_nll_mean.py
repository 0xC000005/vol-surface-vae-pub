#!/usr/bin/env python
"""
212ah: 212ae + exact flow NLL auxiliary + raw-delta sample-mean auxiliary.
"""
...
class ConditionalFlowLocalScaleAsinhNLLMeanModel(ConditionalFlowLocalScaleAsinhModel):
    def training_loss(
        self,
        ...
        mean_l1 = (sample_delta.mean(dim=1) - target_delta).abs().mean()
        loss = es + nll_weight * nll + mean_weight * mean_l1
        metrics = {
            "energy": es.detach(),
            "nll": nll.detach(),
...
) -> tuple[ConditionalFlowLocalScaleAsinhNLLMeanModel, dict[str, Any]]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = payload["config"]
    if cfg["type"] != "minimal_h1_conditional_flow_212ah_local_scale_asinh_nll_mean":
        raise ValueError(f"Unexpected model type: {cfg['type']}")
    model = ConditionalFlowLocalScaleAsinhNLLMeanModel(
        n_cells=cfg["n_cells"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="212ah H=1 conditional flow with exact NLL and sample-mean auxiliaries")
    ...
    print("212ah H=1 conditional flow with exact NLL and sample-mean auxiliaries")
    print(f"  train_windows={train_hist.shape[0]} val_windows={val_hist.shape[0]}")
    print(
        f"  n_coupling_layers={args.n_coupling_layers} train_samples={args.train_samples} "
        f"nll_weight={args.nll_weight} mean_weight={args.mean_weight}"
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        payload = {
            ...
            "epoch": epoch,
            "config": {
                "type": "minimal_h1_conditional_flow_212ah_local_scale_asinh_nll_mean",
                "n_cells": 25,
                "history_feat_dim": history_feat_dim,
                "hidden_dim": args.hidden_dim,
                "gru_layers": args.gru_layers,
