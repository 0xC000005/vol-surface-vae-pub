#!/usr/bin/env python
"""
184d_v0: Latent activity operator mixture on top of the 183c/184b backbone.

Principle:
  - keep the explicit mean/covariance/pathwise residual backbone
  - keep the latent activity process
  - replace the free multiplicative sparse gate with a true quiet-vs-event
    operator mixture
  - train the event operator with posterior-guided event targets so the event
    branch cannot simply collapse to zero while the quiet backbone explains
    everything
"""
...
def sparsemax(logits: torch.Tensor, dim: int = -1) -> torch.Tensor:
    ...
    rhos = torch.arange(1, z.size(dim) + 1, device=logits.device, dtype=logits.dtype)
    view_shape = [1] * z.ndim
    view_shape[dim] = z.size(dim)
    rhos = rhos.view(view_shape)
    support = z_sorted > (z_cumsum / rhos)
    k = support.sum(dim=dim, keepdim=True).clamp_min(1)
    tau = z_cumsum.gather(dim, k.long() - 1) / k.to(logits.dtype)
    return torch.clamp(z - tau, min=0.0)


def masked_sparsemax(logits: torch.Tensor, mask: torch.Tensor, dim: int = -1) -> torch.Tensor:
    masked_logits = logits.masked_fill(~mask, -1e9)
    probs = sparsemax(masked_logits, dim=dim) * mask.to(dtype=logits.dtype)
    return probs / probs.sum(dim=dim, keepdim=True).clamp_min(1e-8)


class NodeBudgetHead(nn.Module):
...
class NeighborhoodKernelHead(nn.Module):
    def __init__(self, hidden_dim: int, kernel_dim: int):
        super().__init__()
        ...
        for layer in (self.query[-1], self.key[-1]):
    ...
    def forward(self, features: torch.Tensor, neighbor_mask: torch.Tensor) -> torch.Tensor:
        ...
        k = self.key(features)
        scores = torch.einsum("btid,btjd->btij", q, k) / float(self.kernel_dim) ** 0.5
        mask = neighbor_mask.to(device=scores.device, dtype=torch.bool).unsqueeze(0).unsqueeze(0)
        return masked_sparsemax(scores, mask, dim=-1)
...
class LatentActivityOperatorMixtureModel(StateMetricTransportModel):
    def __init__(
        self,
        ...
        self.precision_config = dict(precision_config)
        self.mixture_config = dict(mixture_config)
        ...
        self.node_budget_head = NodeBudgetHead(self.activity_config["allocator_hidden_dim"])
        self.neighborhood_kernel_head = NeighborhoodKernelHead(
            hidden_dim=self.precision_config["kernel_hidden_dim"],
            kernel_dim=self.precision_config["kernel_dim"],
        )
    ...
    def build_block_event_targets(self, target_local_log: torch.Tensor) -> torch.Tensor:
        teacher = self.build_block_teacher_inputs(target_local_log)
        concentration = teacher[..., 4]
        abs_mean = teacher[..., 2]
        abs_max = teacher[..., 3]
        conc_target = torch.sigmoid(
            (concentration - self.mixture_config["event_concentration_center"])
            / self.mixture_config["event_concentration_scale"]
        )
        amp_target = torch.sigmoid(
            (abs_max - self.mixture_config["event_absmax_center"])
            / self.mixture_config["event_absmax_scale"]
        )
        mean_target = torch.sigmoid(
            (abs_mean - self.mixture_config["event_absmean_center"])
            / self.mixture_config["event_absmean_scale"]
        )
        return (conc_target * amp_target * mean_target).clamp(0.02, 0.98)
    ...
    def build_activity_metric_controls(
        ...
        node_features = self.build_node_features(state_local, quiet_metric_local)
        node_logits = self.node_budget_head(node_features) + router_map.clamp_min(1e-6).log()
        full_mask = torch.ones_like(node_logits, dtype=torch.bool)
        node_alloc = masked_sparsemax(node_logits, full_mask, dim=-1)

        neighbor_mask = self.activity_geometry.local_adjacency(include_self=False).bool()
        kernel = self.neighborhood_kernel_head(node_features, neighbor_mask)
        neighbor_local = self.activity_geometry.aggregate_local(kernel, quiet_metric_local)
        event_scale = (self.precision_config["node_budget_scale"] * activity_budget).unsqueeze(-1)
        node_strength = event_scale * node_alloc
        precision_delta = node_strength * (quiet_metric_local - neighbor_local)
        event_metric_local = quiet_metric_local + self.precision_config["metric_delta_scale"] * precision_delta
        event_metric_local = event_metric_local - event_metric_local.mean(dim=(1, 2), keepdim=True)
        event_metric_local = event_metric_local.clamp(
            min=-self.metric_config["local_metric_clip"],
            max=self.metric_config["local_metric_clip"],
        )
        mix = activity_gate.unsqueeze(-1)
        metric_local = (1.0 - mix) * quiet_metric_local + mix * event_metric_local
    ...
    def transport_velocity_with_pack(
        ...
        neighbor_velocity = self.activity_geometry.aggregate_local(kernel, raw_v_basis)
        event_raw_basis = (
            raw_v_basis
            + self.precision_config["velocity_delta_scale"] * node_strength * (raw_v_basis - neighbor_velocity)
        )
        quiet_v_basis = self.modulate_velocity_basis(raw_v_basis, quiet_metric_local, metric_band)
        event_v_basis = self.modulate_velocity_basis(event_raw_basis, event_metric_local, metric_band)
        mix = activity_gate.unsqueeze(-1)
        mod_v_basis = (1.0 - mix) * quiet_v_basis + mix * event_v_basis
        flat_alloc = node_alloc.reshape(node_alloc.shape[0], -1)
        flat_kernel = kernel.reshape(kernel.shape[0] * kernel.shape[1] * kernel.shape[2], kernel.shape[3])
        ...
        metrics = {
            ...
            "event_metric_local_abs_mean": event_metric_local.abs().mean(),
            "metric_local_abs_mean": metric_local.abs().mean(),
            "precision_delta_abs_mean": precision_delta.abs().mean(),
            "node_strength_mean": node_strength.mean(),
            ...
            "activity_node_top1_mean": flat_alloc.max(dim=1).values.mean(),
            "activity_node_entropy_mean": (-(flat_alloc * flat_alloc.clamp_min(1e-8).log()).sum(dim=1)).mean(),
            "kernel_top1_mean": flat_kernel.max(dim=1).values.mean(),
            "kernel_entropy_mean": (-(flat_kernel * flat_kernel.clamp_min(1e-8).log()).sum(dim=1)).mean(),
...
def latent_activity_operator_mixture_flow_matching_loss(
    model: LatentActivityOperatorMixtureModel,
    ...
    _, _, quiet_metric_local, _, _, _ = super(
        LatentActivityOperatorMixtureModel, model
    ...
    event_local_loss = F.smooth_l1_loss(event_metric_local, target_local_log)
    ...
    target_support = topk_soft_allocation_masked(
        target_local_log.abs(),
        full_mask,
        model.activity_config["teacher_topk"],
        model.activity_config["teacher_temperature"],
    )
    event_target_block = model.build_block_event_targets(target_local_log)
    event_target = model._expand_block_values(event_target_block)
    support_weight = event_target.unsqueeze(-1)
    support_norm = support_weight.mean().clamp_min(0.05)
    support_loss = (
        F.smooth_l1_loss(node_alloc, target_support, reduction="none") * support_weight
    ).mean() / support_norm

    target_neighbor = model.activity_geometry.aggregate_local(kernel, target_local_log)
    contrast_loss = (
        F.smooth_l1_loss(
            event_metric_local - neighbor_local,
            target_local_log - target_neighbor,
            reduction="none",
        )
        * support_weight
    ).mean() / support_norm

    node_entropy = (-(node_alloc * node_alloc.clamp_min(1e-8).log()).sum(dim=-1) * event_target).mean()
    kernel_entropy = (
        (-(kernel * kernel.clamp_min(1e-8).log()).sum(dim=-1) * node_alloc).sum(dim=-1) * event_target
    )
    kernel_entropy = kernel_entropy.mean()
    ...
    posterior_event_bce = F.binary_cross_entropy(post_gate, event_target_block)
    prior_event_bce = F.binary_cross_entropy(prior_gate, event_target_block)

    total = (
        fm_loss
        + model.integrated_config["local_loss_weight"] * local_loss
        + model.mixture_config["event_local_loss_weight"] * event_local_loss
        ...
        + model.precision_config["contrast_loss_weight"] * contrast_loss
        + model.precision_config["node_entropy_weight"] * node_entropy
        + model.precision_config["kernel_entropy_weight"] * kernel_entropy
        ...
        + model.mixture_config["posterior_event_bce_weight"] * posterior_event_bce
        + model.mixture_config["prior_event_bce_weight"] * prior_event_bce
    )
    ...
    metrics = {
        ...
        "sp_total_loss": total,
        "sp_local_loss": local_loss,
        "sp_event_local_loss": event_local_loss,
        "sp_band_loss": band_loss,
        "sp_smooth_reg": smooth_reg,
        "sp_budget_reg": budget_reg,
        "sp_support_loss": support_loss,
        "sp_contrast_loss": contrast_loss,
        "sp_node_entropy": node_entropy,
        "sp_kernel_entropy": kernel_entropy,
        "sp_activity_gate_kl": activity_gate_kl,
        "sp_activity_budget_consistency": activity_budget_consistency,
        "sp_activity_group_kl": activity_group_kl,
        "sp_posterior_event_bce": posterior_event_bce,
        "sp_prior_event_bce": prior_event_bce,
        "target_local_abs_mean": target_local_log.abs().mean(),
        "target_band_high_mean": target_band_log[:, 2].mean(),
        "target_event_rate": event_target_block.mean(),
...
def evaluate_teacher_forced(
    ...
    keys = [
        ...
        "sp_kernel_entropy",
        "sp_activity_gate_kl",
        "sp_activity_budget_consistency",
        "sp_activity_group_kl",
        "sp_posterior_event_bce",
        "sp_prior_event_bce",
    ...
    for history_01, future_01 in val_loader:
        ...
        _loss, metrics = latent_activity_operator_mixture_flow_matching_loss(model, history_01, future_01)
    ...
    out["val_total_loss"] = out["val_sp_total_loss"]
    return out


def main() -> None:
    parser = argparse.ArgumentParser(description="184d_v0: latent activity operator mixture")
    ...
    parser.add_argument("--kernel_hidden_dim", type=int, default=96)
    parser.add_argument("--kernel_dim", type=int, default=16)
    parser.add_argument("--node_budget_scale", type=float, default=6.0)
    parser.add_argument("--metric_delta_scale", type=float, default=1.0)
    parser.add_argument("--velocity_delta_scale", type=float, default=0.75)
    parser.add_argument("--contrast_loss_weight", type=float, default=0.30)
    parser.add_argument("--node_entropy_weight", type=float, default=0.01)
    parser.add_argument("--kernel_entropy_weight", type=float, default=0.01)
    parser.add_argument("--event_concentration_center", type=float, default=0.34)
    parser.add_argument("--event_concentration_scale", type=float, default=0.08)
    parser.add_argument("--event_absmax_center", type=float, default=0.18)
    parser.add_argument("--event_absmax_scale", type=float, default=0.05)
    parser.add_argument("--event_absmean_center", type=float, default=0.08)
    parser.add_argument("--event_absmean_scale", type=float, default=0.03)
    parser.add_argument("--event_local_loss_weight", type=float, default=0.35)
    parser.add_argument("--posterior_event_bce_weight", type=float, default=0.40)
    parser.add_argument("--prior_event_bce_weight", type=float, default=0.12)
    ...
    precision_config = dict(
        ...
        node_budget_scale=args.node_budget_scale,
        metric_delta_scale=args.metric_delta_scale,
        velocity_delta_scale=args.velocity_delta_scale,
        contrast_loss_weight=args.contrast_loss_weight,
        node_entropy_weight=args.node_entropy_weight,
        kernel_entropy_weight=args.kernel_entropy_weight,
    )
    mixture_config = dict(
        event_concentration_center=args.event_concentration_center,
        event_concentration_scale=args.event_concentration_scale,
        event_absmax_center=args.event_absmax_center,
        event_absmax_scale=args.event_absmax_scale,
        event_absmean_center=args.event_absmean_center,
        event_absmean_scale=args.event_absmean_scale,
        event_local_loss_weight=args.event_local_loss_weight,
        posterior_event_bce_weight=args.posterior_event_bce_weight,
        prior_event_bce_weight=args.prior_event_bce_weight,
    )
    ...
    def set_stage(stage: int):
        activity_params = (
            ...
            + list(model.node_budget_head.parameters())
            + list(model.neighborhood_kernel_head.parameters())
        )
        if stage == 1:
            ...
            stage_name = "latent-operator-ctrl"
        else:
            ...
            stage_name = "latent-operator-joint"
    ...
    print(f"\n{'=' * 72}\n184d_v0: latent activity operator mixture\n{'=' * 72}")
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for history_01, future_01 in train_loader:
            ...
            loss, metrics = latent_activity_operator_mixture_flow_matching_loss(model, history_01, future_01)
            ...
            torch.save(
                {
                    ...
                    "config": {
                        "type": "latent_activity_operator_mixture_pathwise_residual_law_mean_reverting_covariance_mixture_structured_joint_student_t_184d",
        ...
        print(
            ...
            f"nodeTop={val_metrics['val_activity_node_top1_mean']:.3f}  kernTop={val_metrics['val_kernel_top1_mean']:.3f}  "
