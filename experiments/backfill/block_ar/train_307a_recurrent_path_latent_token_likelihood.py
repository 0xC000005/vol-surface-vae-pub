...
from diffusion.block_ar.recurrent_path_latent_token_likelihood import (
    RecurrentPathLatentTokenLikelihood,
    RecurrentPathLatentTokenLikelihoodConfig,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="307a-v0 recurrent path-latent token likelihood")
    ...
    parser.add_argument("--max_df", type=float, default=20.0)
    ...
    model = RecurrentPathLatentTokenLikelihood(cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"nll={rec['val_nll']:.5f} kl={rec['val_kl']:.5f} "
            f"scale={rec['val_scale_mean']:.3f} df={rec['val_df_mean']:.2f} "
            f"latent={rec['val_latent_abs']:.3f} lr={rec['lr']:.2e} time={rec['sec']:.1f}s"
        )
