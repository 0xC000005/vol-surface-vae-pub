#!/usr/bin/env python
"""538a: train an autoregressive daily mixture transition law over IV+factor panels."""
...
from diffusion.block_ar.panel_daily_mixture_transition_model import (  # noqa: E402
...
def main() -> None:
    ...
    model = PanelDailyMixtureTransitionModel(cfg).to(device)
    ...
    print(f"Mixture components: {args.n_components}; daily Cholesky dim: {model.n_tril}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "train_component_scale_mean": train_avg["component_scale_mean"],
            "train_mixture_entropy": train_avg["mixture_entropy"],
            "val_nll": val_avg["nll"],
            "val_innovation_mae": val_avg["innovation_mae"],
            "val_diag_mean": val_avg["diag_mean"],
            "val_offdiag_abs": val_avg["offdiag_abs"],
            "val_component_scale_mean": val_avg["component_scale_mean"],
            "val_innovation_std": val_avg["innovation_std"],
            "val_whitened_std": val_avg["whitened_std"],
            "val_mixture_entropy": val_avg["mixture_entropy"],
        ...
        print(
            ...
            f"diag={rec['val_diag_mean']:.3f} scale={rec['val_component_scale_mean']:.3f} "
            f"entropy={rec['val_mixture_entropy']:.3f} white={rec['val_whitened_std']:.3f} "
