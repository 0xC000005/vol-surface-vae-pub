#!/usr/bin/env python
"""
212ag: 212ae + small raw-delta sample-mean auxiliary on local-scale asinh
innovation space.
"""
...
class ConditionalFlowLocalScaleAsinhMeanAuxModel(ConditionalFlowLocalScaleAsinhModel):
    def training_loss(
        self,
        ...
        sample_mean_delta = sample_delta.mean(dim=1)
        mean_l1 = (sample_mean_delta - target_delta).abs().mean()
        loss = es + mean_weight * mean_l1
        metrics = {
            "energy": es.detach(),
...
) -> tuple[ConditionalFlowLocalScaleAsinhMeanAuxModel, dict[str, Any]]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = payload["config"]
    if cfg["type"] != "minimal_h1_conditional_flow_212ag_local_scale_asinh_mean_aux":
        raise ValueError(f"Unexpected model type: {cfg['type']}")
    model = ConditionalFlowLocalScaleAsinhMeanAuxModel(
        n_cells=cfg["n_cells"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="212ag H=1 conditional flow with sample-mean auxiliary")
    ...
    parser.add_argument("--mean_weight", type=float, default=5.0)
    ...
    print("212ag H=1 conditional flow with sample-mean auxiliary")
    print(f"  train_windows={train_hist.shape[0]} val_windows={val_hist.shape[0]}")
    print(f"  n_coupling_layers={args.n_coupling_layers} train_samples={args.train_samples} mean_weight={args.mean_weight}")
    ...
    for epoch in range(1, args.epochs + 1):
        t0 = time.time()
        model.train()
        running = {"loss": 0.0, "energy": 0.0, "mean_l1": 0.0, "sample_v_std": 0.0, "sample_delta_std": 0.0}
        count = 0
        ...
        for history_01, target_01 in train_loader:
            ...
            running["mean_l1"] += float(metrics["mean_l1"].item()) * batch
        ...
        payload = {
            ...
            "epoch": epoch,
            "config": {
                "type": "minimal_h1_conditional_flow_212ag_local_scale_asinh_mean_aux",
                "n_cells": 25,
                "history_feat_dim": history_feat_dim,
                "hidden_dim": args.hidden_dim,
                "gru_layers": args.gru_layers,
        ...
        print(
            ...
            f"meanL1={train_metrics['train_mean_l1']:.4f}  "
