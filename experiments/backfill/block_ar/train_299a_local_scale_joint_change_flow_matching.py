...
from diffusion.block_ar.local_scale_joint_change_flow_matching import (
    LocalScaleJointChangeFMConfig,
    LocalScaleJointChangeFlowMatching,
    save_checkpoint,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="299a-v0 continuous local-scale joint future-change flow matching"
    )
    ...
    model = LocalScaleJointChangeFlowMatching(cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "train_coord_std": train_avg["coord_std"],
            "train_coord_abs": train_avg["coord_abs"],
            "train_target_velocity_std": train_avg["target_velocity_std"],
            "val_total": val_avg["total"],
            "val_coord_std": val_avg["coord_std"],
            "val_coord_abs": val_avg["coord_abs"],
        ...
        print(
            ...
            f"coord_std={rec['val_coord_std']:.3f} "
            f"coord_abs={rec['val_coord_abs']:.3f} "
            f"lr={rec['lr']:.2e} "
            f"time={rec['sec']:.1f}s"
        )
