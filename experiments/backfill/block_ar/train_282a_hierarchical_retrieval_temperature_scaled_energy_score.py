...
def weighted_energy_score(
    ...
    diff_to_target = torch.linalg.vector_norm(
        scenarios_flat - target_flat.unsqueeze(1), dim=-1
    )  # [B,K]
    term1 = (weights * diff_to_target).sum(dim=1)

    pairwise = torch.linalg.vector_norm(
        scenarios_flat.unsqueeze(2) - scenarios_flat.unsqueeze(1), dim=-1
    )  # [B,K,K]
    weight_outer = weights.unsqueeze(2) * weights.unsqueeze(1)
    term2 = 0.5 * (weight_outer * pairwise).sum(dim=(1, 2))
...
def epoch_pass(
    ...
    for history_norm, future_01 in loader:
        ...
        loss = weighted_energy_score(scenarios_flat, weights, target_flat)
        if training:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="282a-v0 temperature+scale hierarchy trained with weighted energy score")
    ...
    save_payload = {
        ...
        "training_objective": "weighted_energy_score",
    }
