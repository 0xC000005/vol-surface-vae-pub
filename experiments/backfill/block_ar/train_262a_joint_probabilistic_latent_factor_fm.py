...
from diffusion.block_ar.joint_probabilistic_latent_factor_fm import (
    JointProbabilisticLatentFactorFM,
    JointProbabilisticLatentFactorFMConfig,
    save_checkpoint,
)
...
def fm_step(
    ...
    *,
    ...
    target_coord = model.transform_change(raw_target_change, history_norm)
    cond = model.condition(history_norm)

    z_target, eps_target = model.target_factors(target_coord, cond)
    ...
    change_loss = F.smooth_l1_loss(cond["mean_coord"], target_coord)
    ...
    common_rms = cond["mean_common"].pow(2).mean().sqrt()
    idio_rms = cond["mean_idio"].pow(2).mean().sqrt()
    metrics = {
        "total": loss.detach(),
        ...
        "idio_ratio": (idio_rms / (common_rms + 1e-8)).detach(),
    }
    return loss, metrics
...
def main() -> None:
    parser = argparse.ArgumentParser(description="262a-v0 joint probabilistic latent-factor FM")
    ...
    parser.add_argument("--cond_layers", type=int, default=3)
    ...
    parser.add_argument("--max_idio_ratio", type=float, default=0.20)
    ...
    model = JointProbabilisticLatentFactorFM(cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"chg={rec['val_change']:.4f} "
            f"lvl={rec['val_level']:.4f} "
            f"sigma={rec['val_sigma']:.3f} "
