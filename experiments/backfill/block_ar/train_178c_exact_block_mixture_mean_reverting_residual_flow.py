#!/usr/bin/env python
"""
178c: Exact blockwise latent uncertainty mixture on top of the 177a/178b backbone.
...
Change only the uncertainty branch:
  - keep blockwise persistent uncertainty structure
  - restore true latent-mixture semantics over block templates
  - avoid soft template averaging before likelihood
  - keep the rest of the realism backbone unchanged
"""
...
class ExactBlockMixtureMeanRevertingResidualFlowDecoder(ResidualFlowStructuredJointDecoder):
    """177a drift path plus exact blockwise latent uncertainty templates."""
    ...
    def build_local_delta_components(
        self,
        ...
        batch, _n_frames, n_cells = base_local_delta.shape
        n_assign = assignments.shape[0]
        bank = self.block_template_bank.permute(1, 0, 2)
        block_ids = torch.arange(self.n_blocks, device=base_local_delta.device).unsqueeze(0)
        block_delta = bank[block_ids, assignments]
        block_delta = (
            block_delta.unsqueeze(2)
            .expand(n_assign, self.n_blocks, self.block_len, n_cells)
            .reshape(n_assign, self.n_frames, n_cells)
        )
        local_delta_raw = (
            base_local_delta.unsqueeze(1)
            + self.static_local_logvar.unsqueeze(0).unsqueeze(0)
            + block_delta.unsqueeze(0)
        )
    ...
    def build_local_delta_for_batch_assignments(
        self,
        base_local_delta: torch.Tensor,
        assignments: torch.Tensor,
    ) -> torch.Tensor:
        batch, _n_frames, n_cells = base_local_delta.shape
        bank = self.block_template_bank.permute(1, 0, 2)
        block_ids = torch.arange(self.n_blocks, device=base_local_delta.device).view(1, self.n_blocks)
        block_delta = bank[block_ids, assignments]
        block_delta = (
            block_delta.unsqueeze(2)
            .expand(batch, self.n_blocks, self.block_len, n_cells)
            .reshape(batch, self.n_frames, n_cells)
        )
        local_delta_raw = (
            base_local_delta
            + self.static_local_logvar.unsqueeze(0)
            + block_delta
        )
        local_delta = torch.tanh(local_delta_raw) * self.local_delta_clip
...
class ExactBlockMixtureMeanRevertingResidualFlowStructuredJointStudentTModel(
    ResidualFlowStructuredJointStudentTModel
):
    def __init__(
        self,
        ...
        self.decoder = ExactBlockMixtureMeanRevertingResidualFlowDecoder(**decoder_config)
    ...
    def log_prob_future(
        ...
        local_delta_all = self.decoder.build_local_delta_components(base_local_delta, assignments)
        ...
        for start in range(0, assignments.shape[0], self.mix_chunk_size):
            ...
            base_logprob = self._base_logprob(z).view(batch, end - start)
        ...
        local_delta_map = self.decoder.build_local_delta_for_batch_assignments(base_local_delta, map_assign)
        local_scale_map = torch.exp(0.5 * local_delta_map)
        diff_map = (target_u - mu) / local_scale_map
        white_t_map = torch.linalg.solve_triangular(chol_t, diff_map, upper=False)
        white_map = torch.linalg.solve_triangular(chol_c, white_t_map.transpose(1, 2), upper=False).transpose(1, 2)
        white_flat_map = white_map.reshape(batch, n_frames * n_cells)
        ...
        aux = {
            ...
            "local_delta_rms": local_delta_map.pow(2).mean(dim=(1, 2)).sqrt(),
    ...
    def sample_future_u(self, history_01: torch.Tensor, n_samples: int):
        ...
        bank = self.decoder.block_template_bank.permute(1, 0, 2)
        block_ids = torch.arange(self.decoder.n_blocks, device=mu.device).view(1, 1, self.decoder.n_blocks)
        block_delta = bank[block_ids, sampled_assign]
        block_delta = (
            block_delta.unsqueeze(3)
            .expand(batch, n_samples, self.decoder.n_blocks, self.decoder.block_len, n_cells)
            .reshape(batch, n_samples, n_frames, n_cells)
        )
        local_delta_raw = (
            base_local_delta.unsqueeze(1)
            + self.decoder.static_local_logvar.unsqueeze(0).unsqueeze(0)
            + block_delta
        )
        local_delta = torch.tanh(local_delta_raw) * self.decoder.local_delta_clip
        local_delta = local_delta - local_delta.mean(dim=(2, 3), keepdim=True)
        local_scale = torch.exp(0.5 * local_delta)
...
def main():
    parser = argparse.ArgumentParser(description="178c: exact block-mixture mean-reverting residual flow")
    ...
    print(f"\n{'=' * 64}")
    print("178c: exact block-mixture mean-reverting residual flow")
    print(f"{'=' * 64}")
            ...
            torch.save(
                {
                    ...
                    "config": {
                        "type": "exact_block_mixture_mean_reverting_residual_flow_structured_joint_student_t_178c",
