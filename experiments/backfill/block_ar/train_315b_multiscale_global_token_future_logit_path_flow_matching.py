...
from diffusion.block_ar.multiscale_global_token_future_logit_path_flow_matching import (
    MultiscaleGlobalTokenFutureLogitPathFMConfig,
    MultiscaleGlobalTokenFutureLogitPathFlowMatching,
    save_checkpoint,
)
...
def parse_knot_positions(spec: str, future_len: int) -> tuple[int, ...]:
    vals = tuple(int(x.strip()) - 1 for x in spec.split(",") if x.strip())
    if not vals:
        raise ValueError("Need at least one knot position")
    if vals[0] != 0 or vals[-1] != future_len - 1:
        raise ValueError("Knot positions must include 1 and future_len")
    return vals


def main() -> None:
    parser = argparse.ArgumentParser(
        description="315b-v0 multiscale global-token future-logit path flow matching"
    )
    ...
    parser.add_argument("--knot_horizons", type=str, default="1,4,8,14,21,30")
    ...
    cfg = MultiscaleGlobalTokenFutureLogitPathFMConfig(
        ...
        knot_positions=parse_knot_positions(args.knot_horizons, args.future_len),
        n_global_tokens=args.n_global_tokens,
    )
    model = MultiscaleGlobalTokenFutureLogitPathFlowMatching(cfg).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rec = {
            ...
            "val_coarse_std": val_avg["coarse_std"],
        ...
        print(
            ...
            f"coarse_std={rec['val_coarse_std']:.3f} resid_std={rec['val_residual_std']:.3f} "
            f"logit_std={rec['val_future_logit_std']:.3f} trans_std={rec['val_implied_transition_std']:.3f} "
    ...
    (out_dir / "train_summary.json").write_text(
        json.dumps({"best_epoch": best_epoch, "best_val_total": best_val, "config": asdict(cfg)}, indent=2)
    )
