...
def path_distance(candidates: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    # candidates [B,K,T,5,5], target [B,T,5,5]
    target_expand = target.unsqueeze(1)
    level_mse = ((candidates - target_expand) ** 2).mean(dim=(2, 3, 4))
    cand_prev = torch.cat([candidates[:, :, :1], candidates[:, :, :-1]], dim=2)
    targ_prev = torch.cat([target_expand[:, :, :1], target_expand[:, :, :-1]], dim=2)
    cand_delta = candidates - cand_prev
    targ_delta = target_expand - targ_prev
    delta_mse = ((cand_delta - targ_delta) ** 2).mean(dim=(2, 3, 4))
    return level_mse + delta_mse
...
def build_topk_batch(
    ...
    candidates = model.library_future_01[top_idx]
    return top_idx, top_scores, query_z, candidates
...
def run_epoch(
    ...
    for batch in loader:
        ...
        with torch.no_grad():
            top_idx, top_scores, query_z, candidates = build_topk_batch(model, history_norm)
            target_dist = torch.softmax(
                -path_distance(candidates, future_01) / model.cfg.label_temperature, dim=-1
            )
        ...
        total_kl += loss.item() * bs
        pred = logits.argmax(dim=-1)
        truth = target_dist.argmax(dim=-1)
        total_top1 += (pred == truth).float().mean().item() * bs
        total_count += bs
    return {
        "loss": total_loss / max(total_count, 1),
        "top1_match": total_top1 / max(total_count, 1),
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="278c-v0 learned reweighted hierarchical retrieval scenario generator")
    ...
    parser.add_argument("--label_temperature", type=float, default=0.25)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        row = {
            ...
            "train_top1_match": train_metrics["top1_match"],
            "val_loss": val_metrics["loss"],
            "val_top1_match": val_metrics["top1_match"],
        }
        history.append(row)
