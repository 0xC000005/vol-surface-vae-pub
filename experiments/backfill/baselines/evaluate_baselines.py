"""Evaluate classical baselines through the same test suites as the deep models.

Usage:
    # All baselines:
    python experiments/backfill/baselines/evaluate_baselines.py --n_samples 50 --max_batches 20

    # Single baseline:
    python experiments/backfill/baselines/evaluate_baselines.py --baselines garch_ccc --n_samples 50

    # Quick test:
    python experiments/backfill/baselines/evaluate_baselines.py --n_samples 10 --max_batches 5
"""
...
def compute_crps(samples: np.ndarray, ground_truth: np.ndarray) -> dict:
    ...
    for h in horizons:
        ...
        # Efficient: use all pairs via broadcasting
        # For K=50, K*(K-1)/2 = 1225 pairs, feasible
...
def compute_energy_score(samples: np.ndarray, ground_truth: np.ndarray) -> dict:
    ...
    for h in horizons:
        ...
        # E||X - X'||₂ via subsampling (full pairs too expensive for large K)
...
def run_baseline_evaluation(
    ...
    """Run all applicable test suites for a baseline model."""
    print("\n" + "=" * 70)
    ...
    results = {"baseline_name": baseline_name, "generation_time_s": gen_time}
    ...
    suite_names = [
        ("surface", "Suite 1: Surface Validity"),
        ("coverage", "Suite 2: CI Coverage"),
        ("conditionality", "Suite 3: Conditionality"),
        ("time_series", "Suite 4: Time Series"),
        ("cointegration", "Suite 6: Cointegration"),
        ("regime_coverage", "Suite 7: Regime Coverage"),
        ("distributional", "Suite 8: Distributional"),
    ]

    n_pass = 0
    n_total = 0
    for key, name in suite_names:
        if key in results:
...
def print_comparison_table(all_results: dict):
    ...
    print("\n" + "=" * 110)
    print("  COMPARISON TABLE: Classical Baselines")
    print("=" * 110)
    ...
    for name, res in all_results.items():
        ...
        # Keys may be int or str depending on in-memory vs JSON
    ...
    print("-" * len(header))
    print(
        f"{'afCRPS 97a+qmap*':<22} {'5/7':>4} "
        f"{'~best':>7} {'~best':>7} "
        f"{'~87%':>7} {'~93%':>7} {'~0.05':>7} "
        f"{'0.919':>6} {'~9%':>7} {'~16%':>7} "
        f"{'1.64':>5}"
    )
    print("\n  * afCRPS 97a+qmap is the proposed method (not run here — see test_block_ar_requirements.py)")
...
def load_deep_baseline(name, device="cuda"):
    ...
    if name == "csdi":
        ...
        return load_csdi_model("models/backfill/baselines/csdi/best_model.pt", device)
    elif name == "timegrad":
        from experiments.backfill.baselines.timegrad_standalone import load_timegrad_model
        return load_timegrad_model("models/backfill/baselines/timegrad/best_model.pt", device)
    elif name == "deepvar":
        from experiments.backfill.baselines.deepvar_standalone import load_deepvar_model
        return load_deepvar_model("models/backfill/baselines/deepvar/best_model.pt", device)
    elif name == "vae":
        from experiments.backfill.baselines.vae_adapter import load_vae_model
        return load_vae_model(
            "models/backfill/context20_production/backfill_16yr.pt", device)
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate baselines for IV surface scenario generation"
    )
    parser.add_argument(
        "--baselines", nargs="*", default=None, choices=ALL_BASELINES,
        help="Which classical baselines to evaluate (default: all; pass none to skip)",
    )
    parser.add_argument(
        "--deep_baselines", nargs="*", default=[], choices=DEEP_BASELINES,
        help="Which deep baselines to evaluate (require trained checkpoints)",
    )
    ...
    # Standard Block-AR split (config_block_ar.py)
    history_len = 30
    future_len = 30
    train_end = 4040
    test_start = 4540
    ...
    # Build baselines (all fitted on training data only)
    builders = {
    ...
    classical_baselines = args.baselines if args.baselines is not None else ALL_BASELINES
    baseline_models = {}
    ...
    # Load deep baselines
    for name in args.deep_baselines:
    ...
    print(f"\nAll results saved to {output_dir}/all_baselines_results.json")
