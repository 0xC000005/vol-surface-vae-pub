"""
Train TimeGrad on 38-d joint IV + factor forecasting task.

Usage:
    PYTHONPATH=. python experiments/backfill/baselines/train_timegrad_38d.py [--epochs 100] [--fast]
"""
...
class JointChangesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            "history": seq[:self.history_len],
            "future": seq[self.history_len:],
        }
...
def main():
    parser = argparse.ArgumentParser(description="Train TimeGrad 38-d joint")
    ...
    model = TimeGradModel(
        input_dim=38, gru_hidden=args.gru_hidden,
        n_diffusion_steps=args.n_diffusion_steps,
    ).to(args.device)

    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"TimeGrad 38-d: {n_params:,} params")

    save_dir = Path("models/backfill/baselines/timegrad_38d")
