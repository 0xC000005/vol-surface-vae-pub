"""
Experiment 9b: Scale-Normalized P1 Loss + Marginal Constraints

BACKGROUND FROM exp8:
- P1 loss (-log variance) successfully increased decoder gain 1842x
- BUT it broke the unconditional marginal: Gen Mean = 19% of GT, Gen Var = 6.43% of GT
- The model found a degenerate solution: output small values with spread

ROOT CAUSE:
- Absolute P1 loss allows the model to minimize total loss by:
  1. Shrinking outputs (reduces reconstruction loss if outputs cluster near small values)
  2. Adding small absolute variance (satisfies P1 loss)
- No constraint on the OUTPUT SCALE

SOLUTION (exp9b):
1. Scale-Normalized P1 Loss: Use coefficient of variation (std/mean) instead of absolute variance
   - This prevents the model from shrinking outputs to satisfy P1
   - CV = std / |mean| is scale-invariant

2. Marginal Constraints: Explicit loss terms to match GT mean and variance
   - mean_loss = (generated_mean - gt_mean)²
   - var_loss = (generated_var - gt_var)²

3. Scheduled Training: Establish correct marginal first, then add P1 pressure
   - Phase 1 (epochs 0-100): Only marginal constraints (no P1)
   - Phase 2 (epochs 100-200): Gradually add P1 while maintaining marginal
...
P1_WEIGHT = 0.1             # Weight for P1 loss (scale-normalized)
MEAN_WEIGHT = 1.0           # Weight for marginal mean constraint
VAR_WEIGHT = 1.0            # Weight for marginal variance constraint
...
def get_scheduled_weights(epoch):
    """
    Scheduled weight for P1 loss.

    Phase 1 (0-100): p1_weight = 0 (establish marginal)
    Phase 2 (100-200): p1_weight ramps from 0 to P1_WEIGHT
    """
    if epoch < PHASE1_EPOCHS:
        return 0.0
    else:
        # Linear ramp from 0 to P1_WEIGHT over PHASE2_EPOCHS/2 epochs
        progress = (epoch - PHASE1_EPOCHS) / (PHASE2_EPOCHS / 2)
        return min(P1_WEIGHT, P1_WEIGHT * progress)
...
def train_step_with_marginal_constraints(model, batch, optimizer, epoch,
    ...
    # Get scheduled P1 weight
    p1_weight = get_scheduled_weights(epoch)
    ...
    with autocast('cuda', dtype=torch.bfloat16):
        ...
        # ============================================================================
        # 1. SCALE-NORMALIZED P1 LOSS (Coefficient of Variation)
        # ============================================================================
        # CV = std / |mean| is scale-invariant
        # This prevents the model from shrinking outputs to satisfy P1
        cv = output_std / (torch.abs(output_mean) + 1e-8)
        p1_loss = -torch.log(cv + 1e-8)
...
def train_model(model, train_loader, val_surface, config, epochs=TOTAL_EPOCHS):
    ...
    best_loss = float('inf')
    output_dir = Path("results/prior_encoder_ablation/exp9_marginal_preserving")
...
def main():
    print("="*70)
    print("EXPERIMENT 9b: Scale-Normalized P1 Loss + Marginal Constraints")
    print("="*70)

    print("\nKEY CHANGES FROM EXP8:")
    print("  1. Scale-Normalized P1: CV = std/|mean| instead of absolute variance")
    print("  2. Marginal Constraints: Force output mean/var to match GT")
    print("  3. Scheduled Training: Phase 1 (marginal) → Phase 2 (P1)")

    print(f"\nPARAMETERS:")
    print(f"  P1_WEIGHT = {P1_WEIGHT}")
    print(f"  MEAN_WEIGHT = {MEAN_WEIGHT}")
    print(f"  VAR_WEIGHT = {VAR_WEIGHT}")
    ...
    print("\n" + "="*70)
    print("Training CVAEFullCovPrior with Scale-Normalized P1 + Marginal Constraints")
    print("="*70)
