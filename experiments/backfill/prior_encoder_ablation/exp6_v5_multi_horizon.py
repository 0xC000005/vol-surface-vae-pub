"""
Experiment 6 v5: Multi-Horizon Training to Enable phi Learning

ROOT CAUSE FROM v4:
When horizon=1, the AR(1) covariance matrix degenerates:
    Sigma = sigma_sq * phi^|0-0| = sigma_sq * phi^0 = sigma_sq
There's NO phi dependence when H=1 because phi^0 = 1 always!

FIX:
Train with horizon > 1 so phi receives gradients from the temporal structure.
- H=1: phi.grad = 0 (no gradient)
- H=5: phi.grad ~ 300 (strong gradient)
- H=30: phi.grad ~ 2000 (very strong gradient)

This experiment trains with horizon=5 to enable phi learning while keeping
training time reasonable.

Goal: Fair three-way comparison with phi actually learning:
- A: Prior Network (single ctx -> global sigma^2)
- B1: Prior Encoder Diagonal (full ctx -> per-timestep sigma^2)
- B2: Prior Encoder Full Cov (full ctx -> global sigma^2 with AR(1))
"""
...
# =========================================================================
# KEY CHANGE: Training horizon
# =========================================================================
TRAINING_HORIZON = 5  # Must be > 1 for phi to receive gradients!
...
def measure_roughness_ratio(model, val_surface, horizon=30, num_sequences=50, num_samples=30):
    ...
    with torch.no_grad():
        for i in range(num_sequences):
            ...
            daily_changes = trajectories[:, 1:, :, :] - trajectories[:, :-1, :, :]

            # Per-sample roughness, then average
            per_sample_roughness = daily_changes.std(dim=(1, 2, 3))
            model_roughness = per_sample_roughness.mean().item()
...
def train_model(model_name, model, train_loader, val_surface, config, num_epochs=200, horizon=5):
    ...
    history = {
        ...
        'sigma_sq': [],
        'avg_var': [],
        'phi_grad': [],       # NEW: Track phi gradients
        'sigma_sq_grad': []   # NEW: Track sigma_sq gradients
    }

    print(f"\nTraining {model_name} for {num_epochs} epochs...")
    print(f"Training horizon: {horizon} (phi will receive gradients!)")
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}", leave=False):
            ...
            # Track phi/sigma_sq gradients (after backward, before optimizer step clears them)
            # Note: optimizer.step() is called inside train_step, so we check accumulated gradients
            if hasattr(model, 'prior_encoder') and hasattr(model.prior_encoder, 'log_phi'):
                if model.prior_encoder.log_phi.grad is not None:
                    epoch_phi_grads.append(model.prior_encoder.log_phi.grad.item())
                if model.prior_encoder.log_sigma_sq.grad is not None:
                    epoch_sigma_sq_grads.append(model.prior_encoder.log_sigma_sq.grad.item())
            elif hasattr(model, 'full_cov_prior'):
                # Note: FullCovariancePrior also uses log_phi (not raw_phi)
                if model.full_cov_prior.log_phi.grad is not None:
                    epoch_phi_grads.append(model.full_cov_prior.log_phi.grad.item())
                if model.full_cov_prior.log_sigma_sq.grad is not None:
                    epoch_sigma_sq_grads.append(model.full_cov_prior.log_sigma_sq.grad.item())

        avg_recon = np.mean(epoch_losses['recon'])
        avg_kl = np.mean(epoch_losses['kl'])
        avg_total = np.mean(epoch_losses['total'])

        # Average gradients (if any were captured)
        avg_phi_grad = np.mean(epoch_phi_grads) if epoch_phi_grads else None
        avg_sigma_sq_grad = np.mean(epoch_sigma_sq_grads) if epoch_sigma_sq_grads else None
        ...
        history['sigma_sq_grad'].append(avg_sigma_sq_grad)
        ...
        if phi is not None:
            print(f", phi={phi:.4f}, sigma_sq={sigma_sq:.4f}", end="")
            if avg_phi_grad is not None:
                grad_sign = "+" if avg_phi_grad > 0 else ""
                print(f", phi_grad={grad_sign}{avg_phi_grad:.2e}", end="")
        ...
        if cond_var_ratio is not None and cond_var_ratio > 0.05:
            print(f"\n[EARLY STOP] Conditional variance {cond_var_ratio:.4%} > 5%")
            break

    print(f"\n[COMPLETE] Best P1: {best_cond_var_ratio:.4%}, Best P2: {best_roughness_ratio:.2%}")
...
def main():
    print("=" * 80)
    print("EXPERIMENT 6 v5: Multi-Horizon Training (phi Learning Enabled)")
    print("=" * 80)
    print()
    print("ROOT CAUSE FROM v4:")
    print("  - When horizon=1, Sigma = sigma_sq * phi^0 = sigma_sq")
    print("  - phi^0 = 1 always, so phi has NO gradient!")
    print()
    print("FIX:")
    print(f"  - Training with horizon={TRAINING_HORIZON} so phi receives gradients")
    print("  - H=1: phi.grad = 0, H=5: phi.grad ~ 300, H=30: phi.grad ~ 2000")
    print()
    ...
    # =========================================================================
    # KEY CHANGE: Create sequences with horizon > 1
    # =========================================================================
    C = config.context_len
    H = TRAINING_HORIZON
    seq_len = C + H  # Context + horizon (was: C + 1)
    ...
    # =========================================================================
    # KEY CHANGE: Model config with horizon > 1
    # =========================================================================
    model_config = {
        "feat_dim": (5, 5),
        "ex_feats_dim": 3,
        ...
        "horizon": H,               # KEY CHANGE: horizon > 1
    ...
    output_dir = Path("results/prior_encoder_ablation/extended_training_v5")
    ...
    print("=" * 80)
    print("VARIANT A: Baseline (Prior Network - single ctx -> global sigma^2)")
    print("=" * 80)
    ...
    model_a, history_a = train_model("Baseline", model_a, train_loader, val_surface, config, num_epochs=200, horizon=H)
    ...
    print("\n" + "=" * 80)
    print("VARIANT B1: Prior Encoder Diagonal (full ctx -> per-timestep sigma^2)")
    print("=" * 80)
    ...
    model_b1, history_b1 = train_model("Prior Encoder Diagonal", model_b1, train_loader, val_surface, config, num_epochs=200, horizon=H)
    ...
    print("\n" + "=" * 80)
    print("VARIANT B2: Prior Encoder Full Cov (full ctx -> global sigma^2 AR(1))")
    print("=" * 80)
    ...
    model_b2, history_b2 = train_model("Prior Encoder Full Cov", model_b2, train_loader, val_surface, config, num_epochs=200, horizon=H)
    ...
    print("\n" + "=" * 80)
    print(f"RESULTS: Problem 1 & 2 Evaluation (v5 - Multi-Horizon H={H})")
    print("=" * 80)
    print()
    ...
    print("-" * 100)
    print(f"{'Model':<30} {'phi (init=0.5)':<15} {'sigma_sq':<15} {'avg_var':<15} {'phi changed?':<15}")
    print("-" * 100)
    ...
    for name, hist in results.items():
        ...
        # Check if phi changed from initialization (0.5)
        if phi_values:
            phi_changed = "YES!" if abs(phi_values[-1] - 0.5) > 0.01 else "NO (stuck)"
        else:
            phi_changed = "N/A"

        print(f"{name:<30} {phi_str:<15} {sigma_sq_str:<15} {avg_var_str:<15} {phi_changed:<15}")

    print()

    # Gradient summary (new in v5)
    print("GRADIENT SUMMARY (phi learning check)")
    print("-" * 100)
    print(f"{'Model':<30} {'Avg phi_grad':<20} {'Avg sigma_sq_grad':<20} {'phi moving?':<15}")
    print("-" * 100)

    for name, hist in results.items():
        phi_grads = [x for x in hist['phi_grad'] if x is not None]
        sigma_grads = [x for x in hist['sigma_sq_grad'] if x is not None]
        phi_values = [x for x in hist['phi'] if x is not None]

        if phi_grads:
            avg_phi_grad = np.mean(phi_grads)
            avg_sigma_grad = np.mean(sigma_grads) if sigma_grads else 0
            phi_moving = "YES!" if avg_phi_grad != 0 and len(phi_values) > 1 and abs(phi_values[-1] - phi_values[0]) > 0.001 else "NO"
            print(f"{name:<30} {avg_phi_grad:<20.2e} {avg_sigma_grad:<20.2e} {phi_moving:<15}")
        else:
    ...
    print("=" * 80)
    print("INTERPRETATION: Multi-Horizon vs Single-Step Training")
    print("=" * 80)
    print()
    ...
    for name, hist in results.items():
        ...
        p1_solved = "[OK]" if best_p1 >= P1_TARGET else "[X]"
        p2_solved = "[OK]" if best_p2 >= P2_TARGET else "[X]"
    ...
    print("-" * 80)
    print("KEY QUESTION: Does phi learn with multi-horizon training?")
    print("-" * 80)

    fullcov_phi = None
    for name, hist in results.items():
        phi_values = [x for x in hist['phi'] if x is not None]
        if 'full_cov' in name and phi_values:
            fullcov_phi = phi_values[-1]
            break

    baseline_phi = None
    for name, hist in results.items():
        phi_values = [x for x in hist['phi'] if x is not None]
        if 'baseline' in name and phi_values:
            baseline_phi = phi_values[-1]
            break

    if fullcov_phi is not None and abs(fullcov_phi - 0.5) > 0.01:
        print(f"[SUCCESS] Full Cov phi learned: {fullcov_phi:.4f} (init=0.5)")
    elif fullcov_phi is not None:
        print(f"[STUCK] Full Cov phi still at: {fullcov_phi:.4f} (init=0.5)")

    if baseline_phi is not None and abs(baseline_phi - 0.5) > 0.01:
        print(f"[SUCCESS] Baseline phi learned: {baseline_phi:.4f} (init=0.5)")
    elif baseline_phi is not None:
        print(f"[STUCK] Baseline phi still at: {baseline_phi:.4f} (init=0.5)")
