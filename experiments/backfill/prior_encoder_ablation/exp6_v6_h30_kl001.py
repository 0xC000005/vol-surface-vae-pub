"""
Experiment 6 v6: Combined H=30 + KL x100 for P1/P2 Improvement

BACKGROUND FROM v5:
- Multi-horizon training (H=5) enabled phi learning (phi.grad != 0)
- But phi learned SLOWLY (-1.8% in 200 epochs)
- P1/P2 metrics still far below targets

ROOT CAUSES IDENTIFIED:
1. Weak phi gradient: H=5 gives only phi^1...phi^4 terms
2. Variance collapse: KL weight (0.00001) too low to prevent deterministic predictions

FIX (v6):
1. TRAINING_HORIZON = 30 (6x stronger phi gradient from phi^1...phi^29)
2. kl_weight = 0.001 (100x increase to prevent variance collapse)

EXPECTED OUTCOMES:
- Phi should learn faster (stronger gradients)
- P1 should improve (KL prevents variance collapse)
- P2 should improve (better temporal structure from phi)

SUCCESS CRITERIA:
- Partial success: P1 > 0.1% (10x improvement over v5)
- Full success: P1 > 2%, P2 > 40%
"""
...
# KEY CHANGES FROM v5
# =========================================================================
TRAINING_HORIZON = 30       # Was 5 in v5 (6x stronger phi gradient)
KL_WEIGHT_OVERRIDE = 0.001  # Was 0.00001 (100x increase)
...
def train_model(model_name, model, train_loader, val_surface, config, num_epochs=200, horizon=30, kl_weight_override=None):
    """Train a model for specified number of epochs, tracking Problem 1 & 2 metrics."""

    optimizer = optim.Adam(model.parameters(), lr=config.learning_rate)

    # Use override KL weight if provided
    target_kl_weight = kl_weight_override if kl_weight_override is not None else config.kl_weight
    ...
    print(f"Training horizon: {horizon} (phi will receive gradients from phi^1...phi^{horizon-1})")
    print(f"Target KL weight: {target_kl_weight} (100x increase from default)")
    ...
    for epoch in range(num_epochs):
        ...
        # KL Annealing with override weight
        if epoch < kl_anneal_epochs:
            effective_kl_weight = target_kl_weight * ((epoch + 1) / kl_anneal_epochs)
            model.kl_weight = effective_kl_weight
            if epoch == 0 or (epoch + 1) % 10 == 0:
                print(f"  KL annealing: {effective_kl_weight:.6f} ({100*(epoch+1)/kl_anneal_epochs:.1f}% of target {target_kl_weight})")
        else:
            model.kl_weight = target_kl_weight
            effective_kl_weight = target_kl_weight
...
def main():
    print("=" * 80)
    print("EXPERIMENT 6 v6: Combined H=30 + KL x100")
    print("=" * 80)
    print()
    print("CHANGES FROM v5:")
    print(f"  1. TRAINING_HORIZON: 5 -> {TRAINING_HORIZON} (6x stronger phi gradient)")
    print(f"  2. KL_WEIGHT: 0.00001 -> {KL_WEIGHT_OVERRIDE} (100x increase)")
    print()
    print("EXPECTED OUTCOMES:")
    print("  - Phi should learn faster (stronger gradients)")
    print("  - P1 should improve (KL prevents variance collapse)")
    print("  - P2 should improve (better temporal structure)")
    print()
    ...
    # Create sequences with horizon = 30
    ...
    # Model config with KL weight override
    model_config = {
        "feat_dim": (5, 5),
        "ex_feats_dim": 3,
        ...
        "device": device,
        "kl_weight": KL_WEIGHT_OVERRIDE,  # KEY CHANGE: Override from config
        "re_feat_weight": 1.0,
    ...
    output_dir = Path("results/prior_encoder_ablation/extended_training_v6")
    ...
    model_a, history_a = train_model("Baseline", model_a, train_loader, val_surface, config,
                                      num_epochs=200, horizon=H, kl_weight_override=KL_WEIGHT_OVERRIDE)
    ...
    model_b1, history_b1 = train_model("Prior Encoder Diagonal", model_b1, train_loader, val_surface, config,
                                        num_epochs=200, horizon=H, kl_weight_override=KL_WEIGHT_OVERRIDE)
    ...
    model_b2, history_b2 = train_model("Prior Encoder Full Cov", model_b2, train_loader, val_surface, config,
                                        num_epochs=200, horizon=H, kl_weight_override=KL_WEIGHT_OVERRIDE)
    ...
    print("\n" + "=" * 80)
    print(f"RESULTS: Problem 1 & 2 Evaluation (v6 - H={H}, KL={KL_WEIGHT_OVERRIDE})")
    print("=" * 80)
    print()
    ...
    print("=" * 80)
    print("COMPARISON: v5 (H=5, KL=0.00001) vs v6 (H=30, KL=0.001)")
    print("=" * 80)
    print()

    print("v5 Results (for reference):")
    print("  - Baseline P1: 0.0058%, P2: 6.25%")
    print("  - Diagonal P1: 0.0006%, P2: 4.92%")
    print("  - Full Cov P1: 0.0011%, P2: 10.23%")
    print("  - Phi change: -1.8% (200 epochs)")
    print()
    ...
    for name, hist in results.items():
        ...
        phi_change = 0
        if phi_values and len(phi_values) > 1:
            phi_change = (phi_values[-1] - 0.5) / 0.5 * 100

        print(f"  - {name}: P1={final_p1:.4%}, P2={final_p2:.2%}, phi_change={phi_change:+.1f}%")
