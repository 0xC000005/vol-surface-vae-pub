"""
Validate VAE Autoregressive Multi-Step Sequences for Context60 Model

Validates all 8 generated NPZ files (4 periods × 2 horizons) to ensure:
1. File existence
2. Correct shapes
...
8. Correct context_len metadata (60)
9. Correct method metadata ('autoregressive_multistep')

Usage:
    python experiments/backfill/context60/autoregressive/validate_vae_ar_sequences.py --sampling_mode oracle
    python experiments/backfill/context60/autoregressive/validate_vae_ar_sequences.py --sampling_mode prior
"""
import argparse
import numpy as np
...
def validate_ar_file(filepath, expected_horizon, period_name, expected_sampling_mode):
    """
    Validate a single autoregressive NPZ file.

    Args:
        filepath: Path to NPZ file
        expected_horizon: Expected total_horizon value (180 or 270)
    ...
    try:
        ...
        # Check required keys
        required_keys = ['surfaces', 'indices', 'quantiles', 'total_horizon',
                        'period_start', 'period_end', 'method', 'sampling_mode', 'context_len']
        ...
        total_horizon = int(data['total_horizon'])
        method = str(data['method'])
        sampling_mode = str(data['sampling_mode'])
        context_len = int(data['context_len'])

        # Check method
        if method != 'autoregressive_multistep':
            print(f"  ✗ FAIL: Expected method='autoregressive_multistep', got '{method}'")
            return False
        ...
        expected_shape = (n_days, expected_horizon, 3, 5, 5)
        if surfaces.shape[1:] != expected_shape[1:]:
            print(f"  ✗ FAIL: Shape mismatch")
            print(f"    Expected: (n_days, {expected_horizon}, 3, 5, 5)")
            print(f"    Got: {surfaces.shape}")
            return False
        print(f"  ✓ Shape correct: {surfaces.shape}")

        # Check total_horizon value
        if total_horizon != expected_horizon:
            print(f"  ✗ FAIL: Total horizon mismatch (expected {expected_horizon}, got {total_horizon})")
            return False
        print(f"  ✓ Total horizon value correct: {total_horizon}")
        ...
        if min_val < 0.01 or max_val > 5.0:
            ...
            print(f"    (This might be okay for long AR sequences, but verify)")
        else:
        ...
        # Allow higher tolerance for AR sequences (up to 5%)
        # AR sequences accumulate errors, so some violations are expected
        ...
        if total_violations > 0:
            ...
            if violation_pct > violation_threshold:
                ...
                print(f"    Note: {total_violations} / {total_points} points have minor violations (acceptable for AR)")
        else:
        ...
        print(f"    total_horizon: {H} days (AR)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Validate VAE autoregressive multi-step sequences for context60 model'
    )
    ...
    print("=" * 80)
    print("VAE AUTOREGRESSIVE MULTI-STEP SEQUENCE VALIDATION (CONTEXT60)")
    print("=" * 80)
    ...
    # Expected sequence counts for autoregressive (context_len=60, total_horizon=180 or 270)
    expected_counts = {
        180: {'crisis': 526, 'insample': 3732, 'oos': 553, 'gap': 788},
        270: {'crisis': 436, 'insample': 3642, 'oos': 463, 'gap': 698}
    }

    base_dir = Path(f"results/context60_baseline/predictions/autoregressive_multi_step/{args.sampling_mode}")

    # Check directory existence
    if not base_dir.exists():
        print(f"\n✗ ERROR: Output directory does not exist: {base_dir}")
        print("  Run generation scripts first!")
        print(f"  Example: bash experiments/backfill/context60/autoregressive/run_generate_all_ar_sequences.sh {args.sampling_mode}")
        return

    print(f"Checking directory: {base_dir}")
    print(f"Expected files: {len(periods) * len(horizons)} (4 periods × 2 horizons)\n")
    ...
    for period in periods:
        for horizon in horizons:
            filename = f"vae_ar_{period}_{horizon}day.npz"
            ...
            # Validate file
            passed = validate_ar_file(filepath, horizon, period, args.sampling_mode)
            ...
            if passed:
                ...
                expected_count = expected_counts[horizon][period]
    ...
    if passed_files == total_files:
        ...
        print(f"\nSampling mode '{args.sampling_mode}' AR sequences validated successfully.")
        print("\nNext steps:")
        print(f"  1. Compare AR vs teacher forcing results")
        print(f"  2. Analyze error accumulation in long AR sequences")
        print(f"  3. Compare context20 vs context60 performance")
    else:
        print("\n✗ SOME VALIDATIONS FAILED")
        print("\nFailed files:")
        for (period, horizon), passed in results.items():
            if not passed:
                print(f"  - vae_ar_{period}_{horizon}day.npz")
        print("\nPlease regenerate failed files:")
        print(f"  bash experiments/backfill/context60/autoregressive/run_generate_all_ar_sequences.sh {args.sampling_mode}")
