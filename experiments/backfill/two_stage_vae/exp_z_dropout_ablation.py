"""
Experiment: Z Dropout + Lower Z Dimension Ablation

Goal: Prevent z from becoming a lookup key by making it unreliable.

Hypothesis:
- Lower z_dim = less capacity to encode specific outcomes
- Dropout on z = decoder can't rely on z being complete
- Both combined = strongest regularization

Experiments:
- Baseline: latent_dim=16, z_dropout=0.0 (current config)
- Exp A: latent_dim=8, z_dropout=0.0 (lower capacity only)
- Exp B: latent_dim=16, z_dropout=0.3 (dropout only)
- Exp C: latent_dim=8, z_dropout=0.3 (combined)

Metrics:
- CI violations per horizon
- Decoder gain: output_variance / z_variance
- Reconstruction MSE

Usage:
    python experiments/backfill/two_stage_vae/exp_z_dropout_ablation.py
"""
...
def measure_decoder_gain(model, log_returns, device, config, num_contexts=50, num_samples=50):
    ...
    This measures how much the decoder responds to z variations.
    Low gain = decoder ignores z (problem!)
    High gain = decoder uses z effectively (good!)
    """
    model.eval()
    context_len = config["context_len"]
    latent_dim = config["latent_dim"]
    seq_len = context_len + 1  # Need at least 1 horizon step
    ...
    with torch.no_grad():
        for i in range(num_contexts):
            ...
            ctx_emb = model.ctx_encoder(batch)  # (1, T, ctx_embedding_dim)
            ...
            # Compute variances
            samples_tensor = torch.stack(samples).squeeze()  # (num_samples, T, 5, 5)
            z_tensor = torch.stack(z_samples).squeeze()  # (num_samples, T, latent_dim)
...
def measure_ci_coverage(model, log_returns, log_surfaces, device, config,
    ...
    Returns coverage and CI width for each horizon.
    """
    model.eval()
    context_len = config["context_len"]
...
def main():
    print("="*70)
    print("Z DROPOUT + LOWER Z DIMENSION ABLATION STUDY")
    print("="*70)
    print()
    print("Goal: Prevent z from becoming a lookup key")
    print("Method: Make z unreliable via dropout and lower dimensionality")
    print()

    # Output directory
    output_dir = Path("models/backfill/two_stage/z_dropout_ablation")
    ...
    # Define experiments
    experiments = [
        ("baseline", {"latent_dim": 16, "z_dropout": 0.0}),
        ("exp_a_zdim8", {"latent_dim": 8, "z_dropout": 0.0}),
        ("exp_b_dropout03", {"latent_dim": 16, "z_dropout": 0.3}),
        ("exp_c_combined", {"latent_dim": 8, "z_dropout": 0.3}),
    ]
    ...
    print("| Experiment | latent_dim | z_dropout | Decoder Gain | H=1 Viol | H=30 Viol | Val Loss |")
    print("|------------|------------|-----------|--------------|----------|-----------|----------|")
    for exp_name, results in all_results.items():
        cfg = results["config"]
        h1_viol = results["ci_results"].get("1", {}).get("violations", 0) * 100
        h30_viol = results["ci_results"].get("30", {}).get("violations", 0) * 100
        print(f"| {exp_name:10s} | {cfg['latent_dim']:10d} | {cfg['z_dropout']:9.1f} | "
    ...
    for exp_name, results in all_results.items():
        if exp_name == "baseline":
            continue
        gain_improvement = results["decoder_gain"] / baseline_gain
        print(f"\n{exp_name}:")
        print(f"  Decoder gain improvement: {gain_improvement:.1f}x")

        baseline_h30 = all_results["baseline"]["ci_results"]["30"]["violations"]
        exp_h30 = results["ci_results"]["30"]["violations"]
        if baseline_h30 > 0:
            viol_reduction = (baseline_h30 - exp_h30) / baseline_h30 * 100
            print(f"  H=30 violation reduction: {viol_reduction:.1f}%")
