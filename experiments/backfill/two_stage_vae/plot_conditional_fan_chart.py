...
def generate_conditional_trajectories(model, surfaces, log_returns, start_idx,
    ...
    with torch.no_grad():
        for h in range(horizon):
            # For oracle case, we include the target in the sequence
            # Build sequence: context + targets up to horizon h+1
            if h == 0:
            ...
            # Get the last prediction (horizon h)
            samples_h = samples[:, 0, -1, 2, 2].cpu().numpy()  # (n_samples,)
