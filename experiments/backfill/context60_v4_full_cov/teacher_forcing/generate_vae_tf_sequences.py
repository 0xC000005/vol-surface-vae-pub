"""
Generate VAE Teacher Forcing Sequences for Context60 Model (Single-Output Decoder)

Generates full in-sequence data for all horizons (H=1, 7, 14, 30, 60, 90) using teacher forcing.
For each horizon H, generates (n_days, H, 5, 5) arrays containing ALL days in the forecast.
...
Output: NPZ files in results/context60_v4_full_cov/predictions/teacher_forcing/{sampling_mode}/
- vae_tf_crisis_h{1,7,14,30,60,90}.npz (shape: n_days, H, 5, 5)
- vae_tf_insample_h{1,7,14,30,60,90}.npz (shape: n_days, H, 5, 5)
- vae_tf_oos_h{1,7,14,30,60,90}.npz (shape: n_days, H, 5, 5)

Usage:
    # Oracle sampling (posterior, with future knowledge)
    python experiments/backfill/context60_v4_full_cov/teacher_forcing/generate_vae_tf_sequences.py --period crisis --sampling_mode oracle

    # Prior sampling (realistic, no future knowledge)
    python experiments/backfill/context60_v4_full_cov/teacher_forcing/generate_vae_tf_sequences.py --period crisis --sampling_mode prior
"""
import argparse
import numpy as np
import torch
...
def generate_and_save_horizon(model, vol_surf, ex_data,
    ...
    with torch.no_grad():
        for seq_start in tqdm(range(min_start, max_start + 1),
            ...
            if sampling_mode == "oracle":
                ...
                forecast = surf_pred  # (1, H, 5, 5)

            else:
                raise ValueError(f"Unknown sampling_mode: {sampling_mode}")

            # Remove batch dimension and convert to numpy
            forecast_np = forecast[0].cpu().numpy()  # (H, 5, 5)
    ...
    sequences_array = np.array(sequences)  # (n_days, H, 5, 5)
    ...
    np.savez(
        output_file,
        surfaces=sequences_array,      # (n_days, H, 5, 5) - single prediction per timestep
        ...
        context_len=60                  # Context length metadata
    )
...
def main():
    ...
    parser.add_argument('--prior_mode', type=str, default='auto',
                       choices=['standard', 'fitted', 'full_cov', 'auto'],
                       help='Prior sampling mode (only for --sampling_mode prior): standard=N(0,1), fitted=GMM, full_cov=AR(1), auto=detect from model')
    parser.add_argument('--fitted_prior_path', type=str,
                       default='models/backfill/context60_v4_full_cov/fitted_prior_gmm.pt',
                       help='Path to fitted prior parameters (only for --prior_mode fitted)')
    parser.add_argument('--model_path', type=str,
                       default='models/backfill/context60_v4_full_cov/checkpoints/backfill_context60_latent12_v4_full_cov.pt',
    ...
    # Detect which model class to use based on config
    if 'full_cov_init_phi' in model_config or model_config.get('use_full_cov_prior', False):
        # V4 Full Covariance Prior model
        print("  Using CVAEFullCovPrior (AR(1) full covariance prior)")
        model = CVAEFullCovPrior(model_config)
        # Print prior params if model is loaded
        try:
            phi = model.full_cov_prior.get_phi().item()
            sigma_sq = model.full_cov_prior.get_sigma_sq().item()
            print(f"    φ={phi:.4f}, σ²={sigma_sq:.4f}")
        except:
            pass  # Model not loaded yet
    elif model_config.get('use_conditional_prior', False):
        # V3 Conditional Prior model
    ...
    # Print prior parameters if CVAEFullCovPrior (now that weights are loaded)
    if isinstance(model, CVAEFullCovPrior):
        phi = model.full_cov_prior.get_phi().item()
        sigma_sq = model.full_cov_prior.get_sigma_sq().item()
        print(f"  Loaded prior params: φ={phi:.4f}, σ²={sigma_sq:.4f}")

    # Auto-detect prior mode based on model type
    if args.prior_mode == 'auto':
        if isinstance(model, CVAEFullCovPrior):
            effective_prior_mode = 'full_cov'
            print(f"\n✓ Auto-detected prior_mode='full_cov' for CVAEFullCovPrior")
        elif isinstance(model, CVAEMemRandConditionalPrior):
            effective_prior_mode = 'fitted'
            print(f"\n✓ Auto-detected prior_mode='fitted' for CVAEMemRandConditionalPrior")
        else:
            effective_prior_mode = 'standard'
            print(f"\n✓ Auto-detected prior_mode='standard' for CVAEMemRand")
    else:
        effective_prior_mode = args.prior_mode

    # Validate prior_mode for CVAEFullCovPrior
    if isinstance(model, CVAEFullCovPrior) and effective_prior_mode != 'full_cov':
        print(f"\n⚠️ WARNING: CVAEFullCovPrior requires prior_mode='full_cov', "
              f"overriding '{effective_prior_mode}' -> 'full_cov'")
        effective_prior_mode = 'full_cov'

    # Load fitted prior if using fitted mode
    if args.sampling_mode == 'prior' and effective_prior_mode == 'fitted':
    ...
    for horizon in horizons:
        generate_and_save_horizon(
            ...
            prior_mode=effective_prior_mode if args.sampling_mode == 'prior' else 'standard',
    ...
    print()
    print("Next steps:")
    print(f"  1. Validate outputs: python experiments/backfill/context60_v4_full_cov/teacher_forcing/validate_vae_tf_sequences.py --sampling_mode {args.sampling_mode}")
    print(f"  2. Run analysis scripts with context60_v4 data")
    print()
