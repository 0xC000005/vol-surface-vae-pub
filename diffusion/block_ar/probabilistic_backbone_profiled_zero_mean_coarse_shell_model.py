...
@dataclass
class ProbabilisticBackboneProfiledZeroMeanCoarseShellConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class ProfiledZeroMeanCoarseShellCore(nn.Module):
    """296c-v0 shell: 296b plus a shared query-conditioned knot profile."""

    def __init__(self, cfg: ProbabilisticBackboneProfiledZeroMeanCoarseShellConfig):
        super().__init__()
        self.cfg = cfg
...
class BackboneProfiledZeroMeanCoarseShellGenerator(nn.Module):
    def __init__(
        self,
        shell: ProfiledZeroMeanCoarseShellCore,
        backbone: nn.Module,
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[BackboneProfiledZeroMeanCoarseShellGenerator, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = ProbabilisticBackboneProfiledZeroMeanCoarseShellConfig(**payload["config"])
    shell = ProfiledZeroMeanCoarseShellCore(cfg)
    ...
    model = BackboneProfiledZeroMeanCoarseShellGenerator(
...
def save_checkpoint(
    path: str,
    shell: ProfiledZeroMeanCoarseShellCore,
    cfg: ProbabilisticBackboneProfiledZeroMeanCoarseShellConfig,
    epoch: int,
    best_val: float,
    backbone_checkpoint_path: str,
) -> None:
