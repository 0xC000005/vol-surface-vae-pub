...
class DeterministicLearnedRetrievalLocalHistoryTwoTimescaleDeltaBackbone(torch.nn.Module):
    """287c-v0: local-history retrieval with factored fast/slow keys."""
    ...
    def sample_batched(
        ...
        **_: object,
        ...
        history_fast = build_history_fast(history_norm)
...
def build_history_fast(history_norm: torch.Tensor) -> torch.Tensor:
    prev = torch.cat([history_norm[:, :1], history_norm[:, :-1]], dim=1)
    return history_norm - prev


def build_history_slow(history_norm: torch.Tensor) -> torch.Tensor:
    return history_norm - history_norm[:, :1]


def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[DeterministicLearnedRetrievalLocalHistoryTwoTimescaleDeltaBackbone, dict]:
