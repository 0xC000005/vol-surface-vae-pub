...
@dataclass
class JointStateSpaceLatentFactorFMConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
    latent_dim: int = 8
...
class JointStateSpaceLatentFactorFM(nn.Module):
    """263a-v0: joint probabilistic latent factor model with recurrent mean/scale states."""

    def __init__(self, cfg: JointStateSpaceLatentFactorFMConfig):
        super().__init__()
        self.cfg = cfg
        ...
        self.flow_token_proj = nn.Sequential(
            nn.Linear(
                4 * cfg.flow_hidden + cfg.future_pos_embed + cfg.flow_time_embed,
                cfg.flow_hidden,
            ),
            nn.GELU(),
    ...
    def decode_change_coord(self, eps: torch.Tensor, cond: dict[str, torch.Tensor]) -> torch.Tensor:
        if eps.ndim == 3:
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[JointStateSpaceLatentFactorFM, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = JointStateSpaceLatentFactorFMConfig(**payload["config"])
    model = JointStateSpaceLatentFactorFM(cfg)
...
def save_checkpoint(
    path: str,
    model: JointStateSpaceLatentFactorFM,
    cfg: JointStateSpaceLatentFactorFMConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
