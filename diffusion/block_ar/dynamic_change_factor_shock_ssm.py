"""
253b: dynamic change-space factor SSM with bounded common shock channel.

Purpose:
    Follow 253a-ec with the narrowest possible temporal-law extension:
    add a fast common shock path in factor space without reintroducing
    cellwise idiosyncratic leakage.
"""
...
@dataclass
class DynamicFactorShockSSMConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
    latent_dim: int = 8
    ...
    max_shock_ratio: float = 1.0
    shock_shrink: float = 0.10
...
class DynamicChangeFactorShockSSM(nn.Module):
    def __init__(self, cfg: DynamicFactorShockSSMConfig):
        super().__init__()
        self.cfg = cfg
    ...
    def _init_heads(self) -> None:
        ...
        for head in [
            ...
            self.shock_latent_head,
            self.shock_budget_head,
        ]:
            last = head[-1]
            if isinstance(last, nn.Linear):
                with torch.no_grad():
    ...
    def predict_mean(self, history: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        shock_budget_steps: list[torch.Tensor] = []
        ...
        for t in range(self.cfg.future_len):
            ...
            shock_latent = F.softshrink(self.shock_latent_head(ctx), lambd=self.cfg.shock_shrink)
            shock_raw = torch.einsum("bdl,bl->bd", base_loadings, shock_latent)
            shock_budget = (
                self.cfg.max_shock_ratio
                * torch.sigmoid(self.shock_budget_head(ctx))
                * factor_rms
            )
            shock = torch.tanh(shock_raw) * shock_budget
            ...
            delta = factor + shock + idio + ec_delta
        ...
        mean_shock = torch.stack(shock_steps, dim=1)
        mean_common = mean_factor + mean_shock
        mean_idio = torch.stack(idio_steps, dim=1)
        ec_term = torch.stack(ec_steps, dim=1)
        idio_budget = torch.stack(budget_steps, dim=1)
        shock_budget = torch.stack(shock_budget_steps, dim=1)
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[DynamicChangeFactorShockSSM, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = DynamicFactorShockSSMConfig(**payload["config"])
    model = DynamicChangeFactorShockSSM(cfg).to(device)
    model.load_state_dict(payload["model_state_dict"], strict=True)
    model.eval()
    return model, payload


def config_to_dict(cfg: DynamicFactorShockSSMConfig) -> dict:
    return asdict(cfg)
