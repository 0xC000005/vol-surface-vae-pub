...
@dataclass
class ProbabilisticJointTokenPathSupportResidualModelConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class ProbabilisticJointTokenPathSupportResidualModel(nn.Module):
    """293g-v0: explicit coarse support with residual daily next-change law."""
    ...
    def residual_targets(
        ...
        scaffold_coords = self.scaffold_coords_for_training(history_norm, future_norm, coarse_scaffold)
    ...
    def training_loss(
        ...
        residual_coords = self.residual_targets(history_norm, future_norm, coarse_scaffold)
        ...
        **_: object,
        ...
        while remaining > 0:
            ...
            for step in range(self.cfg.future_len):
                ...
                scaffold_coord = self.transform_change(coarse_scaffold[:, step] - curr, hist_window)
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[ProbabilisticJointTokenPathSupportResidualModel, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = ProbabilisticJointTokenPathSupportResidualModelConfig(**payload["config"])
