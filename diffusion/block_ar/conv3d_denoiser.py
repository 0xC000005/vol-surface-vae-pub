...
Uses standard (non-causal) Conv3d for bidirectional temporal info flow, which is
required for MCVD 4-task training (forward/backward/interpolation/unconditional).

Architecture mirrors SimpleDenoiser3D from the DDPM POC:
- Conv3d(1, C) input projection
- N x [ResBlock3D + AdaptiveGroupNorm] backbone
- Conv3d(C, 1) output projection
