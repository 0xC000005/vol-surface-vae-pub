...
@dataclass
class ProbabilisticLatentTokenBypassConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class FutureTokenEncoder(nn.Module):
    def __init__(self, cfg: ProbabilisticLatentTokenBypassConfig):
        super().__init__()
        self.cfg = cfg
        self.gru = nn.GRU(
...
class FuturePathDecoder(nn.Module):
    def __init__(self, cfg: ProbabilisticLatentTokenBypassConfig):
        ...
        token_in = cfg.latent_dim + cfg.pos_dim
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[ProbabilisticLatentTokenReducedBypass, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = ProbabilisticLatentTokenBypassConfig(**payload["config"])
    model = ProbabilisticLatentTokenReducedBypass(cfg)
...
def save_checkpoint(
    path: str,
    model: ProbabilisticLatentTokenReducedBypass,
