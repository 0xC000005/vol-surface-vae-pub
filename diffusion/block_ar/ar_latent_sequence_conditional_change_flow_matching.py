...
@dataclass
class ARLatentSequenceConditionalChangeFMConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class HistoryTokenEncoder(nn.Module):
    def __init__(self, cfg: ARLatentSequenceConditionalChangeFMConfig):
        super().__init__()
        self.cfg = cfg
        self.gru = nn.GRU(
    ...
    def forward(self, history_norm: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        seq, hidden = self.gru(history_norm)
        ...
        tokens = self.norm(self.proj(tail))
        summary = hidden[-1]
...
class ConditionalChangeDecoder(nn.Module):
    def __init__(self, cfg: ARLatentSequenceConditionalChangeFMConfig):
        super().__init__()
        flat_dim = cfg.n_tokens * cfg.token_dim
        self.net = _mlp(
            in_dim=flat_dim + cfg.history_hidden,
            out_dim=cfg.n_cells,
    ...
    def forward(self, tokens_next: torch.Tensor, history_summary: torch.Tensor) -> torch.Tensor:
        flat_next = tokens_next.reshape(tokens_next.shape[0], -1)
        return self.net(torch.cat([flat_next, history_summary], dim=-1))


class ARLatentSequenceConditionalChangeFlowMatching(nn.Module):
    """271a-v0: AR latent-sequence FM with conditional next-change decoder."""
    ...
    def __init__(self, cfg: ARLatentSequenceConditionalChangeFMConfig):
        ...
        self.decoder = ConditionalChangeDecoder(cfg)
    ...
    def decode_next_change(
        self,
        tokens_next: torch.Tensor,
        history_summary: torch.Tensor,
    ) -> torch.Tensor:
        return self.decoder(tokens_next, history_summary)
    ...
    def step_losses(
        ...
        tokens_prev, summary_prev = self.encode_history(history_norm)
        next_history = torch.cat([history_norm[:, 1:, :], next_level_norm[:, None, :]], dim=1)
        tokens_next, _summary_next = self.encode_history(next_history)
        flat_prev = tokens_prev.reshape(tokens_prev.shape[0], -1)
        flat_next = tokens_next.reshape(tokens_next.shape[0], -1)
        ...
        decoded_change = self.decode_next_change(tokens_next, summary_prev)
        recon_loss = F.smooth_l1_loss(decoded_change, target_change)
    ...
    def sample_batched(
        ...
        **_: object,
        ...
        for start in range(0, n_samples, chunk_size):
            ...
            for step in range(self.cfg.future_len):
                tokens_prev, summary_prev = self.encode_history(hist)
                flat_prev = tokens_prev.reshape(tokens_prev.shape[0], -1)
                ...
                next_change = self.decode_next_change(tokens_next, summary_prev)
                next_level = hist[:, -1, :] + next_change
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[ARLatentSequenceConditionalChangeFlowMatching, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = ARLatentSequenceConditionalChangeFMConfig(**payload["config"])
    model = ARLatentSequenceConditionalChangeFlowMatching(cfg)
...
def save_checkpoint(
    path: str,
    model: ARLatentSequenceConditionalChangeFlowMatching,
    cfg: ARLatentSequenceConditionalChangeFMConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
