...
@dataclass
class HistoryConditionedProbabilisticTokenStateLogitPathConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class TokenPosterior(nn.Module):
    def __init__(self, cfg: HistoryConditionedProbabilisticTokenStateLogitPathConfig):
        super().__init__()
        self.cfg = cfg
        latent_dim = cfg.n_tokens * cfg.token_dim
        self.net = _mlp(
            in_dim=cfg.n_cells + cfg.context_dim,
    ...
    def forward(
        ...
        if level_logits.ndim == 2:
            inputs = torch.cat([level_logits, context], dim=-1)
            params = self.net(inputs)
            mean, logvar = params.chunk(2, dim=-1)
            mean = mean.view(level_logits.shape[0], self.cfg.n_tokens, self.cfg.token_dim)
            logvar = logvar.view(level_logits.shape[0], self.cfg.n_tokens, self.cfg.token_dim)
            return mean, logvar.clamp(-8.0, 4.0)

        bsz, steps, _ = level_logits.shape
        ctx = context[:, None, :].expand(-1, steps, -1)
        flat_inputs = torch.cat([level_logits, ctx], dim=-1).reshape(
            bsz * steps, self.cfg.n_cells + self.cfg.context_dim
        )
...
class TokenPrior(nn.Module):
    ...
    def forward(
        ...
        hidden = torch.cat([ctx, z_prev + self.pos], dim=1)
        hidden = self.encoder(hidden)
        params = self.out_proj(hidden[:, 1:, :])
        mean, logvar = params.chunk(2, dim=-1)
        return mean, logvar.clamp(-8.0, 4.0)
...
class HistoryConditionedProbabilisticTokenStateLogitPathModel(nn.Module):
    """314a-v0: history-conditioned probabilistic token-state model over future logit paths."""
    ...
    def training_loss(
        ...
        last_logit = self.to_logits(history_norm[:, -1:, :]).squeeze(1)
        future_logits = self.to_logits(future_norm)
        observed_logits = torch.cat([last_logit[:, None, :], future_logits], dim=1)
        post_mean, post_logvar = self.posterior(observed_logits, context)
        ...
        for step in range(self.cfg.future_len):
            prev_mean = post_mean[:, step]
            prev_logvar = post_logvar[:, step]
            next_mean = post_mean[:, step + 1]
            next_logvar = post_logvar[:, step + 1]

            z_prev = self._sample(prev_mean, prev_logvar)
            z_next = self._sample(next_mean, next_logvar)
            prior_mean, prior_logvar = self.prior(z_prev, context)
            recon_next = self.decoder(z_next)

            recon_losses.append(F.smooth_l1_loss(recon_next, future_logits[:, step, :]))
            kl_losses.append(
            ...
            recon_abs.append((recon_next - future_logits[:, step, :]).abs().mean())
        ...
        metrics = {
            ...
            "recon_logit_abs": torch.stack(recon_abs).mean().detach(),
        }
    ...
    def sample_batched(
        ...
        **_: object,
        ...
        context = self.encode_history(history_norm)
        last_logit = self.to_logits(history_norm[:, -1:, :]).squeeze(1)
        last_mean, _ = self.posterior(last_logit, context)
        ...
        for start in range(0, n_samples, chunk_size):
            ...
            for _step in range(self.cfg.future_len):
                prior_mean, prior_logvar = self.prior(z_prev, ctx)
                z_prev = self._sample(prior_mean, prior_logvar)
                next_logit = self.decoder(z_prev)
                steps.append(next_logit)
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[HistoryConditionedProbabilisticTokenStateLogitPathModel, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = HistoryConditionedProbabilisticTokenStateLogitPathConfig(**payload["config"])
    model = HistoryConditionedProbabilisticTokenStateLogitPathModel(cfg)
...
def save_checkpoint(
    path: str,
    model: HistoryConditionedProbabilisticTokenStateLogitPathModel,
    cfg: HistoryConditionedProbabilisticTokenStateLogitPathConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
