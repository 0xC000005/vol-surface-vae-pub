...
@dataclass
class ARLatentSequenceChangeFMConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class HistoryTokenEncoder(nn.Module):
    def __init__(self, cfg: ARLatentSequenceChangeFMConfig):
        super().__init__()
        self.cfg = cfg
        self.gru = nn.GRU(
...
class ARLatentSequenceChangeFlowMatching(nn.Module):
    """270c-v0: AR latent-token bottleneck FM with next-change decoding."""
    ...
    def decode_next_change(self, tokens_next: torch.Tensor) -> torch.Tensor:
    ...
    def step_losses(
        ...
        decoded_change = self.decode_next_change(tokens_next)
        ...
        **_: object,
        ...
        for start in range(0, n_samples, chunk_size):
            ...
            for step in range(self.cfg.future_len):
                ...
                next_change = self.decode_next_change(tokens_next)
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[ARLatentSequenceChangeFlowMatching, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = ARLatentSequenceChangeFMConfig(**payload["config"])
    model = ARLatentSequenceChangeFlowMatching(cfg)
...
def save_checkpoint(
    path: str,
    model: ARLatentSequenceChangeFlowMatching,
    cfg: ARLatentSequenceChangeFMConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
