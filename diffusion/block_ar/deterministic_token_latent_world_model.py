...
@dataclass
class DeterministicTokenLatentWorldModelConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class DeterministicTokenLatentWorldModel(nn.Module):
    """289d-v0: deterministic latent token/state world model."""

    def __init__(self, cfg: DeterministicTokenLatentWorldModelConfig):
        super().__init__()
        self.cfg = cfg
        ...
        self.token_init = nn.Linear(cfg.latent_dim, cfg.n_latent_tokens * cfg.latent_dim)
        self.token_pos = nn.Parameter(torch.randn(1, cfg.n_latent_tokens + 1, cfg.latent_dim) * 0.02)
        self.obs_encoder = nn.Sequential(
            nn.Linear(2 * cfg.n_cells, cfg.obs_hidden),
            nn.GELU(),
            nn.Linear(cfg.obs_hidden, cfg.latent_dim),
        )
        self.transition = nn.TransformerEncoderLayer(
            d_model=cfg.latent_dim,
            nhead=cfg.nhead,
            dim_feedforward=cfg.ff_mult * cfg.latent_dim,
            dropout=cfg.dropout,
            batch_first=True,
            norm_first=True,
            activation="gelu",
        ...
        self.decoder = nn.Sequential(
            nn.Linear(2 * cfg.latent_dim, cfg.decoder_hidden),
            nn.GELU(),
    ...
    def encode_history(self, history_norm: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        seed = h_last[-1]
        latent_tokens = self.token_init(seed).view(seed.shape[0], self.cfg.n_latent_tokens, self.cfg.latent_dim)
        curr_level = hist[:, -1]
        prev_change = hist[:, -1] - hist[:, -2]
        return latent_tokens, curr_level, prev_change
    ...
    def _transition_step(
        ...
        obs_token = self._obs_embed(curr_level, prev_change).unsqueeze(1)
        seq = torch.cat([latent_tokens, obs_token], dim=1) + self.token_pos
        seq = self.transition(seq)
        latent_tokens = seq[:, : self.cfg.n_latent_tokens]
        obs_token = seq[:, -1]
        pooled = latent_tokens.mean(dim=1)
        next_change = self.decoder(torch.cat([pooled, obs_token], dim=-1))
        return latent_tokens, next_change
    ...
    def teacher_forced_changes(
        ...
        latent_tokens, curr_level, prev_change = self.encode_history(history_norm)
        future = self._flatten_levels(future_norm)
        preds: list[torch.Tensor] = []
        for step in range(self.cfg.future_len):
            latent_tokens, next_change = self._transition_step(latent_tokens, curr_level, prev_change)
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[DeterministicTokenLatentWorldModel, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = DeterministicTokenLatentWorldModelConfig(**payload["config"])
    model = DeterministicTokenLatentWorldModel(cfg)
...
def save_checkpoint(
    path: str,
    model: DeterministicTokenLatentWorldModel,
    cfg: DeterministicTokenLatentWorldModelConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
