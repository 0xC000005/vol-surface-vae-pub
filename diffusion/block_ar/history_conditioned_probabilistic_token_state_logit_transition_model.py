...
@dataclass
class HistoryConditionedProbabilisticTokenStateLogitTransitionConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class TokenPosterior(nn.Module):
    def __init__(self, cfg: HistoryConditionedProbabilisticTokenStateLogitTransitionConfig):
        super().__init__()
        self.cfg = cfg
    ...
    def forward(
        ...
        if transitions.ndim == 2:
            inputs = torch.cat([transitions, context], dim=-1)
        ...
        flat_inputs = torch.cat([transitions, ctx], dim=-1).reshape(
...
class HistoryConditionedProbabilisticTokenStateLogitTransitionModel(nn.Module):
    """314b-v0: history-conditioned probabilistic token-state model over future logit transitions."""
    ...
    def future_transitions(self, history_norm: torch.Tensor, future_norm: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def training_loss(
        ...
        last_transition, future_trans = self.future_transitions(history_norm, future_norm)

        init_mean, init_logvar = self.posterior(last_transition, context)
        post_mean, post_logvar = self.posterior(future_trans, context)
        ...
        for step in range(self.cfg.future_len):
            ...
            recon_trans = self.decoder(z_next)
    ...
    def sample_batched(
        ...
        **_: object,
        ...
        prev_logit = history_logits[:, -2, :]
        last_transition = last_logit - prev_logit
        init_mean, _ = self.posterior(last_transition, context)
        ...
        for start in range(0, n_samples, chunk_size):
            ...
            current_logit = last_logit.repeat_interleave(k, dim=0)
            ...
            for _step in range(self.cfg.future_len):
                ...
                delta = self.decoder(z_prev)
                current_logit = current_logit + delta
                steps.append(current_logit)
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[HistoryConditionedProbabilisticTokenStateLogitTransitionModel, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = HistoryConditionedProbabilisticTokenStateLogitTransitionConfig(**payload["config"])
    model = HistoryConditionedProbabilisticTokenStateLogitTransitionModel(cfg)
...
def save_checkpoint(
    path: str,
    model: HistoryConditionedProbabilisticTokenStateLogitTransitionModel,
    cfg: HistoryConditionedProbabilisticTokenStateLogitTransitionConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
