...
from diffusion.block_ar.center_conditioned_residual_logit_level_flow_matching import (
    CenterConditionedResidualLogitLevelFlowMatching,
)
...
class StateAwareCenterConditionedResidualLevelPathVelocity(nn.Module):
    ...
    def forward(
        ...
        total_transitions = torch.cat(
            [
                total_logits[:, :1] - last_logit[:, None, :],
                total_logits[:, 1:] - total_logits[:, :-1],
            ],
            dim=1,
        )
        values = torch.stack(
            [residual_t, center_logits, total_logits, total_transitions], dim=-1
...
class StateAwareCenterConditionedResidualLogitLevelFlowMatching(
    CenterConditionedResidualLogitLevelFlowMatching
):
    """311a-v0: level-residual flow with implied daily transitions exposed."""

    def __init__(self, cfg: JointTokenLogitTransitionFMConfig):
        super().__init__(cfg)
        self.velocity = StateAwareCenterConditionedResidualLevelPathVelocity(cfg)
    ...
    def predict_velocity(
        ...
        return self.velocity(residual_t, center_logits, last_logit, context, t)
    ...
    def training_loss(
        ...
        total_transitions = torch.cat(
            [
                total_logits[:, :1] - self.history_last_logit(history_norm)[:, None, :],
                total_logits[:, 1:] - total_logits[:, :-1],
            ],
            dim=1,
        )
    ...
    def sample_with_center(
        ...
        for start in range(0, n_samples, chunk_size):
            ...
            center_logits = self.center_future_logits(cen_future)
            future_01 = torch.sigmoid(center_logits + residual)
...
class ObsWorldBackboneStateAwareCenterResidualLevelFlowGenerator(nn.Module):
    def __init__(
        self,
        flow: StateAwareCenterConditionedResidualLogitLevelFlowMatching,
        backbone: nn.Module,
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[ObsWorldBackboneStateAwareCenterResidualLevelFlowGenerator, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = JointTokenLogitTransitionFMConfig(**payload["config"])
    flow = StateAwareCenterConditionedResidualLogitLevelFlowMatching(cfg)
    ...
    model = ObsWorldBackboneStateAwareCenterResidualLevelFlowGenerator(
...
def save_checkpoint(
    path: str,
    model: StateAwareCenterConditionedResidualLogitLevelFlowMatching,
