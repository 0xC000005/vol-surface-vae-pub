...
@dataclass
class DeterministicHistoryMemoryWorldModelConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class DeterministicHistoryMemoryWorldModel(nn.Module):
    """289e-v0: global latent world model with explicit history memory reads."""

    def __init__(self, cfg: DeterministicHistoryMemoryWorldModelConfig):
        super().__init__()
        self.cfg = cfg
    ...
    def _transition_step(
        ...
        next_change = self.decoder(torch.cat([latent, obs_emb, context], dim=-1))
    ...
    def teacher_forced_changes(
        ...
        for step in range(self.cfg.future_len):
            latent, next_change = self._transition_step(latent, memory, curr_level, prev_change)
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[DeterministicHistoryMemoryWorldModel, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = DeterministicHistoryMemoryWorldModelConfig(**payload["config"])
    model = DeterministicHistoryMemoryWorldModel(cfg)
...
def save_checkpoint(
    path: str,
    model: DeterministicHistoryMemoryWorldModel,
    cfg: DeterministicHistoryMemoryWorldModelConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
