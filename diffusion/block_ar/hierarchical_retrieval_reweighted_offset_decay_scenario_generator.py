...
class HierarchicalReweightedOffsetDecayScenarioGenerator(
    HierarchicalReweightedRetrievalScenarioGenerator
):
    """285a-v0: horizon-decayed transport between raw future and anchored-delta support."""
    ...
    def sample_batched(
        ...
        **_: object,
        ...
        history_01 = history if not history_is_normalized else None
        ...
        library_last = self.library_last_level_01[chosen_idx]
        if history_01 is None:
        ...
        offset = query_last - library_last
        beta = torch.linspace(1.0, 0.0, steps=n_steps, device=history_norm.device).view(1, 1, n_steps, 1, 1)
        transported = torch.clamp(raw_future + beta * offset.unsqueeze(2), 0.0, 1.0)
...
def load_model(checkpoint_path: str, device: torch.device):
    ...
    model = HierarchicalReweightedOffsetDecayScenarioGenerator(base_model, cfg)
