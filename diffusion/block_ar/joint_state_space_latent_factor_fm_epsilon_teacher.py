...
@dataclass
class JointStateSpaceLatentFactorFMEpsilonTeacherConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
    latent_dim: int = 8
...
class JointStateSpaceLatentFactorFMEpsilonTeacher(nn.Module):
    """264b-v0: 263b prior/decoder with a learned future-factor epsilon teacher."""

    def __init__(self, cfg: JointStateSpaceLatentFactorFMEpsilonTeacherConfig):
        super().__init__()
        self.cfg = cfg
        ...
        self.posterior_eps_head = _mlp(2 * cfg.state_hidden, cfg.latent_dim, cfg.head_hidden, cfg.head_layers, cfg.head_dropout)
    ...
    def _init_heads(self) -> None:
        for net in [self.mean_init, self.scale_init, self.mu_head, self.sigma_head, self.loadings_head, self.ec_gain_head, self.short_ec_head, self.posterior_eps_head, self.flow_head]:
            last = net[-1]
    ...
    def posterior_target(self, target_coord: torch.Tensor, cond: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        eps_target = self.posterior_eps_head(post_hidden)
        sigma = cond["sigma"].detach().clamp_min(self.cfg.sigma_floor)
        z_target = cond["mu"].detach() + sigma * eps_target
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[JointStateSpaceLatentFactorFMEpsilonTeacher, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = JointStateSpaceLatentFactorFMEpsilonTeacherConfig(**payload["config"])
    model = JointStateSpaceLatentFactorFMEpsilonTeacher(cfg)
    model.load_state_dict(payload["model_state_dict"], strict=True)
    model.to(device).eval()
    return model, payload


def save_checkpoint(path: str, model: JointStateSpaceLatentFactorFMEpsilonTeacher, cfg: JointStateSpaceLatentFactorFMEpsilonTeacherConfig, epoch: int, best_val: float) -> None:
    torch.save(
        {
            "config": asdict(cfg),
            "epoch": int(epoch),
