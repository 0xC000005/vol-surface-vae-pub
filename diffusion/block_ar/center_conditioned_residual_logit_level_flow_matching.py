...
class CenterConditionedResidualLevelPathVelocity(nn.Module):
    ...
    def forward(
        ...
        total_logits = center_logits + residual_t
        values = torch.stack(
            [residual_t, center_logits, total_logits], dim=-1
...
class CenterConditionedResidualLogitLevelFlowMatching(
    JointTokenLogitTransitionFlowMatching
):
    """310a-v0: vanilla joint residual flow over future logit levels around a learned center."""

    def __init__(self, cfg: JointTokenLogitTransitionFMConfig):
        super().__init__(cfg)
        self.velocity = CenterConditionedResidualLevelPathVelocity(cfg)

    def future_logits(self, future_norm: torch.Tensor) -> torch.Tensor:
        future_norm = self._flatten(future_norm)
        return iv_to_logit(denormalize_iv(future_norm), self.cfg.logit_eps)

    def center_future_logits(self, center_future_norm: torch.Tensor) -> torch.Tensor:
        center_future_norm = self._flatten(center_future_norm)
        return iv_to_logit(denormalize_iv(center_future_norm), self.cfg.logit_eps)
    ...
    def target_residual(
        ...
        return self.future_logits(future_norm) - self.center_future_logits(center_future_norm)
    ...
    def predict_velocity(
        ...
        center_logits = self.center_future_logits(center_future_norm)
        return self.velocity(residual_t, center_logits, context, t)
    ...
    def training_loss(
        ...
        center_logits = self.center_future_logits(center_future_norm)
        residual = self.target_residual(center_future_norm, future_norm)
        ...
        pred_velocity = self.predict_velocity(residual_t, center_future_norm, context, t)
        fm_loss = F.mse_loss(pred_velocity, target_velocity)
        total_logits = center_logits + residual
        metrics = {
            "total": fm_loss.detach(),
            "fm_loss": fm_loss.detach(),
            "center_logit_std": center_logits.std(unbiased=False).detach(),
            "center_logit_abs": center_logits.abs().mean().detach(),
            "residual_std": residual.std(unbiased=False).detach(),
            "residual_abs": residual.abs().mean().detach(),
            "total_logit_std": total_logits.std(unbiased=False).detach(),
        }
    ...
    def sample_with_center(
        ...
        for start in range(0, n_samples, chunk_size):
            ...
            cen_logits = center_logits.repeat_interleave(k, dim=0)
            ...
            for step in range(self.cfg.flow_steps):
                ...
                residual = residual + dt * self.predict_velocity(
                    residual, cen_future, ctx, t
                )
            future_01 = logit_to_iv(cen_logits + residual)
...
class ObsWorldBackboneCenterResidualLevelFlowGenerator(nn.Module):
    def __init__(
        self,
        flow: CenterConditionedResidualLogitLevelFlowMatching,
        backbone: nn.Module,
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[ObsWorldBackboneCenterResidualLevelFlowGenerator, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = JointTokenLogitTransitionFMConfig(**payload["config"])
    flow = CenterConditionedResidualLogitLevelFlowMatching(cfg)
    ...
    model = ObsWorldBackboneCenterResidualLevelFlowGenerator(
...
def save_checkpoint(
    path: str,
    model: CenterConditionedResidualLogitLevelFlowMatching,
