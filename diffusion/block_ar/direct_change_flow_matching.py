...
class DirectChangeVelocity(nn.Module):
    def __init__(self, cfg: DirectChangeFMConfig):
        super().__init__()
        self.cfg = cfg
...
class DirectChangeFlowMatching(nn.Module):
    """268c-v0: direct conditional future-change flow matching."""
    ...
    def __init__(self, cfg: DirectChangeFMConfig):
        ...
        self.velocity = DirectChangeVelocity(cfg)
    ...
    @staticmethod
    def _future_changes(history_norm: torch.Tensor, future_norm: torch.Tensor) -> torch.Tensor:
        prev = torch.cat([history_norm[:, -1:, :], future_norm[:, :-1, :]], dim=1)
        return future_norm - prev
    ...
    def training_loss(
        ...
        x1 = self._future_changes(history_norm, future_norm)
    ...
    def sample_batched(
        ...
        **_: object,
        ...
        for start in range(0, n_samples, chunk_size):
            ...
            base = last_hist.repeat_interleave(k, dim=0)
            for step in range(self.cfg.flow_steps):
                ...
                v = self.velocity(x, ctx, t)
                x = x + dt * v
            future_norm = base[:, None, :] + torch.cumsum(x, dim=1)
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[DirectChangeFlowMatching, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = DirectChangeFMConfig(**payload["config"])
    model = DirectChangeFlowMatching(cfg)
...
def save_checkpoint(
    path: str,
    model: DirectChangeFlowMatching,
    cfg: DirectChangeFMConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
