...
@dataclass
class LearnedRetrievalRichTargetConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
    embed_dim: int = 96
...
class LearnedRetrievalRichTargetBackbone(nn.Module):
    """277d-v0: learned retrieval with change-plus-displacement future keys."""
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[LearnedRetrievalRichTargetBackbone, dict]:
