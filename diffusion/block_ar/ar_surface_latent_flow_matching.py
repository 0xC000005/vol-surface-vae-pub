...
class LatentVelocity(nn.Module):
    def __init__(self, cfg: ARSurfaceLatentFMConfig):
        super().__init__()
        self.cfg = cfg
        self.net = _mlp(
            in_dim=cfg.latent_dim + cfg.latent_dim + cfg.time_dim,
    ...
    def forward(self, delta_t: torch.Tensor, z_prev: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        return self.net(torch.cat([delta_t, z_prev, _time_features(t, self.cfg.time_dim)], dim=-1))


class ARSurfaceLatentFlowMatching(nn.Module):
    """272a-v0: AR latent state-space FM with explicit observation autoencoder."""
    ...
    def __init__(self, cfg: ARSurfaceLatentFMConfig):
        ...
        self.transition = LatentVelocity(cfg)
        self.decoder = _mlp(
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[ARSurfaceLatentFlowMatching, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = ARSurfaceLatentFMConfig(**payload["config"])
    model = ARSurfaceLatentFlowMatching(cfg)
...
def save_checkpoint(
    path: str,
    model: ARSurfaceLatentFlowMatching,
    cfg: ARSurfaceLatentFMConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
