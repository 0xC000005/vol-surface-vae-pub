...
@dataclass
class DeterministicHistoryMemoryJointSupportWorldModelConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class DeterministicHistoryMemoryJointSupportWorldModel(nn.Module):
    """291a-v0: history-memory world model with learned joint next-change support."""

    def __init__(self, cfg: DeterministicHistoryMemoryJointSupportWorldModelConfig):
        super().__init__()
        self.cfg = cfg
        ...
        self.support_head = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(cfg.decoder_hidden, cfg.support_dim),
        )
        self.support_encoder = nn.Sequential(
            nn.Linear(cfg.n_cells, cfg.support_hidden),
            nn.GELU(),
            nn.Linear(cfg.support_hidden, cfg.support_dim),
            nn.Tanh(),
        )
        self.support_decoder = nn.Sequential(
            nn.Linear(cfg.support_dim, cfg.support_hidden),
            nn.GELU(),
            nn.Linear(cfg.support_hidden, cfg.n_cells),
        )
    ...
    def _decode_change(self, support_code: torch.Tensor) -> torch.Tensor:
        return self.support_decoder(support_code)
    ...
    def _transition_step(
        ...
        support_code = self.support_head(torch.cat([latent, obs_emb, context], dim=-1))
        next_change = self._decode_change(support_code)
        return latent, support_code, next_change

    def teacher_forced_codes_and_changes(
        self,
        ...
        pred_codes: list[torch.Tensor] = []
        pred_changes: list[torch.Tensor] = []
        for step in range(self.cfg.future_len):
            latent, support_code, next_change = self._transition_step(
                latent, memory, curr_level, prev_change
            )
            pred_codes.append(support_code)
            pred_changes.append(next_change)
            next_level = future[:, step]
            prev_change = next_level - curr_level
            curr_level = next_level
        return torch.stack(pred_codes, dim=1), torch.stack(pred_changes, dim=1)
    ...
    def rollout(self, history_norm: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for _ in range(self.cfg.future_len):
            latent, _support_code, next_change = self._transition_step(
                latent, memory, curr_level, prev_change
            )
    ...
    def training_loss(
        ...
        target_codes = self.support_encoder(target_changes)
        target_recon = self._decode_change(target_codes)
        pred_codes, teacher_pred_changes = self.teacher_forced_codes_and_changes(hist, future)
        rollout_levels, rollout_changes = self.rollout(hist)

        recon_loss = F.smooth_l1_loss(target_recon, target_changes)
        code_loss = F.smooth_l1_loss(pred_codes, target_codes.detach())
        ...
        total = (
            ...
            + self.cfg.code_weight * code_loss
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[DeterministicHistoryMemoryJointSupportWorldModel, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = DeterministicHistoryMemoryJointSupportWorldModelConfig(**payload["config"])
    model = DeterministicHistoryMemoryJointSupportWorldModel(cfg)
...
def save_checkpoint(
    path: str,
    model: DeterministicHistoryMemoryJointSupportWorldModel,
    cfg: DeterministicHistoryMemoryJointSupportWorldModelConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
