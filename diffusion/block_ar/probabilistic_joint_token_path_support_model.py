...
@dataclass
class ProbabilisticJointTokenPathSupportModelConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class ProbabilisticJointTokenPathSupportModel(nn.Module):
    """293d-v0: fixed-horizon joint-token law with explicit coarse path support."""

    def __init__(
        self,
        cfg: ProbabilisticJointTokenPathSupportModelConfig,
    ...
    def coarse_targets(self, future_norm: torch.Tensor) -> torch.Tensor:
        future = self._flatten_history(future_norm)
        knot_levels = future[:, self.knot_steps.to(future.device)]
        return knot_levels
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[ProbabilisticJointTokenPathSupportModel, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = ProbabilisticJointTokenPathSupportModelConfig(**payload["config"])
    model = ProbabilisticJointTokenPathSupportModel(
        cfg,
