...
@dataclass
class DeterministicObsEncodedLatentWorldModelConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class DeterministicObsEncodedLatentWorldModel(nn.Module):
    """289c-v0: deterministic latent world model with explicit observation encoder."""

    def __init__(self, cfg: DeterministicObsEncodedLatentWorldModelConfig):
        super().__init__()
        self.cfg = cfg
        ...
        self.transition = nn.GRUCell(cfg.obs_hidden, cfg.latent_dim)
        self.decoder = nn.Sequential(
            nn.Linear(cfg.latent_dim + cfg.obs_hidden, cfg.decoder_hidden),
            nn.GELU(),
    ...
    def _transition_step(
        ...
        latent = self.transition(obs_emb, latent)
        next_change = self.decoder(torch.cat([latent, obs_emb], dim=-1))
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[DeterministicObsEncodedLatentWorldModel, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = DeterministicObsEncodedLatentWorldModelConfig(**payload["config"])
    model = DeterministicObsEncodedLatentWorldModel(cfg)
...
def save_checkpoint(
    path: str,
    model: DeterministicObsEncodedLatentWorldModel,
    cfg: DeterministicObsEncodedLatentWorldModelConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
