...
@dataclass
class ARSeq2SeqTransformerJointTokenConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class ARSeq2SeqTransformerJointTokenBackbone(nn.Module):
    """276b-v0: deterministic seq2seq with a joint token over full next-step changes."""

    def __init__(self, cfg: ARSeq2SeqTransformerJointTokenConfig, codebook: torch.Tensor):
        super().__init__()
        self.cfg = cfg
    ...
    def rollout(self, history_norm: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for _ in range(self.cfg.future_len):
            ...
            hist_window = torch.cat([hist_window[:, 1:], next_level.unsqueeze(1)], dim=1)
            curr = next_level
        return torch.stack(pred_levels, dim=1), torch.stack(pred_vecs, dim=1), torch.stack(pred_ids, dim=1)
    ...
    def training_loss(
        ...
        loss = F.cross_entropy(logits.reshape(-1, self.cfg.codebook_size), target_ids.reshape(-1))
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[ARSeq2SeqTransformerJointTokenBackbone, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = ARSeq2SeqTransformerJointTokenConfig(**payload["config"])
    model = ARSeq2SeqTransformerJointTokenBackbone(cfg, payload["codebook"].to(device))
...
def save_checkpoint(
    path: str,
    model: ARSeq2SeqTransformerJointTokenBackbone,
    cfg: ARSeq2SeqTransformerJointTokenConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
