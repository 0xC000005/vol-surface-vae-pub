...
@dataclass
class StateAwareLogitTransitionFMConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class StateAwareLogitTransitionVelocity(nn.Module):
    def __init__(self, cfg: StateAwareLogitTransitionFMConfig):
        super().__init__()
        self.cfg = cfg
        self.pos_embed = nn.Embedding(cfg.future_len, cfg.pos_dim)
        in_dim = 2 * cfg.n_cells + cfg.context_dim + cfg.time_dim + cfg.pos_dim
    ...
    def forward(
        ...
        hidden = self.in_proj(torch.cat([x_t, state_logits, ctx, time, pos], dim=-1))
        hidden = self.backbone(hidden)
        return self.out(hidden)


class StateAwareLogitTransitionFlowMatching(nn.Module):
    """302a-v0: support-valid logit-transition flow with implied state path exposed."""
    ...
    def __init__(self, cfg: StateAwareLogitTransitionFMConfig):
        ...
        self.velocity = StateAwareLogitTransitionVelocity(cfg)
    ...
    def transitions_to_future(self, history_norm: torch.Tensor, transitions: torch.Tensor) -> torch.Tensor:
        state_logits = self.implied_state_logits(history_norm, transitions)
        return logit_to_iv(state_logits)
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[StateAwareLogitTransitionFlowMatching, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = StateAwareLogitTransitionFMConfig(**payload["config"])
    model = StateAwareLogitTransitionFlowMatching(cfg)
...
def save_checkpoint(
    path: str,
    model: StateAwareLogitTransitionFlowMatching,
    cfg: StateAwareLogitTransitionFMConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
