...
@dataclass
class ProbabilisticJointTokenPathAnchorRefineModelConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class ProbabilisticJointTokenPathAnchorRefineModel(nn.Module):
    """293f-v0: monolithic coarse anchor plus light knotwise refinement."""

    def __init__(
        self,
        cfg: ProbabilisticJointTokenPathAnchorRefineModelConfig,
        ...
        expected_coarse_shape = (cfg.coarse_codebook_size, cfg.n_knots, cfg.n_cells)
        if coarse_codebook.shape != expected_coarse_shape:
            raise ValueError(
                f"Expected coarse codebook {expected_coarse_shape}, got {tuple(coarse_codebook.shape)}"
            )
        ...
        self.knot_pos = nn.Embedding(cfg.n_knots, cfg.d_model)
        ...
        self.knot_encoder = nn.TransformerEncoder(enc_layer, num_layers=1)
        ...
        self.coarse_head = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(cfg.d_model, cfg.coarse_codebook_size),
        )
    ...
    def assign_coarse_codes(self, coarse_targets: torch.Tensor) -> torch.Tensor:
        flat = coarse_targets.reshape(coarse_targets.shape[0], -1)
        code_flat = self.coarse_codebook.reshape(self.cfg.coarse_codebook_size, -1)
        dists = torch.cdist(flat, code_flat)
        return dists.argmin(dim=-1)

    def coarse_vectors(self, coarse_ids: torch.Tensor) -> torch.Tensor:
        return self.coarse_codebook[coarse_ids]

    def refine_targets(self, coarse_targets: torch.Tensor, anchor_panels: torch.Tensor) -> torch.Tensor:
        return (coarse_targets - anchor_panels) / self.cfg.refine_scale
    ...
    def encode_knot_context(self, anchor_panels: torch.Tensor) -> torch.Tensor:
        pos = torch.arange(self.cfg.n_knots, device=anchor_panels.device).unsqueeze(0)
        x = self.coarse_proj(anchor_panels) + self.knot_pos(pos)
        return self.knot_encoder(x)

    def predict_refinement(self, memory: torch.Tensor, anchor_panels: torch.Tensor) -> torch.Tensor:
        hist_summary = memory.mean(dim=1, keepdim=True).expand(-1, self.cfg.n_knots, -1)
        knot_ctx = self.encode_knot_context(anchor_panels)
        refine_in = torch.cat([hist_summary, knot_ctx], dim=-1)
        return torch.tanh(self.refine_head(refine_in))
    ...
    def training_loss(
        ...
        coarse_ids = self.assign_coarse_codes(coarse_targets)
        anchor_panels = self.coarse_vectors(coarse_ids)

        memory = self.encode_history(history_norm)
        coarse_logits = self.coarse_head(memory.mean(dim=1))
        ...
        pred_refine = self.predict_refinement(memory, anchor_panels)
        target_refine = self.refine_targets(coarse_targets, anchor_panels)
        refine_loss = F.smooth_l1_loss(pred_refine, target_refine)
        refined_knots = anchor_panels + self.cfg.refine_scale * pred_refine
        coarse_scaffold = self.expand_coarse_scaffold(refined_knots)
        ...
        loss = token_nll + self.cfg.coarse_loss_weight * coarse_ce + self.cfg.refine_weight * refine_loss

        probs = torch.softmax(logits, dim=-1)
        pred_ids = logits.argmax(dim=-1)
        token_acc = (pred_ids == target_ids).float().mean()
        coarse_acc = (coarse_logits.argmax(dim=-1) == coarse_ids).float().mean()
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[ProbabilisticJointTokenPathAnchorRefineModel, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = ProbabilisticJointTokenPathAnchorRefineModelConfig(**payload["config"])
    model = ProbabilisticJointTokenPathAnchorRefineModel(
        cfg,
