"""
253a: dynamic change-space factor state-space model.

Purpose:
    Architectural follow-up to 252a's failure. Keep the explicit low-rank thesis,
    but move from a static decoded center path to a dynamic latent change process.

Core design:
    - deterministic dynamic latent factor state z_t
    - static low-rank loading matrix Lambda learned from history
    - hard idiosyncratic budget tied to factor RMS
    - optional bounded error-correction term for MR / cointegration

This remains non-AR across *windows* but is dynamic within the generated path.
"""
...
class DynamicChangeFactorSSM(nn.Module):
    def __init__(self, cfg: DynamicFactorSSMConfig):
        super().__init__()
        self.cfg = cfg
    ...
    def _init_heads(self) -> None:
        for head in [self.state_init, self.loading_head, self.state_gate, self.state_prop, self.factor_gate]:
            last = head[-1]
        ...
        for head in [self.idio_head, self.idio_budget_head]:
            last = head[-1]
            if isinstance(last, nn.Linear):
                with torch.no_grad():
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[DynamicChangeFactorSSM, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = DynamicFactorSSMConfig(**payload["config"])
    model = DynamicChangeFactorSSM(cfg).to(device)
    model.load_state_dict(payload["model_state_dict"], strict=True)
    model.eval()
    return model, payload


def config_to_dict(cfg: DynamicFactorSSMConfig) -> dict:
    return asdict(cfg)
