...
@dataclass
class ProbabilisticBackboneBudgetedMultiresolutionZeroMeanCoarseShellConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
    ...
    knot_positions: tuple[int, ...] = (1, 2, 3, 5, 10, 15, 22, 30)
    scale_floor: float = 0.02
    budget_floor: float = 0.05
    sample_scale_mult: float = 1.0


class BudgetedMultiresolutionZeroMeanCoarseShellCore(nn.Module):
    """296f-v0 shell: multiresolution Gaussian shell with explicit scale budget."""

    def __init__(self, cfg: ProbabilisticBackboneBudgetedMultiresolutionZeroMeanCoarseShellConfig):
        super().__init__()
        self.cfg = cfg
        ...
        self.raw_scale_head = nn.Sequential(
    ...
    def training_loss(
        ...
        knot_scale, center_raw, _center_scales, budget, daily_scale = self.predict_knot_scales(
    ...
    def sample_with_center(
        ...
        knot_scale, center_raw, center_scales, _budget, _daily_scale = self.predict_knot_scales(
            history_norm, center_future_norm
        )
...
class BackboneBudgetedMultiresolutionZeroMeanCoarseShellGenerator(nn.Module):
    def __init__(
        self,
        shell: BudgetedMultiresolutionZeroMeanCoarseShellCore,
        backbone: nn.Module,
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[BackboneBudgetedMultiresolutionZeroMeanCoarseShellGenerator, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = ProbabilisticBackboneBudgetedMultiresolutionZeroMeanCoarseShellConfig(**payload["config"])
    shell = BudgetedMultiresolutionZeroMeanCoarseShellCore(cfg)
    ...
    model = BackboneBudgetedMultiresolutionZeroMeanCoarseShellGenerator(
...
def save_checkpoint(
    path: str,
    shell: BudgetedMultiresolutionZeroMeanCoarseShellCore,
    cfg: ProbabilisticBackboneBudgetedMultiresolutionZeroMeanCoarseShellConfig,
    epoch: int,
    best_val: float,
    backbone_checkpoint_path: str,
) -> None:
