...
@dataclass
class AxialMixerFutureLogitPathFMConfig(JointTokenLogitTransitionFMConfig):
    standardize_logits: bool = True
    logit_std_floor: float = 1e-3


class AxialMixerBlock(nn.Module):
    """Factorized time/cell/channel mixing for full future-path flow tokens."""

    def __init__(self, cfg: AxialMixerFutureLogitPathFMConfig):
        super().__init__()
        self.time_norm = nn.LayerNorm(cfg.future_len)
        self.time_mlp = nn.Sequential(
            nn.Linear(cfg.future_len, 2 * cfg.future_len),
            nn.GELU(),
            nn.Dropout(cfg.model_dropout),
            nn.Linear(2 * cfg.future_len, cfg.future_len),
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (batch, horizon, cell, channel)
        y = self.time_norm(x.permute(0, 2, 3, 1))
        y = self.time_mlp(y).permute(0, 3, 1, 2)
        x = x + self.dropout(y)

        y = self.cell_norm(x.permute(0, 1, 3, 2))
        y = self.cell_mlp(y).permute(0, 1, 3, 2)
        x = x + self.dropout(y)
...
class AxialMixerFutureLogitPathVelocity(nn.Module):
    def __init__(self, cfg: AxialMixerFutureLogitPathFMConfig):
        super().__init__()
        self.cfg = cfg
    ...
    def forward(
        ...
        values = torch.stack([future_logits_t, implied_transitions_t], dim=-1)
...
class AxialMixerFutureLogitPathFlowMatching(nn.Module):
    """324c-v0: direct full future-logit path flow with efficient axial mixing."""
    ...
    def __init__(self, cfg: AxialMixerFutureLogitPathFMConfig):
        ...
        self.velocity = AxialMixerFutureLogitPathVelocity(cfg)
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[AxialMixerFutureLogitPathFlowMatching, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = AxialMixerFutureLogitPathFMConfig(**payload["config"])
    model = AxialMixerFutureLogitPathFlowMatching(cfg)
...
def save_checkpoint(
    path: str,
    model: AxialMixerFutureLogitPathFlowMatching,
    cfg: AxialMixerFutureLogitPathFMConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
