...
class UnifiedStateAwareFutureLogitPathVelocity(nn.Module):
...
class UnifiedStateAwareFutureLogitPathFlowMatching(JointTokenLogitTransitionFlowMatching):
    """312a-v0: single-stage unified future-logit path flow with implied transitions exposed."""

    def __init__(self, cfg: JointTokenLogitTransitionFMConfig):
        super().__init__(cfg)
        self.velocity = UnifiedStateAwareFutureLogitPathVelocity(cfg)
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[UnifiedStateAwareFutureLogitPathFlowMatching, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = JointTokenLogitTransitionFMConfig(**payload["config"])
    model = UnifiedStateAwareFutureLogitPathFlowMatching(cfg)
...
def save_checkpoint(
    path: str,
    model: UnifiedStateAwareFutureLogitPathFlowMatching,
