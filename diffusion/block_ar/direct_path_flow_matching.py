...
class DirectPathVelocity(nn.Module):
    def __init__(self, cfg: DirectPathFMConfig):
        super().__init__()
        self.cfg = cfg
...
class DirectPathFlowMatching(nn.Module):
    ...
    def __init__(self, cfg: DirectPathFMConfig):
        ...
        self.velocity = DirectPathVelocity(cfg)
        ...
        **_: object,
        ...
        for start in range(0, n_samples, chunk_size):
            ...
            for step in range(self.cfg.flow_steps):
                ...
                v = self.predict_velocity(x, ctx, t)
                x = x + dt * v
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[DirectPathFlowMatching, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = DirectPathFMConfig(**payload["config"])
    model = DirectPathFlowMatching(cfg)
...
def save_checkpoint(
    path: str,
    model: DirectPathFlowMatching,
    cfg: DirectPathFMConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
