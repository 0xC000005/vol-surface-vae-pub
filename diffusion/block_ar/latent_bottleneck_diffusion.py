...
@dataclass
class LatentBottleneckDiffusionConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class FuturePathDecoder(nn.Module):
    def __init__(self, cfg: LatentBottleneckDiffusionConfig):
        super().__init__()
        self.cfg = cfg
        ...
        self.init_proj = nn.Linear(cfg.context_dim + cfg.latent_dim, cfg.decoder_hidden)
        self.gru = nn.GRU(
    ...
    def forward(self, context: torch.Tensor, latent: torch.Tensor) -> torch.Tensor:
        bsz = context.shape[0]
        ctx = torch.cat([context, latent], dim=-1)
        pos_idx = torch.arange(self.cfg.future_len, device=context.device)
        pos = self.pos_embed(pos_idx)[None, :, :].expand(bsz, -1, -1)
        tokens = torch.cat([ctx[:, None, :].expand(-1, self.cfg.future_len, -1), pos], dim=-1)
        tokens = self.token_proj(tokens)
        h0 = self.init_proj(ctx).unsqueeze(0).expand(self.cfg.decoder_layers, -1, -1).contiguous()
        hidden, _ = self.gru(tokens, h0)
        return self.out(hidden)


class LatentBottleneckDiffusion(nn.Module):
    """266a-v0: fixed-horizon conditional bottleneck generator with latent diffusion."""
    ...
    def __init__(self, cfg: LatentBottleneckDiffusionConfig):
        ...
        fut_cfg = EncoderConfig(
            input_dim=cfg.n_cells,
            gru_hidden_dim=cfg.future_hidden,
            bottleneck_dim=cfg.latent_dim,
            dropout=cfg.encoder_dropout,
            cond_aug_sigma=0.0,
        )
        self.history_encoder = GRUEncoder(hist_cfg)
        self.future_encoder = GRUEncoder(fut_cfg)
        ...
        self.denoiser = _mlp(
            cfg.latent_dim + cfg.context_dim + cfg.timestep_dim,
            cfg.latent_dim,
            cfg.denoiser_hidden,
            cfg.denoiser_layers,
    ...
    def encode_future(self, future_norm: torch.Tensor) -> torch.Tensor:
        return self.latent_norm(self.future_encoder(self._flatten(future_norm)))

    def decode_future(self, context: torch.Tensor, latent: torch.Tensor) -> torch.Tensor:
        return self.decoder(context, latent)
    ...
    def predict_noise(self, latent_noisy: torch.Tensor, context: torch.Tensor, t_idx: torch.Tensor) -> torch.Tensor:
        ...
        denoise_in = torch.cat([latent_noisy, context, t_emb], dim=-1)
    ...
    def q_sample(self, latent_clean: torch.Tensor, t_idx: torch.Tensor, noise: torch.Tensor) -> torch.Tensor:
        return (
            self.sqrt_alpha_bars[t_idx][:, None] * latent_clean
            + self.sqrt_one_minus_alpha_bars[t_idx][:, None] * noise
        )
    ...
    def sample_latents(self, context: torch.Tensor, n_samples: int) -> torch.Tensor:
        ...
        latent = torch.randn(bsz * n_samples, self.cfg.latent_dim, device=device, dtype=context.dtype)
        ...
        for step in reversed(range(self.cfg.diffusion_steps)):
            ...
            if step > 0:
                ...
                latent = mean
        return latent.view(bsz, n_samples, self.cfg.latent_dim)
    ...
    def sample_batched(
        ...
        **_: object,
        ...
        for start in range(0, n_samples, chunk_size):
            ...
            flat_latent = latent.reshape(bsz * k, self.cfg.latent_dim)
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[LatentBottleneckDiffusion, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = LatentBottleneckDiffusionConfig(**payload["config"])
    model = LatentBottleneckDiffusion(cfg)
...
def save_checkpoint(
    path: str,
    model: LatentBottleneckDiffusion,
    cfg: LatentBottleneckDiffusionConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
