...
@dataclass
class JointStateSpaceLatentFactorFMECConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
    latent_dim: int = 8
...
class JointStateSpaceLatentFactorFMEC(nn.Module):
    """263b-v0: 263a state-space backbone plus bounded history-mean EC baseline."""

    def __init__(self, cfg: JointStateSpaceLatentFactorFMECConfig):
        super().__init__()
        self.cfg = cfg
    ...
    def _init_heads(self) -> None:
        for net in [self.mean_init, self.scale_init, self.mu_head, self.sigma_head, self.loadings_head, self.ec_gain_head, self.short_ec_head, self.flow_head]:
            last = net[-1]
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[JointStateSpaceLatentFactorFMEC, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = JointStateSpaceLatentFactorFMECConfig(**payload["config"])
    model = JointStateSpaceLatentFactorFMEC(cfg)
    model.load_state_dict(payload["model_state_dict"], strict=True)
    model.to(device).eval()
    return model, payload


def save_checkpoint(path: str, model: JointStateSpaceLatentFactorFMEC, cfg: JointStateSpaceLatentFactorFMECConfig, epoch: int, best_val: float) -> None:
    torch.save(
        {
            "config": asdict(cfg),
            "epoch": int(epoch),
