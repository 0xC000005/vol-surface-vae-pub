...
@dataclass
class JointStateSpaceLatentFactorFMPosteriorTeacherConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
    latent_dim: int = 8
...
class JointStateSpaceLatentFactorFMPosteriorTeacher(nn.Module):
    """264a-v0: 263b prior/decoder with a learned future-factor posterior teacher."""

    def __init__(self, cfg: JointStateSpaceLatentFactorFMPosteriorTeacherConfig):
        super().__init__()
        self.cfg = cfg
        ...
        self.posterior_gru = nn.GRU(
            input_size=cfg.n_cells + cfg.bottleneck_dim + cfg.future_pos_embed,
            hidden_size=cfg.state_hidden,
            batch_first=True,
            bidirectional=True,
        )
        self.posterior_mu_head = _mlp(2 * cfg.state_hidden, cfg.latent_dim, cfg.head_hidden, cfg.head_layers, cfg.head_dropout)
    ...
    def _init_heads(self) -> None:
        for net in [self.mean_init, self.scale_init, self.mu_head, self.sigma_head, self.loadings_head, self.ec_gain_head, self.short_ec_head, self.posterior_mu_head, self.flow_head]:
            last = net[-1]
    ...
    def condition(self, history_norm: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        for t in range(self.cfg.future_len):
            ...
            sigma_t = self.cfg.sigma_floor + F.softplus(self.sigma_head(scale_state))
    ...
    def posterior_target(self, target_coord: torch.Tensor, cond: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        bsz, steps, _ = target_coord.shape
        pos = self.future_pos.weight[:steps].unsqueeze(0).expand(bsz, -1, -1)
        h_ctx = cond["h"].unsqueeze(1).expand(-1, steps, -1)
        post_in = torch.cat([target_coord, h_ctx, pos], dim=-1)
        post_hidden, _ = self.posterior_gru(post_in)
        z_post = self.posterior_mu_head(post_hidden)
        sigma = cond["sigma"].detach().clamp_min(self.cfg.sigma_floor)
        eps_target = (z_post - cond["mu"].detach()) / sigma
        return z_post, eps_target
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[JointStateSpaceLatentFactorFMPosteriorTeacher, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = JointStateSpaceLatentFactorFMPosteriorTeacherConfig(**payload["config"])
    model = JointStateSpaceLatentFactorFMPosteriorTeacher(cfg)
    model.load_state_dict(payload["model_state_dict"], strict=True)
    model.to(device).eval()
    return model, payload


def save_checkpoint(path: str, model: JointStateSpaceLatentFactorFMPosteriorTeacher, cfg: JointStateSpaceLatentFactorFMPosteriorTeacherConfig, epoch: int, best_val: float) -> None:
    torch.save(
        {
            "config": asdict(cfg),
            "epoch": int(epoch),
