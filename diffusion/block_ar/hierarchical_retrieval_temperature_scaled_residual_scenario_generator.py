...
@dataclass
class HierarchicalTemperatureScaledResidualRetrievalConfig:
    top_k: int = 16
    tau_min: float = 0.05
    tau_max: float = 1.5
...
class HierarchicalTemperatureScaledResidualRetrievalScenarioGenerator(torch.nn.Module):
    """281b-v0: fixed center path plus learned residual temperature and scalar scale."""
    ...
    def __init__(
        ...
        self.cfg = HierarchicalTemperatureScaledResidualRetrievalConfig(
            top_k=top_k,
            tau_min=tau_min,
            tau_max=tau_max,
            min_scale=min_scale,
    ...
    def predict_controls(self, query_z: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        raw = self.head(query_z)
        tau = self.cfg.tau_min + (self.cfg.tau_max - self.cfg.tau_min) * torch.sigmoid(raw[:, 0])
        scale = F.softplus(raw[:, 1]) + self.cfg.min_scale
        return tau, scale
    ...
    def build_weighted_residual_bank(
        ...
        tau, scale = self.predict_controls(query_z)
        ...
        return anchored_center, centered_residuals, weights, tau, scale
    ...
    def training_loss(
        ...
        anchored_center, centered_residuals, weights, tau, scale = self.build_weighted_residual_bank(history_norm)
        actual_residual = future_01 - anchored_center
        scaled_residuals = scale.view(-1, 1, 1, 1, 1) * centered_residuals
        dists = (scaled_residuals - actual_residual.unsqueeze(1)).pow(2).mean(dim=(2, 3, 4))
    ...
    def sample_batched(
        ...
        **_: object,
        ...
        anchored_center, centered_residuals, weights, _tau, scale = self.build_weighted_residual_bank(history_norm)
        ...
        scenarios = torch.clamp(
            anchored_center.unsqueeze(1) + scale.view(-1, 1, 1, 1, 1) * sampled_residuals,
            0.0,
            1.0,
        )
