...
class UnifiedHistoryMemoryStateAwareFutureLogitPathVelocity(nn.Module):
    def __init__(self, cfg: JointTokenLogitTransitionFMConfig):
        ...
        self.token_type_embed = nn.Parameter(torch.zeros(3, cfg.token_dim))
    ...
    def forward(
        ...
        hidden = torch.cat([time_token[:, None, :], hist_token, future_token], dim=1)
        hidden = self.mixer(hidden)
        vel = self.out(hidden[:, 1 + history_tokens.shape[1] :, :]).squeeze(-1)
        return vel.view(bsz, horizon, n_cells)


class UnifiedHistoryMemoryStateAwareFutureLogitPathFlowMatching(nn.Module):
    """312b-v0: unified future-logit path flow with explicit history-token memory."""

    def __init__(self, cfg: JointTokenLogitTransitionFMConfig):
        super().__init__()
        self.cfg = cfg
        self.velocity = UnifiedHistoryMemoryStateAwareFutureLogitPathVelocity(cfg)
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[UnifiedHistoryMemoryStateAwareFutureLogitPathFlowMatching, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = JointTokenLogitTransitionFMConfig(**payload["config"])
    model = UnifiedHistoryMemoryStateAwareFutureLogitPathFlowMatching(cfg)
...
def save_checkpoint(
    path: str,
    model: UnifiedHistoryMemoryStateAwareFutureLogitPathFlowMatching,
