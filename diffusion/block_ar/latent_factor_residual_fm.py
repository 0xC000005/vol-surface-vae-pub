...
@dataclass
class LatentFactorResidualFMConfig:
    base_checkpoint: str
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class LatentFactorResidualFM(nn.Module):
    """261b-v0: residual FM in frozen factor space on top of 260e."""
    ...
    def sample_batched(
        ...
        **_: object,
        ...
        for start in range(0, n_samples, chunk_size):
            ...
            factor_resid = self.sample_factor_residuals(history_norm, n_samples=k, cond=cond)
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[LatentFactorResidualFM, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = LatentFactorResidualFMConfig(**payload["config"])
    base_model, _ = load_260_model(cfg.base_checkpoint, device)
    model = LatentFactorResidualFM(cfg, base_model=base_model)
