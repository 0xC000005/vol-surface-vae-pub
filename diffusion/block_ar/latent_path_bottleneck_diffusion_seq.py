...
@dataclass
class LatentPathBottleneckDiffusionSeqConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
...
class FuturePathEncoder(nn.Module):
    def __init__(self, cfg: LatentPathBottleneckDiffusionSeqConfig):
        super().__init__()
        self.cfg = cfg
        self.gru = nn.GRU(
...
class TokenSequenceDenoiser(nn.Module):
    def __init__(self, cfg: LatentPathBottleneckDiffusionSeqConfig):
        super().__init__()
        self.cfg = cfg
        self.pos_embed = nn.Embedding(cfg.latent_tokens, cfg.pos_dim)
        in_dim = cfg.latent_dim + cfg.context_dim + cfg.timestep_dim + cfg.pos_dim
        self.token_proj = nn.Sequential(
            nn.Linear(in_dim, cfg.denoiser_hidden),
            nn.GELU(),
            nn.Linear(cfg.denoiser_hidden, cfg.denoiser_hidden),
        )
        self.backbone = TemporalConvTower(
            channels=cfg.denoiser_hidden,
            layers=cfg.denoiser_layers,
        ...
        self.out = nn.Sequential(
            nn.Linear(cfg.denoiser_hidden, cfg.denoiser_hidden),
            nn.GELU(),
            nn.Linear(cfg.denoiser_hidden, cfg.latent_dim),
        )

    def forward(self, latent_noisy: torch.Tensor, context: torch.Tensor, time_emb: torch.Tensor) -> torch.Tensor:
        bsz, k, _ = latent_noisy.shape
        pos_idx = torch.arange(k, device=latent_noisy.device)
        pos = self.pos_embed(pos_idx)[None, :, :].expand(bsz, -1, -1)
        ctx = context[:, None, :].expand(-1, k, -1)
        t = time_emb[:, None, :].expand(-1, k, -1)
        tokens = self.token_proj(torch.cat([latent_noisy, ctx, t, pos], dim=-1))
        hidden = self.backbone(tokens)
        return self.out(hidden)


class LatentPathBottleneckDiffusionSeq(nn.Module):
    """266c-v0: temporal bottleneck with sequence-aware latent diffusion prior."""
    ...
    def __init__(self, cfg: LatentPathBottleneckDiffusionSeqConfig):
        ...
        self.denoiser = TokenSequenceDenoiser(cfg)
    ...
    def predict_noise(self, latent_noisy: torch.Tensor, context: torch.Tensor, t_idx: torch.Tensor) -> torch.Tensor:
        ...
        return self.denoiser(latent_noisy, context, t_emb)
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[LatentPathBottleneckDiffusionSeq, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = LatentPathBottleneckDiffusionSeqConfig(**payload["config"])
    model = LatentPathBottleneckDiffusionSeq(cfg)
...
def save_checkpoint(
    path: str,
    model: LatentPathBottleneckDiffusionSeq,
    cfg: LatentPathBottleneckDiffusionSeqConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
