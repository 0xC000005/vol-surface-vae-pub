...
@dataclass
class MultiscaleFutureLogitPathFMConfig(JointTokenLogitTransitionFMConfig):
    knot_positions: tuple[int, ...] = (0, 3, 7, 13, 20, 29)


class MultiscaleFutureLogitPathVelocity(nn.Module):
    def __init__(self, cfg: MultiscaleFutureLogitPathFMConfig):
        super().__init__()
        self.cfg = cfg
    ...
    def forward(
        ...
        vel = self.out(hidden[:, 2:, :]).squeeze(-1).view(bsz, horizon, n_cells)
        return vel[:, self.cfg.knot_positions, :], vel[:, ~knot_mask[:, 0], :]


class MultiscaleFutureLogitPathFlowMatching(JointTokenLogitTransitionFlowMatching):
    """315a-v0: one-stage multiscale flow over coarse knot path and fine residual coordinates."""
    ...
    def __init__(self, cfg: MultiscaleFutureLogitPathFMConfig):
        ...
        self.velocity = MultiscaleFutureLogitPathVelocity(cfg)
...
def load_model(
    checkpoint_path: str,
    device: torch.device,
) -> tuple[MultiscaleFutureLogitPathFlowMatching, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = MultiscaleFutureLogitPathFMConfig(**payload["config"])
    model = MultiscaleFutureLogitPathFlowMatching(cfg)
...
def save_checkpoint(
    path: str,
    model: MultiscaleFutureLogitPathFlowMatching,
