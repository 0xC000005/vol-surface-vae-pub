...
@dataclass
class JointProbabilisticLatentFactorFMConfig:
    history_len: int = 30
    future_len: int = 30
    n_cells: int = 25
    latent_dim: int = 8
    ...
    change_coord: str = "asinh_local_scale"  # "raw" | "asinh_local_scale"
    change_scale_eps: float = 1e-3


class JointProbabilisticLatentFactorFM(nn.Module):
    """262a-v0: joint probabilistic latent-factor FM.

    The model is intentionally minimal:
    - history encoder
    - future latent mean path mu_t
    - future latent positive scale path sigma_t
    - explicit low-rank loadings Lambda(h)
    - bounded deterministic idio mean path
    - vanilla FM over standardized latent innovations eps_t
    """

    def __init__(self, cfg: JointProbabilisticLatentFactorFMConfig):
        super().__init__()
        self.cfg = cfg
    ...
    def condition(self, history_norm: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        mean_coord = mean_common + mean_idio
    ...
    def deterministic_center_path(
        ...
        change_coord = self.decode_change_coord(zero_eps, cond)
        raw_change = self.inverse_transform_change(change_coord, history_norm)
    ...
    def sample_batched(
        ...
        **_: object,
        ...
        for start in range(0, n_samples, chunk_size):
            ...
            change_coord = self.decode_change_coord(eps, cond)
            raw_change = self.inverse_transform_change(change_coord, history_norm)
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple[JointProbabilisticLatentFactorFM, dict]:
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    cfg = JointProbabilisticLatentFactorFMConfig(**payload["config"])
    model = JointProbabilisticLatentFactorFM(cfg)
...
def save_checkpoint(
    path: str,
    model: JointProbabilisticLatentFactorFM,
    cfg: JointProbabilisticLatentFactorFMConfig,
    epoch: int,
    best_val: float,
) -> None:
    torch.save(
