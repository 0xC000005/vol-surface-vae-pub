...
**Mechanistic diagnosis:** The primary 233a failure is **scale regression + sign-correction oscillation** under self-feeding, not raw compounding. The slow-path FiLM modulation is creating a negative feedback loop (model corrects each step's overshoot, building -0.34 lag-1 autocorrelation). This artificially flattens the KS at late horizons by producing a mean-reverting innovation sequence instead of i.i.d. draws.
...
**Mechanistic diagnosis:** The 229a failure is **variance collapse** (not compounding in the classical sense). Under self-feeding, MSD falls 10x by h30: the model's EWMA scale estimate drifts toward the noise floor as self-generated moves are smaller than GT (each generation slightly underestimates, which shrinks local_scale, which further shrinks next-generation, etc.). Teacher-forcing confirms this diagnosis: TF feeds GT moves (larger) which partially counteracts variance collapse in early steps, but the GRU state update was trained for self-fed dynamics and enters an off-distribution regime under TF, producing over-autocorrelated innovations that worsen KS at long horizons.

**Critical implication for the RC23 paradigm:** The variance collapse in 229a is a self-reinforcing loop between local_scale (EWMA) and the GRU state — exactly the "tight feedback between temporal state and emission" that the rc23_local_temporal_decomposition design aims to break. The local-vs-temporal decomposition with restricted coupling should prevent this feedback: a separate temporal state module modulates only drift/scale/intensity, not rewriting full emission per step.
