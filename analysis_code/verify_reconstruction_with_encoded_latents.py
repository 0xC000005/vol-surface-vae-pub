"""
Verify quantile model reconstruction using encoded latents (z_mean).

This script tests model performance on training data (days 1000-1060) using
the EXACT latent mean from encoding the full sequence. This isolates decoder
performance by removing encoder uncertainty.
...
def plot_verification(all_predictions, all_ground_truth, model_names):
    ...
    plt.suptitle('Reconstruction Verification: Using Encoded z_mean (2008-2010 Financial Crisis)',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig('reconstruction_verification_2008_2010.png', dpi=150, bbox_inches='tight')
    print("Saved: reconstruction_verification_2008_2010.png")
    plt.close()
...
def print_summary(model_name, metrics):
    ...
    print(f"\nQuantile Ordering:")
    print(f"  Crossing Rate:    {metrics['quantile_crossings']*100:.2f}% (should be 0%)")


def main():
    print("="*60)
    print("RECONSTRUCTION VERIFICATION WITH ENCODED LATENTS (z_mean)")
